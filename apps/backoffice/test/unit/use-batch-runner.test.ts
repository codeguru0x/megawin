 ...
 * thiết kế gốc, xem JSDoc `use-batch-runner.ts`). Test tĩnh (`tsc`/`oxlint`/`prettier`) KHÔNG bắt được lỗi
 ...
 * (`T extends Promise<any> ? Promise<undefined> : undefined`) mà linter không resolve được qua
 * generic alias (đúng caveat đã ghi ở `oxlint-lint-conventions.mdc` §d, case `GetSessionFn`), nên
 * `await act(async () => {...})` bị linter báo sai `useAwaitThenable` dù `tsc --noEmit` xanh (đã
 * verify). Khai báo tường minh `Promise<void>` ở ĐÂY thay vì rải `linter-ignore` ở 11 chỗ gọi.
 */
