...
interface DailyByGameViewProps {
  ...
  to: string;
  game: string;
  onRowClick: (financialDate: string) => void;
}

/**
 * View 2 — Chi tiết ngày tài chính theo game.
 *
 * Cột: Ngày tài chính, Kỳ quay, Phiếu cược, Tiền cược, Trả thưởng, Tỷ lệ TT,
 *       Doanh thu thuần, Hoa hồng ĐL, Lợi nhuận ròng.
 * Click row → drill vào View 3 (draw breakdown).
 */
export function DailyByGameView({ accountId, from, to, game, onRowClick }: DailyByGameViewProps) {
  const { data: rawData, isLoading, isError } = usePlayerFinancials(accountId, from, to, game);
    ...
    return (
      <Card className="gap-0 py-0">
        <CardContent className="flex h-40 flex-col items-center justify-center gap-1 text-center">
          ...
          <p className="text-muted-foreground text-xs">Không có dữ liệu trong khoảng thời gian đã chọn.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card className="gap-0 py-0">
      <CardHeader className="px-5 pt-4 pb-2">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-sm font-semibold">Chi tiết theo ngày</CardTitle>
        </div>
      </CardHeader>
