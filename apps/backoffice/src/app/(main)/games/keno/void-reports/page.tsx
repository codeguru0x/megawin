...
import { useKenoVoidReports } from "../financial-reports/_lib/use-report-queries";
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Snapshot trước khi Void — Keno</DialogTitle>
          <DialogDescription>
...
function VoidTable({ from, to }: { from: string; to: string }) {
  ...
  const { data, isLoading, error } = useKenoVoidReports(from, to);
...
function KenoVoidReportsContent() {
  const today = todayVN();
    ...
    <div className="@container/main flex flex-col gap-4 md:gap-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold tracking-tight text-foreground">Keno — Kỳ huỷ</h1>
...
export default function KenoVoidReportsPage() {
  return (
    <Suspense
      fallback={
