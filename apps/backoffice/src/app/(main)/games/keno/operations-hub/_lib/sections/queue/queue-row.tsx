...
export interface QueueRowProps {
  ...
  entries: number;
  sets: number;
  /** Exposure xấu nhất RAW (VND, chưa cap) — cột mới (review 07/09 §A7: staff cần thấy rủi ro
   * tiền per-kỳ ngay ở bảng, không phải mở panel mới biết). */
  exposureRaw: number;
 ...
 * không trả theo `KenoOpsAlertType` (xem `hub-snapshot.dto.ts` — `alertsOpen`/`alertsCritical`
