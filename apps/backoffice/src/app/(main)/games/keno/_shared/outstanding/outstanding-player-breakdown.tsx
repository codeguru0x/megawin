...
import { useKenoOutstandingTenantPlayers } from "../../financial-reports/_lib/use-report-queries";
 ...
 *
 * Tự gọi useKenoOutstandingFilters() để lấy navigateToPlayer.
...
export function OutstandingPlayerBreakdown({ drawId, tenantId }: OutstandingPlayerBreakdownProps) {
  const { navigateToPlayer } = useKenoOutstandingFilters();
  const { data, isLoading, error, refetch } = useKenoOutstandingTenantPlayers(drawId, tenantId);
    ...
    <Card className="gap-0 py-0">
      <CardHeader className="px-5 pb-2 pt-4">
        ...
        <CardDescription className="text-xs">
          {rows.length} players · Click để xem entries
        </CardDescription>
      </CardHeader>
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow
                    ...
                    onClick={() => navigateToPlayer(row.accountId, row.username)}
                  >
