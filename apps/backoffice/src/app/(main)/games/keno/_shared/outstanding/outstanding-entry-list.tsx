...
import { KenoEntryDetailDialog } from "../../financial-reports/_lib/sections/entry-list";
import { useKenoOutstandingPlayerEntries } from "../../financial-reports/_lib/use-report-queries";
 ...
 *
 * Click row → mở KenoEntryDetailDialog ở outstanding mode
 * (status = "scheduled" → dialog ẩn result/payout, hiển thị "Kết quả có sau kỳ quay").
 * Keno boards: unified pick1-pick10 + bigSmall + evenOdd — dialog xử lý tất cả.
 * Keno KHÔNG có lineCount — 4 cột.
 */
export function OutstandingEntryList({
  ...
  const { data, isLoading, error, refetch } = useKenoOutstandingPlayerEntries(
    drawId,
    tenantId,
    accountId,
  );
