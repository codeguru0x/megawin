...
import { Clock, RefreshCw, CalendarClock, Ticket, Banknote, HandCoins, X } from "lucide-react";
...
function KpiStrip({ data }: KpiStripProps) {
  ...
  // Keno không có lineCount — chỉ 4 KPI cards
  return (
...
function OutstandingTable() {
  const { data, isLoading, error, dataUpdatedAt, isFetching, refetch } = useKenoOutstanding();
  ...
  // Tổng cho footer row — Keno không có lineCount
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Table Card — không có CardDescription vì Keno có ~10+ kỳ, thông tin đã rõ */}
      <Card className="gap-0 py-0">
        <CardHeader className="px-5 pb-2 pt-4">
          <div className="flex items-center gap-2">
                ...
                <TableBody>
                  {filteredRows.map((row) => (
                    <TableRow key={row.drawId}>
                      ...
                      <TableCell>
                        <Link
                          href={`/games/keno/operations?draw=${row.drawId}`}
