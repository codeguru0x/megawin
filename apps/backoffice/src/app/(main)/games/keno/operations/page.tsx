 ...
 *   4. Analytics (play type, heatmap 80 số, live feed)
 */
...
function LastUpdatedBadge({ drawId }: { drawId: string | undefined }) {
  ...
  useEffect(() => {
    function tick() {
      const state = qc.getQueryState(kenoKeys.opsSnapshot(drawId ?? ""));
    ...
    <div className="@container/main flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-foreground text-lg font-semibold tracking-tight">Keno — Vận hành</h1>
            <div className="flex items-center gap-2">
              <p className="text-muted-foreground text-xs">Quản lý và giám sát kỳ quay (~119 kỳ/ngày)</p>
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="gap-6">
        ...
        {/* Tab Giám sát: draw-management → Alerts (tín hiệu cần hành động, ưu tiên
            đọc ngay dưới command center) → KPI (+Exposure, nhịp tim kỳ) → Result. */}
        ...
        {/* Tab Phân tích cược: unmount khi ở tab Giám sát (heatmap 80 cell không render) */}
    ...
    <div className="@container/main flex flex-col gap-6">
      ...
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed py-20 text-center">
        ...
        <p className="text-muted-foreground mt-1 max-w-sm text-sm">
          {noData
            ? "Hệ thống chưa có kỳ quay Keno nào được tạo. Hãy tạo kỳ quay đầu tiên để bắt đầu vận hành."
            : "Kỳ quay được yêu cầu không tồn tại hoặc đã bị xóa khỏi hệ thống."}
        </p>
        <div className="mt-5 flex items-center gap-3">
          <Button variant="outline" size="sm" asChild>
            <Link prefetch={false} href="/games/keno/draws">
              Lịch sử kỳ quay
            </Link>
          </Button>
          {noData ? (
            ...
            <Button size="sm" asChild>
              <Link prefetch={false} href="/games/keno/operations">
                Về trang vận hành
              </Link>
            </Button>
          )}
...
export default function KenoOperationsPage() {
  return (
    <Suspense>
      <DrawContextProvider>
        <OperationsContent />
