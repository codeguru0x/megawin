...
import { useKenoPlayers, useKenoTenantDraws, useKenoTenantList } from "../use-report-queries";
...
function TenantDrawList({ tenantId }: { tenantId: string }) {
  ...
  return (
    <GameTenantDrawList tenantId={tenantId} rows={rows} totalCount={data.total} onRowClick={navigateToDrawInTenant} />
  );
}
...
function PlayerBreakdown({ drawId, tenantId }: { drawId: string; tenantId: string }) {
  const { navigateToEntriesFromTenant } = useKenoReportFilters();
...
export function ByTenantTab() {
  const { level, tenantId, drawId, accountId, playerName } = useKenoReportFilters();
