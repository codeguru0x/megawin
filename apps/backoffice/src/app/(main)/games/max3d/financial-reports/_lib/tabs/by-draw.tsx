...
function KpiStrip({ from, to }: { from: string; to: string }) {
  const { data, isLoading } = useMax3DDrawSummary(from, to);
  ...
  // Max 3D: bỏ JP Contribution, có 5 KPI cards
  const cards = [
    {
      label: "Kỳ Quay",
...
/** Tab "Theo Kỳ Quay" — 4 cấp drill-down. Max 3D: bỏ JP Contribution, có Lines. */
export function ByDrawTab() {
  const { from, to, level, drawId, tenantId, playerId } = useMax3DReportFilters();
