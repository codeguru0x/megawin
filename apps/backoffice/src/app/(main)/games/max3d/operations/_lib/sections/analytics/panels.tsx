...
export function PairTable({ rows }: { rows: PairRow[] }) {
  return (
    <Card className="gap-0 py-0 shadow-sm">
      ...
      <CardContent className="px-5 pt-0 pb-4">
        {rows.length === 0 ? (
          <p className="text-muted-foreground py-6 text-center text-sm">Chưa có cược Max 3D+</p>
        ) : (
            ...
            {rows.map((r) => (
              ...
              >
                <span className="inline-flex items-center gap-1 font-mono text-xs font-bold tabular-nums">
                  <span className="bg-game-max3d/15 text-game-max3d rounded px-1 py-0.5">{r.triplet1}</span>
                  <span className="text-muted-foreground/50">–</span>
                  <span className="bg-game-max3d/15 text-game-max3d rounded px-1 py-0.5">{r.triplet2}</span>
                </span>
