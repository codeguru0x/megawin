...
export function useVietlottResult(drawId: string | undefined, drawPeriod: string, enabled: boolean) {
  return useQuery({
    queryKey: max3dKeys.vietlottResult(drawId ?? "", drawPeriod),
    queryFn: () =>
      apiClient.get<GetVietlottResultOutput>(`/max3d/draws/${drawId}/vietlott-result`, {
        params: { drawPeriod },
      }),
 ...
 * worker `ops.stats.tickSeconds`, Max 3D default 30s) → FE khớp cadence worker.
 *
...
export function useOpsSnapshot<TData = GetOpsSnapshotOutput>(
  ...
  return useQuery({
    queryKey: max3dKeys.opsSnapshot(drawId ?? ""),
    queryFn: () =>
...
export function useAlerts(drawId: string | undefined, status: string | undefined, enabled: boolean) {
  return useQuery({
    queryKey: max3dKeys.opsAlerts(drawId ?? "", status),
    queryFn: () =>
...
/**
 * Acknowledge 1 alert. Invalidate toàn bộ Max 3D cache để refresh cả panel alert
