...
function LastUpdatedBadge({ drawId }: { drawId: string | undefined }) {
  ...
  useEffect(() => {
    function tick() {
      const state = qc.getQueryState(max3dKeys.opsSnapshot(drawId ?? ""));
    ...
    <div className="@container/main flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-foreground text-lg font-semibold tracking-tight">Max 3D — Vận hành</h1>
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="gap-6">
        ...
        {/* Tab Giám sát: draw-management → Alerts (đầu tab) → KPI (+Exposure) → Result. */}
        ...
        {/* Tab Phân tích cược: unmount khi ở tab Giám sát. */}
      ...
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed py-20 text-center">
        ...
        <div className="mt-5 flex items-center gap-3">
          <Button variant="outline" size="sm" asChild>
            <Link prefetch={false} href="/games/max3d/draws">
              Lịch sử kỳ quay
            </Link>
          </Button>
          {noData ? (
            ...
            <Button size="sm" asChild>
              <Link prefetch={false} href="/games/max3d/operations">
                Về trang vận hành
              </Link>
            </Button>
          )}
...
export default function Max3dOperationsPage() {
  return (
    <Suspense>
      <DrawContextProvider>
        <OperationsContent />
