 ...
 *
 * Báo cáo entries trúng thưởng Max 3D.
 * Max 3D đặc thù:
...
function BoardChip({ board }: { board: WinningEntryItem["boards"][number] }) {
  const modeLabel =
    MAX3D_PLAY_MODE_LABELS[board.playMode as keyof typeof MAX3D_PLAY_MODE_LABELS] ?? board.playMode;
  const typeLabel =
    MAX3D_PLAY_TYPE_LABELS[board.playType as keyof typeof MAX3D_PLAY_TYPE_LABELS] ?? board.playType;
  const isPlus = board.playMode === "plus";
      ...
      <div className="flex flex-col gap-0.5">
        <div className="flex items-center gap-1 flex-wrap">
          {board.triplets.map((t, i) => (
            <TripletDisplay key={i} value={t} variant={isPlus ? "special" : "first"} size="sm" />
          ))}
        </div>
...
function TierChip({ tier }: { tier: WinningEntryItem["tiers"][number] }) {
  const label =
    ...
    tier.tier;
  const badgeClass =
    TIER_BADGE_COLORS[tier.tier] ?? "border-border bg-muted/40 text-muted-foreground";
