...
/**
 * Max 3D Operations — Alerts Panel (tab Giám sát)
 *
 ...
 * Đặc biệt quan trọng với Max 3D: `pair_liability` bắn 1 alert / cặp, kỳ bán nhiều
...
import { MAX3D_OPS_ALERT_TYPE_LABELS } from "../../ops-constants";
...
function describeAlert(type: string, payload: Record<string, unknown>): AlertDescription {
  switch (type) {
    case Max3dOpsAlertType.LargeBet: {
    ...
    case Max3dOpsAlertType.ExposureThreshold: {
      ...
      return {
        ...
        chips: [
          ...
          {
            label: "Cặp ĐB max",
            value: `${formatNumber(toNum(payload.maxPairLiability))} VND`,
            danger: true,
          },
          {
    ...
    case Max3dOpsAlertType.PairLiability: {
      ...
      return {
        ...
        chips: [
          ...
          { label: "Số bộ", value: formatNumber(units) },
          { label: "Account", value: formatNumber(toNum(payload.accounts)) },
          { label: "Tiền vào cặp", value: `${formatNumber(toNum(payload.amount))} VND` },
        ],
      };
    }
    case Max3dOpsAlertType.ComboConcentration: {
      const accounts = toNum(payload.accounts);
      return {
        summary: `${formatNumber(accounts)} account khác nhau cùng cược cặp ${String(payload.triplet1)}–${String(payload.triplet2)} (nghi syndicate).`,
        chips: [
...
function AlertItemRow({ alert }: { alert: Max3dOpsAlertEntity }) {
 ...
 */
function AlertGroupContent({ items }: { items: Max3dOpsAlertEntity[] }) {
