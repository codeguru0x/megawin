...
function VoidTable({ from, to }: { from: string; to: string }) {
  ...
  const { data, isLoading, error } = useMax3DVoidReports(from, to);
...
function Max3DVoidReportsContent() {
  const today = todayVN();
...
export default function Max3DVoidReportsPage() {
  return (
    <Suspense
      fallback={
