...
import {
  useMax3DOutstanding,
  useMax3DOutstandingDrawTenants,
  useMax3DOutstandingPlayerEntries,
  useMax3DOutstandingTenantPlayers,
} from "../../settle/_lib/use-report-queries";
import { useMax3DOutstandingFilters } from "./use-outstanding-filters";
...
/** Map entry entity → shared OutstandingEntryRow. Max 3D dùng lineCount cho số cặp số. */
...
/**
 * Wrapper cho Max 3D Outstanding — kết nối game-specific hooks vào shared OutstandingContent.
 *
 * showLineCount=true vì Max 3D có "cặp số" (lineCount = số cặp bộ ba số).
 * toTenantUsername áp dụng tại mapPlayerRow: bỏ suffix @local / @tenantId.
 */
export function Max3DOutstandingContent() {
  const {
    drawId,
    tenantId,
    playerId,
    ...
    navigateToTenant,
    navigateToPlayer,
  } = useMax3DOutstandingFilters();

  const [selectedEntry, setSelectedEntry] = useState<TicketEntryEntity | null>(null);

  // Level 1 — draws outstanding
  const drawsQuery = useMax3DOutstanding();

  // Level 2 — tenant breakdown cho draw đang drill
  const tenantQuery = useMax3DOutstandingDrawTenants(drawId);

  // Level 3 — player breakdown cho draw × tenant đang drill
  const playerQuery = useMax3DOutstandingTenantPlayers(drawId ?? "", tenantId);

  // Level 4 — entries của player đang drill
  const entryQuery = useMax3DOutstandingPlayerEntries(drawId ?? "", tenantId ?? "", playerId);
