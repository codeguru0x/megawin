...
const BASIC_FIELDS = [
  { key: "basicSpecial" as const, tier: "special" as const, label: "Giải Đặc Biệt", desc: "Trùng 1 trong 2 bộ ĐB", badge: "ĐB", color: "bg-red-600 text-white" },
  { key: "basicFirst" as const, tier: "first" as const, label: "Giải Nhất", desc: "Trùng 1 trong 4 bộ Nhất", badge: "1st", color: "bg-amber-500 text-white" },
  { key: "basicSecond" as const, tier: "second" as const, label: "Giải Nhì", desc: "Trùng 1 trong 6 bộ Nhì", badge: "2nd", color: "bg-slate-400 text-white" },
  { key: "basicThird" as const, tier: "third" as const, label: "Giải Ba", desc: "Trùng 1 trong 8 bộ Ba", badge: "3rd", color: "bg-amber-700 text-white" },
] as const;

const COMBO3_FIELDS = [
  { key: "combo3Special" as const, tier: "special" as const, label: "Giải Đặc Biệt", desc: "3 hoán vị, trùng bộ ĐB", badge: "ĐB", color: "bg-red-600 text-white" },
  { key: "combo3First" as const, tier: "first" as const, label: "Giải Nhất", desc: "3 hoán vị, trùng bộ Nhất", badge: "1st", color: "bg-amber-500 text-white" },
  { key: "combo3Second" as const, tier: "second" as const, label: "Giải Nhì", desc: "3 hoán vị, trùng bộ Nhì", badge: "2nd", color: "bg-slate-400 text-white" },
  { key: "combo3Third" as const, tier: "third" as const, label: "Giải Ba", desc: "3 hoán vị, trùng bộ Ba", badge: "3rd", color: "bg-amber-700 text-white" },
] as const;

const COMBO6_FIELDS = [
  { key: "combo6Special" as const, tier: "special" as const, label: "Giải Đặc Biệt", desc: "6 hoán vị, trùng bộ ĐB", badge: "ĐB", color: "bg-red-600 text-white" },
  { key: "combo6First" as const, tier: "first" as const, label: "Giải Nhất", desc: "6 hoán vị, trùng bộ Nhất", badge: "1st", color: "bg-amber-500 text-white" },
  { key: "combo6Second" as const, tier: "second" as const, label: "Giải Nhì", desc: "6 hoán vị, trùng bộ Nhì", badge: "2nd", color: "bg-slate-400 text-white" },
  { key: "combo6Third" as const, tier: "third" as const, label: "Giải Ba", desc: "6 hoán vị, trùng bộ Ba", badge: "3rd", color: "bg-amber-700 text-white" },
] as const;

const PLUS_FIELDS = [
  { key: "plusSpecial" as const, tier: "special" as const, label: "Giải Đặc Biệt", desc: "2 bộ đều trùng 2 bộ ĐB", badge: "ĐB", color: "bg-red-600 text-white" },
  { key: "plusFirst" as const, tier: "first" as const, label: "Giải Nhất", desc: "2 bộ đều trùng trong 4 bộ Nhất", badge: "1st", color: "bg-amber-500 text-white" },
  { key: "plusSecond" as const, tier: "second" as const, label: "Giải Nhì", desc: "2 bộ đều trùng trong 6 bộ Nhì", badge: "2nd", color: "bg-slate-400 text-white" },
  { key: "plusThird" as const, tier: "third" as const, label: "Giải Ba", desc: "2 bộ đều trùng trong 8 bộ Ba", badge: "3rd", color: "bg-amber-700 text-white" },
  { key: "plusFourth" as const, tier: "fourth" as const, label: "Giải Tư", desc: "2 bộ trùng bất kỳ (cross-tier)", badge: "4th", color: "bg-slate-500 text-white" },
  { key: "plusFifth" as const, tier: "fifth" as const, label: "Giải Năm", desc: "1 bộ trùng bộ ĐB", badge: "5th", color: "bg-slate-600 text-white" },
  { key: "plusSixth" as const, tier: "sixth" as const, label: "Giải Sáu", desc: "1 bộ trùng bộ Nhất/Nhì/Ba", badge: "6th", color: "bg-emerald-600 text-white" },
] as const;
...
export function PrizesSection({ config, onSave, isPending }: PrizesSectionProps) {
  ...
  const basicAnalysis = useMemo(
    () => analyzeBasicStraightProfitability(
      { special: w.basicSpecial, first: w.basicFirst, second: w.basicSecond, third: w.basicThird },
      unitPrice,
    ),
  ...
  const combo3Analysis = useMemo(
    () => {
      ...
      const prizes = { special: w.combo3Special, first: w.combo3First, second: w.combo3Second, third: w.combo3Third };
      const tiers = odds.map((o) => {
        const prize = prizes[o.tier];
        const ep = o.probability * prize;
        return { tier: o.tier, label: o.label, probability: o.probability, oneInN: o.oneInN, currentPrize: prize, expectedPayout: ep, payoutRatio: unitPrice > 0 ? ep / unitPrice : 0, breakEvenPrize: o.probability > 0 ? unitPrice / o.probability : Infinity };
      });
      const totalEP = tiers.reduce((s, t) => s + t.expectedPayout, 0);
      const gm = unitPrice - totalEP;
      return { tiers, totalExpectedPayout: totalEP, totalPayoutRatio: unitPrice > 0 ? totalEP / unitPrice : 0, grossMarginPerLine: gm, grossMarginPercent: unitPrice > 0 ? (gm / unitPrice) * 100 : 0 };
    },
  ...
  const combo6Analysis = useMemo(
    () => {
      ...
      const prizes = { special: w.combo6Special, first: w.combo6First, second: w.combo6Second, third: w.combo6Third };
      const tiers = odds.map((o) => {
        const prize = prizes[o.tier];
        const ep = o.probability * prize;
        return { tier: o.tier, label: o.label, probability: o.probability, oneInN: o.oneInN, currentPrize: prize, expectedPayout: ep, payoutRatio: unitPrice > 0 ? ep / unitPrice : 0, breakEvenPrize: o.probability > 0 ? unitPrice / o.probability : Infinity };
  ...
  const plusAnalysis = useMemo(
    () => analyzePlusProfitability(
      { special: w.plusSpecial, first: w.plusFirst, second: w.plusSecond, third: w.plusThird, fourth: w.plusFourth, fifth: w.plusFifth, sixth: w.plusSixth },
      unitPrice,
    ),
  ...
  function handleSubmit(values: PrizesFormValues) {
    onSave({
      defaultPrizes: {
        basic: { special: values.basicSpecial, first: values.basicFirst, second: values.basicSecond, third: values.basicThird },
        combo: {
          combo3: { special: values.combo3Special, first: values.combo3First, second: values.combo3Second, third: values.combo3Third },
          combo6: { special: values.combo6Special, first: values.combo6First, second: values.combo6Second, third: values.combo6Third },
        },
        plus: { special: values.plusSpecial, first: values.plusFirst, second: values.plusSecond, third: values.plusThird, fourth: values.plusFourth, fifth: values.plusFifth, sixth: values.plusSixth },
      },
    });
  }
            ...
            <Tabs defaultValue="basic" className="w-full">
              <div className="border-b px-6 pt-4">
                <TabsList variant="line" className="w-full justify-start gap-0 border-b-0 px-0">
                  <TabsTrigger value="basic" className="gap-1.5 text-xs">Cơ Bản (Straight)</TabsTrigger>
                  <TabsTrigger value="combo" className="gap-1.5 text-xs">Tổ Hợp (Combo)</TabsTrigger>
                  <TabsTrigger value="plus" className="gap-1.5 text-xs">Max 3D+</TabsTrigger>
                </TabsList>
              </div>
              ...
              <TabsContent value="basic" className="mt-0">
                <div className="p-6 pb-4">
                  <ProfitBar analysis={basicAnalysis} unitPrice={unitPrice} totalOutcomes={BASIC_TOTAL_OUTCOMES} modeLabel="Max 3D Cơ Bản — Straight" />
                </div>
              ...
              <TabsContent value="combo" className="mt-0">
                <div className="p-6 pb-4">
                  <ProfitBar analysis={combo3Analysis} unitPrice={unitPrice} totalOutcomes={BASIC_TOTAL_OUTCOMES} modeLabel="Tổ Hợp 3 (Combo3) — 3 hoán vị" />
                </div>
                <div className="border-t overflow-x-auto">
                  <TableHeader />
                ...
                <div className="border-t p-6 pb-4">
                  <ProfitBar analysis={combo6Analysis} unitPrice={unitPrice} totalOutcomes={BASIC_TOTAL_OUTCOMES} modeLabel="Tổ Hợp 6 (Combo6) — 6 hoán vị" />
                </div>
                <div className="border-t overflow-x-auto">
                  <TableHeader />
              ...
              <TabsContent value="plus" className="mt-0">
                <div className="p-6 pb-4">
                  <ProfitBar analysis={plusAnalysis} unitPrice={unitPrice} totalOutcomes={PLUS_TOTAL_OUTCOMES} modeLabel="Max 3D+ — 2 bộ ba số" />
                </div>
                <div className="border-t overflow-x-auto">
                  <TableHeader />
