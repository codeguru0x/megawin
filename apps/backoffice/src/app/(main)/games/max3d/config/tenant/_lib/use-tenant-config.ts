...
const LIST_KEY = ["max3d", "tenant-configs"] as const;

function detailKey(tenantId: string) {
  return ["max3d", "tenant-config", tenantId] as const;
}

export function useTenantConfigs() {
  return useQuery({
    queryKey: LIST_KEY,
    queryFn: () => apiClient.get<{ configs: TenantConfig[] }>("/max3d/tenant-config").then((r) => r.configs),
  });
}
...
export function useTenantConfig(tenantId: string | null) {
  return useQuery({
    ...
    queryFn: () => apiClient.get<{ config: TenantConfig }>(`/max3d/tenant-config/${tenantId}`).then((r) => r.config),
    enabled: !!tenantId,
  });
}
...
export function useCreateTenantConfig() {
  ...
  return useMutation({
    mutationFn: (tenantId: string) =>
      apiClient.put<{ config: TenantConfig; version: number }>(`/max3d/tenant-config/${tenantId}`, { isEnabled: true }),
...
export function useUpdateTenantConfig(tenantId: string) {
  ...
  return useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      apiClient.put<{ config: TenantConfig; version: number }>(`/max3d/tenant-config/${tenantId}`, data),
    onSuccess: (res) => {
      queryClient.setQueryData(detailKey(tenantId), res.config);
      queryClient.invalidateQueries({ queryKey: LIST_KEY });
