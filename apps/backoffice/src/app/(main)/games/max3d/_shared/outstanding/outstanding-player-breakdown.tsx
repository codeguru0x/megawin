...
import { useMax3DOutstandingTenantPlayers } from "../../financial-reports/_lib/use-report-queries";
 ...
 *
 * Tự gọi useMax3DOutstandingFilters() để lấy navigateToPlayer — cùng pattern với financial reports.
...
export function OutstandingPlayerBreakdown({ drawId, tenantId }: OutstandingPlayerBreakdownProps) {
  const { navigateToPlayer } = useMax3DOutstandingFilters();
  const { data, isLoading, error, refetch } = useMax3DOutstandingTenantPlayers(drawId, tenantId);
