...
import { useMax3DOutstandingDrawTenants } from "../../financial-reports/_lib/use-report-queries";
 ...
 *
 * Tự gọi useMax3DOutstandingFilters() để lấy navigateToTenant — cùng pattern với financial reports.
...
export function OutstandingTenantBreakdown({ drawId }: OutstandingTenantBreakdownProps) {
  const { navigateToTenant } = useMax3DOutstandingFilters();
  const { data, isLoading, error, refetch } = useMax3DOutstandingDrawTenants(drawId);
