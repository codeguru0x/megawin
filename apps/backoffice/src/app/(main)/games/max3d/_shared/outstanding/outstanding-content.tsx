  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
      <KpiCard
        ...
        sub="kỳ quay chưa settle (T2, T4, T6)"
      />
      <KpiCard
        icon={Ticket}
...
/**
 * Orchestrator cho Max 3D Outstanding drill-down.
 *
 * Render đúng level component dựa trên URL state từ useMax3DOutstandingFilters.
 * KpiStrip chỉ hiển thị ở Level 1.
 * Breadcrumb hiển thị từ Level 2 trở lên.
 */
export function OutstandingContent() {
  const { drawId, tenantId, playerId, playerName, level } = useMax3DOutstandingFilters();

  const { data, isLoading, error, isFetching, refetch } = useMax3DOutstanding();
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Level components — mỗi child tự gọi hook, KHÔNG nhận callbacks qua props */}
