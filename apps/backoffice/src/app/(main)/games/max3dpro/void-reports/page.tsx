...
import { useMax3DProVoidReports } from "../financial-reports/_lib/use-report-queries";
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Snapshot trước khi Void</DialogTitle>
          <DialogDescription>
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <div className="rounded-lg border bg-card p-4">
        ...
        <p className="mt-1 text-2xl font-bold tabular-nums">{totalVoids}</p>
...
function VoidTable({ from, to }: { from: string; to: string }) {
  ...
  const { data, isLoading, error } = useMax3DProVoidReports(from, to);
...
function Max3DProVoidReportsContent() {
  const today = todayVN();
...
export default function Max3DProVoidReportsPage() {
  return (
    <Suspense
      fallback={
