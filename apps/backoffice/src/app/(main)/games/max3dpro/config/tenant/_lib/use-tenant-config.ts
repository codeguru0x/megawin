...
const LIST_KEY = ["max3dpro", "tenant-configs"] as const;

function detailKey(tenantId: string) {
  return ["max3dpro", "tenant-config", tenantId] as const;
}

export function useTenantConfigs() {
  return useQuery({
    queryKey: LIST_KEY,
    queryFn: () => apiClient.get<{ configs: TenantConfig[] }>("/max3dpro/tenant-config").then((r) => r.configs),
  });
}
...
export function useTenantConfig(tenantId: string | null) {
  return useQuery({
    ...
    queryFn: () => apiClient.get<{ config: TenantConfig }>(`/max3dpro/tenant-config/${tenantId}`).then((r) => r.config),
    enabled: !!tenantId,
  });
}
...
export function useCreateTenantConfig() {
  ...
  return useMutation({
    mutationFn: (tenantId: string) =>
      apiClient.put<{ config: TenantConfig; version: number }>(`/max3dpro/tenant-config/${tenantId}`, {
        isEnabled: true,
      }),
...
export function useUpdateTenantConfig(tenantId: string) {
  ...
  return useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      apiClient.put<{ config: TenantConfig; version: number }>(`/max3dpro/tenant-config/${tenantId}`, data),
    onSuccess: (res) => {
      queryClient.setQueryData(detailKey(tenantId), res.config);
      queryClient.invalidateQueries({ queryKey: LIST_KEY });
