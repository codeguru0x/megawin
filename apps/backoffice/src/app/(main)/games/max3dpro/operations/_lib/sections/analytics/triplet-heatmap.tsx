...
/**
 * Max 3D Pro — Triplet Analytics Panel
 *
 ...
 *
 * Pattern đồng nhất với Max 3D: heat intensity, ranked list, SummaryRibbon.
 * Max 3D Pro specific: chỉ 1 loại combo (ordered pair TripletPair).
 */
...
const BAR_COLORS: Record<HeatLevel, string> = {
  ...
  low: "bg-pink-300/60 dark:bg-pink-700/40",
  mid: "bg-pink-400/70 dark:bg-pink-600/50",
  warm: "bg-pink-500/80 dark:bg-pink-500/60",
  hot: "bg-amber-500 dark:bg-amber-500/80",
};

const RANK_STYLES: Record<HeatLevel, string> = {
  cold: "text-muted-foreground",
  low: "text-pink-500/70 dark:text-pink-400/70",
  mid: "text-pink-600 dark:text-pink-400",
  warm: "text-pink-700 dark:text-pink-300",
...
function RankedTripletList({ triplets }: { triplets: TripletFreq[] }) {
  const maxCount = triplets[0]?.count ?? 1;
...
function TopPairCombos({ combos }: { combos: TopPairComboItem[] }) {
  ...
  return (
    <Card className="gap-0 py-0 shadow-sm">
      ...
      <CardContent className="px-5 pb-4 pt-0 space-y-4">
        ...
        {pairCombos && pairCombos.length > 0 && <TopPairCombos combos={pairCombos} />}
