...
const vietlottConfigLink = vietlottConfigHref(GameProduct.Max3dpro);
...
const TIER_CONFIG: TierConfigEntry[] = (() => {
  const base: { key: TierVariant; label: string; count: number }[] = [
    { key: "special", label: "Giải Đặc Biệt", count: MAX3D_PRO_DRAW_COUNT_SPECIAL },
    { key: "first", label: "Giải Nhất", count: MAX3D_PRO_DRAW_COUNT_FIRST },
    { key: "second", label: "Giải Nhì", count: MAX3D_PRO_DRAW_COUNT_SECOND },
    { key: "third", label: "Giải Ba", count: MAX3D_PRO_DRAW_COUNT_THIRD },
  ];
  let offset = 0;
...
function validateMax3dProTriplets(triplets: string[]): ValidationResult {
...
export function PublishResultAction({
  ...
  const [triplets, setTriplets] = useState<string[]>(Array(MAX3D_PRO_DRAW_TOTAL).fill(""));
  ...
  useEffect(() => {
    if (isOpen && currentResult) {
      ...
      setTriplets(flattened.length === MAX3D_PRO_DRAW_TOTAL ? flattened : Array(MAX3D_PRO_DRAW_TOTAL).fill(""));
      ...
      setTriplets(Array(MAX3D_PRO_DRAW_TOTAL).fill(""));
  ...
  function applyIncomingNumbers() {
    ...
    if (!data?.found || !data.numbers || data.numbers.length < MAX3D_PRO_DRAW_TOTAL) {
      return;
    }
    setTriplets(data.numbers.slice(0, MAX3D_PRO_DRAW_TOTAL));
    setValidation(VALID);
    setHasAppliedAutoResult(true);
  }

  // Tự động điền — CHỈ khi TẤT CẢ ô đang rỗng — KHÔNG tự điền phần thiếu khi form đã có bất
  // kỳ số nào ở BẤT KỲ hạng giải nào (quy tắc bất biến §5.0/§11.0 quy tắc A).
  useEffect(() => {
    const data = vietlottResultQuery.data;
    if (
      data?.found &&
      data.numbers &&
      data.numbers.length >= MAX3D_PRO_DRAW_TOTAL &&
  ...
  // lần (plan §11.0 quy tắc B + §11.3 bảng cắt lát). Không gộp thành 1 mảng phẳng.
   ...
   *   Với Max 3D Pro, thứ tự 2 số ĐB (ô 1, ô 2) có ý nghĩa (phân biệt giải ĐB/phụ ĐB)
   *   nên PHẢI giữ đúng thứ tự dán vào, không sắp xếp lại.
  ...
  function handleGridPaste(e: React.ClipboardEvent<HTMLDivElement>) {
    ...
    if (tokens.length === MAX3D_PRO_DRAW_TOTAL) {
      setTriplets(tokens);
      setValidation(validateMax3dProTriplets(tokens));
      setPasteNotice(null);
      requestAnimationFrame(() => inputRefs.current[MAX3D_PRO_DRAW_TOTAL - 1]?.focus());
      return;
    }
    ...
    if (startIndex + tokens.length > MAX3D_PRO_DRAW_TOTAL) {
      setPasteNotice(
        `Dán được ${tokens.length} bộ số, không khớp đủ ${MAX3D_PRO_DRAW_TOTAL - startIndex} ô còn lại. Copy đúng phần kết quả (không kèm ngày/mã kỳ) rồi dán lại.`,
      );
      return;
    }
    ...
    requestAnimationFrame(() => inputRefs.current[Math.min(lastFilled + 1, MAX3D_PRO_DRAW_TOTAL - 1)]?.focus());
  }

  function fillRandom() {
    setTriplets(Array.from({ length: MAX3D_PRO_DRAW_TOTAL }, generateRandomTriplet));
    setValidation(VALID);
  }

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const result = validateMax3dProTriplets(triplets);
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Dice3 className="text-game-max3dpro size-4.5" />
            {formatResultDialogTitle(draw.drawId, draw.drawTime)}
          </DialogTitle>
          <DialogDescription className="flex items-center gap-2">
            Nhập {MAX3D_PRO_DRAW_TOTAL} bộ ba số (000–999): {MAX3D_PRO_DRAW_COUNT_SPECIAL} ĐB +{" "}
            {MAX3D_PRO_DRAW_COUNT_FIRST} Nhất + {MAX3D_PRO_DRAW_COUNT_SECOND} Nhì + {MAX3D_PRO_DRAW_COUNT_THIRD} Ba.
            ...
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="max-w-xs">
                {MAX3D_PRO_DRAW_TOTAL} bộ ba số từ &apos;000&apos; đến &apos;999&apos;. Thứ tự 2 bộ Đặc Biệt phân biệt
                giải ĐB/phụ ĐB. Gồm 4 giải: Đặc Biệt ({MAX3D_PRO_DRAW_COUNT_SPECIAL}), Nhất (
                {MAX3D_PRO_DRAW_COUNT_FIRST}), Nhì ({MAX3D_PRO_DRAW_COUNT_SECOND}), Ba ({MAX3D_PRO_DRAW_COUNT_THIRD}).
              </TooltipContent>
            </Tooltip>
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-5 py-2">
            <div className="flex items-center justify-between">
              <Badge variant="secondary" className="tabular-nums">
                {filledCount}/{MAX3D_PRO_DRAW_TOTAL} bộ số
              </Badge>
                ...
                return (
                  <div key={tier.key} className="mb-3.5 space-y-2.5 last:mb-0">
                    <div className="flex items-center gap-2">
                      ...
                      {tier.key === "special" && (
                        <span className="text-muted-foreground text-xs">(thứ tự quay có ý nghĩa)</span>
                      )}
                    </div>
                            ...
                            <div key={i} className="flex flex-col items-center gap-1">
                              <div className="relative w-full">
                                ...
                                <Input
                                  ...
                                  className={cn(
                                    "w-full text-center font-mono text-sm font-bold tabular-nums",
                                    validation.fieldErrors.has(flatIndex) && "border-destructive",
                                    !validation.fieldErrors.has(flatIndex) && isDiff && "border-warning bg-warning/50",
                                  )}
                                />
                              </div>
                              {showTierDiff && (
            ...
            <VietlottResultPanel
              ...
              diff={combinedDiff}
              totalCount={MAX3D_PRO_DRAW_TOTAL}
