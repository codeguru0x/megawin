...
/**
 * Max 3D Pro Operations — Alerts Panel (tab Giám sát)
 *
 ...
 * Đặc biệt quan trọng với Max 3D Pro: `pair_liability` bắn 1 alert / cặp, kỳ bán nhiều
 * ngày nên alert tích luỹ lâu.
 *
...
import type { Max3dproOpsAlertEntity, Max3dproTopPotential } from "@megawin/game-max3dpro/entities";
import {
 ...
 * `evaluateMax3dAlerts` (p0-04). Loại chưa có formatter → fallback field primitive.
 */
function describeAlert(type: string, payload: Record<string, unknown>): AlertDescription {
  switch (type) {
    case Max3dproOpsAlertType.LargeBet: {
    ...
    case Max3dproOpsAlertType.PairLiability: {
      ...
      return {
        ...
        chips: [
          ...
          { label: "Bộ đúng chiều", value: formatNumber(toNum(payload.unitsForward)) },
          { label: "Bộ chiều ngược", value: formatNumber(toNum(payload.unitsReverse)) },
          { label: "Account", value: formatNumber(toNum(payload.accounts)) },
        ],
      };
    }
    case Max3dproOpsAlertType.ComboConcentration: {
      const accounts = toNum(payload.accounts);
      return {
        summary: `${formatNumber(accounts)} account khác nhau cùng cược cặp ${String(payload.first)}→${String(payload.second)} (nghi syndicate).`,
        chips: [
          { label: "Account", value: formatNumber(accounts), danger: true },
          { label: "Cặp (có thứ tự)", value: `${String(payload.first)} → ${String(payload.second)}` },
          { label: "Số bộ", value: formatNumber(toNum(payload.units)) },
          { label: "Tổng tiền", value: `${formatNumber(toNum(payload.amount))} VND` },
        ],
      };
    }
    default: {
...
/** Đọc `payload.top` (mảng Max3dproTopPotential) an toàn — chỉ large_bet có. */
function readTopEntries(payload: Record<string, unknown>): Max3dproTopPotential[] {
  const top = payload.top;
  if (!Array.isArray(top)) return [];
  return top as Max3dproTopPotential[];
}
...
function AlertTopEntries({
  ...
  return (
    <div className="mt-1 rounded-md border border-border/50 divide-y divide-border/40 overflow-hidden">
      {entries.map((e) => {
        const href = buildOutstandingHref(GameProduct.Max3dpro, drawId, e.accountId, e.username);
        return (
...
function AlertItemRow({ alert }: { alert: Max3dproOpsAlertEntity }) {
 ...
 * (UI v6 — tránh panel dài khi pair_liability sinh nhiều alert per-cặp).
 */
function AlertGroupContent({ items }: { items: Max3dproOpsAlertEntity[] }) {
                ...
                <AccordionTrigger className="py-3 hover:no-underline">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span className="text-sm font-semibold truncate">
                      {MAX3DPRO_OPS_ALERT_TYPE_LABELS[g.type] ?? g.type}
                    </span>
  ...
  return (
    ...
    >
      ...
      <span className="tabular-nums">
        {isCritical
          ? `${formatNumber(counts.critical)} nghiêm trọng`
          : `${formatNumber(counts.new)} mới`}
      </span>
    </button>
  );
}
