...
const TIER_CONFIG = [
  {
    ...
    label: "Giải Đặc Biệt",
    shortLabel: "ĐB",
    count: MAX3D_PRO_DRAW_COUNT_SPECIAL,
  ...
  },
  {
    ...
    label: "Giải Nhất",
    shortLabel: "Nhất",
    count: MAX3D_PRO_DRAW_COUNT_FIRST,
  ...
  },
  {
    ...
    label: "Giải Nhì",
    shortLabel: "Nhì",
    count: MAX3D_PRO_DRAW_COUNT_SECOND,
  ...
  },
  {
    ...
    label: "Giải Ba",
    shortLabel: "Ba",
    count: MAX3D_PRO_DRAW_COUNT_THIRD,
...
const INITIAL_STATE: TripletState = {
  special: Array.from({ length: MAX3D_PRO_DRAW_COUNT_SPECIAL }, () => ""),
  first: Array.from({ length: MAX3D_PRO_DRAW_COUNT_FIRST }, () => ""),
  second: Array.from({ length: MAX3D_PRO_DRAW_COUNT_SECOND }, () => ""),
  third: Array.from({ length: MAX3D_PRO_DRAW_COUNT_THIRD }, () => ""),
};
...
export function PublishResultAction({
  ...
  function fillRandom() {
    setTriplets({
      special: Array.from({ length: MAX3D_PRO_DRAW_COUNT_SPECIAL }, generateRandomTriplet),
      first: Array.from({ length: MAX3D_PRO_DRAW_COUNT_FIRST }, generateRandomTriplet),
      second: Array.from({ length: MAX3D_PRO_DRAW_COUNT_SECOND }, generateRandomTriplet),
      third: Array.from({ length: MAX3D_PRO_DRAW_COUNT_THIRD }, generateRandomTriplet),
    });
    setError(null);
  }
    ...
    for (const tier of TIER_CONFIG) {
      for (let i = 0; i < tier.count; i++) {
        ...
        if (!val || !validateTriplet(val)) {
          setError(
            `${tier.label} #${i + 1}: phải là 3 chữ số (000–999).`
          );
          return;
        }
      }
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="flex items-center gap-2">
            Nhập {MAX3D_PRO_DRAW_TOTAL} bộ ba số (000–999):{" "}
            {MAX3D_PRO_DRAW_COUNT_SPECIAL} ĐB +{" "}
            {MAX3D_PRO_DRAW_COUNT_FIRST} Nhất +{" "}
            {MAX3D_PRO_DRAW_COUNT_SECOND} Nhì +{" "}
            {MAX3D_PRO_DRAW_COUNT_THIRD} Ba.
            ...
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="max-w-xs">
                Mỗi kỳ quay Max 3D Pro có {MAX3D_PRO_DRAW_TOTAL} bộ ba số kết
                quả, phân theo 4 hạng giải. Mỗi bộ là 3 chữ số từ 000 đến 999.
              </TooltipContent>
            </Tooltip>
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5 py-2">
          ...
          {TIER_CONFIG.map((tier) => (
            <div key={tier.key} className="space-y-2.5">
              <div className="flex items-center gap-2">
                <div
                  className={`h-1 w-6 rounded-full bg-linear-to-r ${tier.color}`}
                />
                <Label className="text-sm font-semibold">{tier.label}</Label>
                <Badge
                  variant="outline"
                  className={`text-[10px] border-0 ${tier.badgeClass}`}
                >
                  {tier.count} bộ
                </Badge>
              </div>
                    ...
                    return (
                      <div key={i} className="flex items-center gap-1.5">
                        ...
                        <Input
                          ...
                          value={val}
                          onChange={(e) =>
                            updateTriplet(tier.key, i, e.target.value)
                          }
                          className={`w-16 text-center font-mono text-sm font-bold tabular-nums ${
                            isValid
          ...
          {filledCount === MAX3D_PRO_DRAW_TOTAL && (
