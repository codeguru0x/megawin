...
import type { ListOutstandingTenantPlayersOutput } from "@megawin/game-max3dpro-application/use-cases/reports";
...
import { useMax3DProOutstandingTenantPlayers } from "../../financial-reports/_lib/use-report-queries";
...
/**
 * Level 3 — player breakdown cho 1 draw × 1 tenant outstanding Max 3D Pro.
 *
 * Tự gọi useMax3DProOutstandingFilters() để lấy navigateToPlayer.
...
export function OutstandingPlayerBreakdown({ drawId, tenantId }: OutstandingPlayerBreakdownProps) {
  const { navigateToPlayer } = useMax3DProOutstandingFilters();
  const { data, isLoading, error, refetch } = useMax3DProOutstandingTenantPlayers(drawId, tenantId);
