...
import { Max3dproEntryDetailDialog } from "../../financial-reports/_lib/sections/entry-list";
import { useMax3DProOutstandingPlayerEntries } from "../../financial-reports/_lib/use-report-queries";
...
/**
 * Level 4 — danh sách entries outstanding của 1 player Max 3D Pro.
 *
 * Click row → mở Max3dproEntryDetailDialog ở outstanding mode
 * (status = "scheduled" → dialog ẩn result/payout, hiển thị "Kết quả có sau kỳ quay").
 */
export function OutstandingEntryList({
  ...
  const { data, isLoading, error, refetch } = useMax3DProOutstandingPlayerEntries(
    drawId,
    tenantId,
    accountId,
  );
  ...
  const rows: TicketEntryEntity[] = data ?? [];
  const totalPairs = rows.reduce((s, r) => s + (r.lineCount ?? 0), 0);
          ...
          {rows.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="pl-5 w-[200px]">Mã vé</TableHead>
                    <TableHead className="w-[180px]">Thời gian đặt</TableHead>
                    <TableHead className="text-right w-[100px]">{LABEL_PAIRS}</TableHead>
                    <TableHead className="text-right w-[150px]">
                      {REPORT_COLUMN_LABELS.estimatedCommission}
                    </TableHead>
                    <TableHead className="pr-5 text-right w-[150px]">
