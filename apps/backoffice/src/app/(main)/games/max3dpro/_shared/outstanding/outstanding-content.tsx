...
/**
 * Orchestrator cho Max 3D Pro Outstanding drill-down.
 *
 * Render đúng level component dựa trên URL state từ useMax3DProOutstandingFilters.
 * KpiStrip chỉ hiển thị ở Level 1.
 * Breadcrumb hiển thị từ Level 2 trở lên.
 */
export function OutstandingContent() {
  const { drawId, tenantId, playerId, playerName, level } = useMax3DProOutstandingFilters();

  const { data, isLoading, error, isFetching, refetch } = useMax3DProOutstanding();
