...
export default function AdminDrawsPage() {
  ...
  const drawTime = gameConfig?.play.drawTime ?? "18:00";
  const drawsPerWeek = gameConfig?.play.drawsPerWeek ?? 3;
    ...
    <div className="@container/main flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex size-9 items-center justify-center rounded-xl bg-linear-to-br from-orange-500 to-amber-600 shadow-sm">
            <CalendarClock className="size-4.5 text-white" />
          </div>
          <div>
            <h1 className="text-lg font-semibold tracking-tight text-foreground">
              Mega 6/45 — Quản lý kỳ quay
            </h1>
            <p className="text-xs text-muted-foreground">
              {drawsPerWeek} kỳ/tuần (quay lúc {drawTime}). Mở/đóng bán,
