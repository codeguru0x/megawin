...
/**
 * Mega 6/45 — Config tab "Vận hành"
 *
 * Ngưỡng cảnh báo rủi ro + bật/tắt 4 alert P0 + nhịp worker & top-K. Mirror
 * `power655/config/game/_lib/ops-section.tsx` (cùng bộ 4 alert: large_bet,
 * exposure_threshold, combo_concentration, bao_high_stake). KHÁC Power 6/55:
 * - Default `fixedExposureWarnAmount` = 500tr (scale ¼ theo tier1 Mega 6/45), không phải 2 tỷ.
 * - Bao thấp nhất là bao5 (Mega 6/45 có bao5..bao18); giá board tính theo BAO_COMBINATIONS Mega 6/45.
 *
...
const ALERT_META: AlertMeta[] = [
  {
    type: Mega645OpsAlertType.ExposureThreshold,
  ...
  {
    type: Mega645OpsAlertType.ComboConcentration,
    label: "Dồn bộ số",
    icon: Users,
    severity: OpsAlertSeverity.Warning,
...
export function OpsSection({ config, onSave, isPending }: OpsSectionProps) {
  // Schema evolution: doc cũ chưa có `ops` → server đã lấp default (get-global-config.ts),
  // nhưng phòng khi build/cache lệch, fallback default ở đây để UI không crash.
  const { alerts, stats } = config.ops ?? DEFAULT_MEGA645_CONFIG.ops;
            ...
            <div className="grid gap-0 lg:grid-cols-2">
              ...
              <div className="space-y-5 p-6">
                ...
                <IntField
                  ...
                  suffix="VND"
                  tip="Ý nghĩa: entry có tổng tiền ≥ giá trị này bị đánh dấu 'cược lớn' và tính vào alert large_bet. · Hợp lệ: số nguyên > 0. · Mặc định: 30.000.000 (cao vì vé Bao phổ biến lớn). · Tác động: hạ ngưỡng → nhiều entry bị coi là lớn hơn; hiệu lực trong ~1 chu kỳ worker."
                />
                ...
                <IntField
                  ...
                  suffix="VND"
                  tip="Ý nghĩa: playType nhóm Bao có board với giá chuẩn (BAO_COMBINATIONS × unitPrice) ≥ giá trị này → alert bao_high_stake. · Hợp lệ: số nguyên > 0. · Mặc định: 30.000.000. · Tác động: giảm → nhạy hơn với vé Bao mức cao."
                />
