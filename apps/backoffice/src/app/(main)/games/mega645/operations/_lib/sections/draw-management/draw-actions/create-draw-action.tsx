...
/**
 * Mega 6/45 – Create Draw Action Dialog
 *
 * Tạo nhiều kỳ quay Mega 6/45 liên tiếp (1 kỳ/ngày, drawNo luôn = 1).
...
/** Số kỳ mặc định mỗi lần mở dialog — Mega 6/45 quay 3 kỳ/tuần, mặc định 1 tuần ≈ 3 kỳ. */
      ...
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <CalendarPlus className="text-game-mega645 size-4.5" />
          ...
          <div className="flex flex-wrap items-end gap-4">
            <div className="space-y-1.5">
              ...
              <Input
                type="number"
                min={1}
                max={MEGA645_CREATE_DRAW_BATCH_MAX}
                value={count}
                onChange={(e) => {
                  const v = parseInt(e.target.value, 10);
                  if (!isNaN(v) && v >= 1 && v <= MEGA645_CREATE_DRAW_BATCH_MAX) {
                    setCount(v);
                  }
                }}
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex h-9 flex-wrap items-center gap-1.5">
                ...
                {openCount > 0 && (
                  <Badge className="bg-game-mega645 hover:bg-game-mega645 text-xs text-white">{openCount} mở bán</Badge>
                )}
                {scheduledCount > 0 && (
          ...
          <div className="overflow-hidden rounded-xl border">
            ...
            >
              ...
              <div className="flex items-center justify-end">
                ...
                >
                  {allOpen ? <Unlock className="text-game-mega645 size-3" /> : <Lock className="size-3" />}
                  <span className={cn(allOpen && "text-game-mega645")}>{allOpen ? "Đóng" : "Mở"}</span>
                </button>
              </div>
            </div>
              ...
              {rows.map((row, i) => (
                <div
                  ...
                  className={cn(
                    ...
                    row.isOpen ? "bg-game-mega645/50" : "hover:bg-muted/20",
                  )}
                  style={{ gridTemplateColumns: "1.5rem 3rem 1fr 6.5rem 9rem" }}
                >
                  {/* Số thứ tự */}
                  <span
                    className={cn(
                      ...
                      row.isOpen ? "text-game-mega645" : "text-foreground",
                    )}
                  >
                    {i + 1}
                  </span>
                  ...
                  >
                    {row.isOpen ? (
                      <Unlock className="text-game-mega645 size-3 shrink-0" />
                    ) : (
                    ...
                    <span
                      className={cn(
                        ...
                        row.isOpen ? "text-game-mega645" : "text-muted-foreground",
                      )}
                    >
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={!canSubmit}
            className={cn(openCount > 0 && "bg-game-mega645 hover:bg-game-mega645 text-white")}
          >
