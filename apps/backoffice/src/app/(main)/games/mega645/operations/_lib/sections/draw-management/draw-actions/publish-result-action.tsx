...
function validateMega645Numbers(numbers: string[]): ValidationResult {
  ...
  for (let i = 0; i < numbers.length; i++) {
    ...
    if (Number.isNaN(n) || n < MEGA645_NUMBER_MIN || n > MEGA645_NUMBER_MAX) {
...
export function PublishResultAction({
  ...
  function applyIncomingNumbers() {
    ...
    setNumbers(data.numbers.map((n) => n.padStart(2, "0")).slice(0, MEGA645_NUMBER_COUNT));
    setValidation(VALID);
    setHasAppliedAutoResult(true);
  }

  // Tự động điền — CHỈ khi TẤT CẢ ô đang rỗng (chưa nhập tay/chưa điền trước) — KHÔNG tự điền
  // phần thiếu khi form đã có bất kỳ số nào, dù các ô rỗng còn lại có thể điền được. Quy tắc
  // bất biến, không đổi thành `.some()` hay biến thể "điền phần thiếu" (plan §5.0/§11.0 quy
  // tắc A) — tránh tạo ra 1 kết quả LAI (nửa tay, nửa nguồn) không ai chủ động xác nhận toàn
  // bộ. Khi form không rỗng hoàn toàn, luồng đi vào diff (bên dưới) → staff tự thấy lệch ở
  // đâu → tự bấm "Áp dụng" nếu muốn ghi đè toàn bộ.
  useEffect(() => {
    const data = vietlottResultQuery.data;
  ...
  // So sánh số đang nhập với số ResultFeed trả về — dùng để highlight lệch trên lưới (§5.2/§7b.5)
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-3">
              ...
              <VietlottResultPanel
                ...
                diff={diff}
                totalCount={MEGA645_NUMBER_COUNT}
