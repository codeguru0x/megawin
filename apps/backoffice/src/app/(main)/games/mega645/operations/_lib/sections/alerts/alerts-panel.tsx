...
/**
 * Mega 6/45 Operations — Alerts Panel (tab Giám sát)
 *
 ...
 * tự bung (mirror Keno/Power 6/55). Mặc định chỉ hiện alert `new`, phần đã ack gộp xuống
 * 1 dòng "N đã xử lý" mỗi nhóm.
 *
 ...
 *
 * 4 loại alert (KHÁC Keno): `large_bet`, `exposure_threshold`, `combo_concentration`,
 * `bao_high_stake` (đặc thù Mega 6/45). `describeAlert` format theo payload từng loại
 * do `evaluate-alerts.ts` sinh (KHÔNG hiện JSON thô).
 */

import { useState } from "react";

import { GameProduct } from "@megawin/game-core/entities/game-core.enums";
import type { Mega645OpsAlertEntity, Mega645TopPotential } from "@megawin/game-mega645/entities";
import { Mega645OpsAlertType, OpsAlertSeverity, OpsAlertStatus, PlayType } from "@megawin/game-mega645/entities";
...
import { MEGA645_OPS_ALERT_TYPE_LABELS } from "../../ops-constants";
...
/** 1 playType Bao chạm ngưỡng trong payload `bao_high_stake`. */
interface BaoTriggered {
  playType: string;
  boardPrice: number;
  boards: number;
}

/** Đọc `payload.triggered` (mảng BaoTriggered) an toàn — chỉ bao_high_stake có. */
function readBaoTriggered(payload: Record<string, unknown>): BaoTriggered[] {
  const t = payload.triggered;
  if (!Array.isArray(t)) return [];
  return t as BaoTriggered[];
}
 ...
 * nested như `large_bet.top`, `bao_high_stake.triggered`). Loại chưa có formatter riêng →
 * fallback liệt kê field primitive (bỏ object/array).
 */
function describeAlert(type: string, payload: Record<string, unknown>): AlertDescription {
  switch (type) {
    case Mega645OpsAlertType.LargeBet: {
    ...
    case Mega645OpsAlertType.ExposureThreshold: {
    ...
    case Mega645OpsAlertType.ComboConcentration: {
      ...
      const label = MEGA645_PLAY_TYPE_LABELS[pt as keyof typeof MEGA645_PLAY_TYPE_LABELS] ?? pt;
      return {
    ...
    case Mega645OpsAlertType.BaoHighStake: {
      const triggered = readBaoTriggered(payload);
      const threshold = toNum(payload.threshold);
      const names = triggered
        .map((t) => MEGA645_PLAY_TYPE_LABELS[t.playType as keyof typeof MEGA645_PLAY_TYPE_LABELS] ?? t.playType)
        .join(", ");
      return {
        summary: `Có vé Bao mức cao (${names}) — giá 1 board ≥ ${formatNumber(threshold)} VND.`,
        chips: triggered.map((t) => ({
          label: MEGA645_PLAY_TYPE_LABELS[t.playType as keyof typeof MEGA645_PLAY_TYPE_LABELS] ?? t.playType,
...
/** Đọc `payload.top` (mảng Mega645TopPotential) an toàn — chỉ large_bet có. */
function readTopEntries(payload: Record<string, unknown>): Mega645TopPotential[] {
  const top = payload.top;
  if (!Array.isArray(top)) return [];
  return top as Mega645TopPotential[];
}
...
function AlertTopEntries({ drawId, payload }: { drawId: string; payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-1 rounded-md border border-border/50 divide-y divide-border/40 overflow-hidden">
      {entries.map((e) => {
        const href = buildOutstandingHref(GameProduct.Mega645, drawId, e.accountId, e.username);
        return (
...
function AlertItemRow({ alert }: { alert: Mega645OpsAlertEntity }) {
 ...
 * (mirror Keno — tránh panel dài khi ngưỡng quá nhạy sinh nhiều alert).
 */
function AlertGroupContent({ items }: { items: Mega645OpsAlertEntity[] }) {
                ...
                <AccordionTrigger className="py-3 hover:no-underline">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span className="text-sm font-semibold truncate">
                      {MEGA645_OPS_ALERT_TYPE_LABELS[g.type] ?? g.type}
                    </span>
 ...
 * tab Giám sát. Snapshot Mega 6/45 KHÔNG mang severity → không phân biệt "critical" như
