...
function describeAlert(type: string, payload: Record<string, unknown>): AlertDescription {
  switch (type) {
    ...
    case Mega645OpsAlertType.ComboConcentration: {
      ...
      const label = MEGA645_PLAY_TYPE_LABELS[pt as keyof typeof MEGA645_PLAY_TYPE_LABELS];
      return {
    ...
    case Mega645OpsAlertType.BaoHighStake: {
      ...
      const names = triggered
        .map((t) => MEGA645_PLAY_TYPE_LABELS[t.playType as keyof typeof MEGA645_PLAY_TYPE_LABELS])
        .join(", ");
      return {
        summary: `Có vé Bao mức cao (${names}) — giá 1 board ≥ ${formatNumber(threshold)} VND.`,
        chips: triggered.map((t) => ({
          label: MEGA645_PLAY_TYPE_LABELS[t.playType as keyof typeof MEGA645_PLAY_TYPE_LABELS],
              ...
              <AccordionItem key={g.type} value={g.type}>
                <AccordionTrigger className="py-3 hover:no-underline">
                  <div className="flex min-w-0 items-center gap-2.5">
                    ...
                    <span className="truncate text-sm font-semibold">{MEGA645_OPS_ALERT_TYPE_LABELS[g.type]}</span>
