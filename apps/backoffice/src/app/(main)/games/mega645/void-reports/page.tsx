...
import { useMega645VoidReports } from "../financial-reports/_lib/use-report-queries";
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Snapshot trước khi Void — Mega 6/45</DialogTitle>
          <DialogDescription>
...
function VoidTable({ from, to }: { from: string; to: string }) {
  ...
  const { data, isLoading, error } = useMega645VoidReports(from, to);
...
function Mega645VoidReportsContent() {
  const today = todayVN();
