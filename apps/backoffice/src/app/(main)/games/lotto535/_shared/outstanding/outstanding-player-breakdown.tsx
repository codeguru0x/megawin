...
import type { ListOutstandingTenantPlayersOutput } from "@megawin/game-lotto535-application/use-cases/reports";

type OutstandingPlayerBreakdownRow = ListOutstandingTenantPlayersOutput["data"][number];
import { useLotto535OutstandingFilters } from "./use-outstanding-filters";
import { useLotto535OutstandingTenantPlayers } from "../../financial-reports/_lib/use-report-queries";
...
/**
 * Level 3 — player breakdown cho 1 draw × 1 tenant outstanding Lotto 5/35.
 *
 * Tự gọi useLotto535OutstandingFilters() để lấy navigateToPlayer.
...
export function OutstandingPlayerBreakdown({ drawId, tenantId }: OutstandingPlayerBreakdownProps) {
  const { navigateToPlayer } = useLotto535OutstandingFilters();
  const { data, isLoading, error, refetch } = useLotto535OutstandingTenantPlayers(drawId, tenantId);
  ...
  const rows: OutstandingPlayerBreakdownRow[] = data ?? [];
