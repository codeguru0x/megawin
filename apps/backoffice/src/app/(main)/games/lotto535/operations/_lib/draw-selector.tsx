...
export function DrawSelector({
  ...
  // Khi kỳ chọn là kỳ cũ ngoài selector list, Select value sẽ không match →
  // dùng value="" để không tick item nào, hiển thị custom trigger label thay thế
  const isInList = draws.some((d) => d.drawId === selectedDrawId);

  return (
    <Select value={isInList ? selectedDrawId : ""} onValueChange={onSelect}>
      <SelectTrigger className="h-9 w-[260px] gap-2 text-sm font-medium">
        <div className="flex items-center gap-2 min-w-0">
      ...
      <SelectContent className="w-[320px]" align="end">
        {active.length > 0 && (
          <SelectGroup>
