...
// ─── Mini SVG Pie Chart ───────────────────────────────────────────────────────
// Renders a donut-style pie: filled arc for `pct`%, empty arc for the rest.

function MiniPie({ pct, fill, size = 44 }: { pct: number; fill: string; size?: number }) {
  const r = (size - 6) / 2; // radius (3px stroke each side)
  const cx = size / 2;
  const cy = size / 2;
  const circumference = 2 * Math.PI * r;
  // Clamp to avoid full-circle oddity at 100%
...
export function PlayTypeCard({ distribution }: { distribution: PlayTypeRow[] }) {
  ...
  // Chỉ hiện kiểu chơi có dữ liệu, sort theo lines desc
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        {tenants.map((t) => (
          // Grid 5 cột cố định: [tên] [ent] [ln] [revenue] [bar+%]
          // Cột cuối chiếm ~50% để bar thẳng hàng giữa các row
          <div
            key={t.tenantId}
