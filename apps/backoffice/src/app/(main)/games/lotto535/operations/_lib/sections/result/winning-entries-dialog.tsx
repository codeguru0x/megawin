...
function TierChip({ tier }: { tier: WinningEntryTierDetail }) {
  ...
  return (
    ...
    >
      {isJackpot && <Star className="fill-warning text-warning size-3" />}
      {tier.tierLabel}
  ...
  return (
    <TableRow
      ...
      className={cn(
        ...
        hasJackpot && "border-l-3 border-l-warning",
      )}
    >
      {/* Row number */}
      <TableCell className="py-3 pl-6 text-center">
        <span
          className={cn(
            ...
            hasJackpot ? "bg-warning/10 text-warning" : "bg-muted text-muted-foreground",
          )}
        >
          {rowNo}
        </span>
