...
/**
 * Lotto 5/35 — Config tab "Vận hành"
 *
 * Ngưỡng cảnh báo rủi ro + bật/tắt 5 alert P0 + nhịp worker & top-K. Mirror
 * `power655/config/game/_lib/ops-section.tsx` nhưng KHÁC:
 * - Đổi `baoHighStakeAmount` → `coverHighStakeAmount` (đánh giá nhóm `mainCover6..15`).
 * - THÊM 2 field ĐẶC THÙ Lotto 5/35: `specialSkewRatio` (% thập phân 0–1, KHÔNG phải VND)
 *   và `specialSkewMinAmount` (VND) — cấu hình rule `special_skew` (dồn tiền 1 số ĐB).
 * - 5 alert P0 có toggle (KHÁC Power 6/55 4 alert): thêm `SpecialSkew`.
 *
 * Range PHẢI khớp Zod server (`api/lotto535/config/_lib/schema.ts` §ops).
 */
...
/**
 * 5 alert P0 (bỏ `RevenueAnomaly`/`SettleStuck` để dành). Thứ tự = mức nghiêm trọng
 * giảm dần để người vận hành quét từ trên xuống. Điều kiện bật lấy từ JSDoc
 * `ops-alert.ts` (game-lotto535/entities).
 */
const ALERT_META: AlertMeta[] = [
  {
    type: Lotto535OpsAlertType.ExposureThreshold,
  ...
  {
    type: Lotto535OpsAlertType.SpecialSkew,
    label: "Dồn số đặc biệt",
    icon: Sparkles,
    severity: OpsAlertSeverity.Warning,
    summary: "1 số ĐB chiếm tỷ trọng ≥ ngưỡng 'Tỷ trọng dồn số ĐB' trong tổng tiền cược ĐB kỳ.",
    tip: "Ý nghĩa: số ĐB có amount / Σamount(kind=special) ≥ ngưỡng VÀ Σamount(kind=special) ≥ 'Tiền cược ĐB tối thiểu' → bắn alert special_skew. Không gian ĐB chỉ 12 số nên rủi ro tập trung rõ rệt hơn số chính. · Ngưỡng liên quan: 'Tỷ trọng dồn số ĐB' + 'Tiền cược ĐB tối thiểu' ở trên. · Nâng Critical khi tỷ trọng ≥ 2× ngưỡng. · Tác động khi TẮT: không phát hiện dồn tiền bất thường vào 1 số ĐB.",
  },
  {
    type: Lotto535OpsAlertType.CoverHighStake,
    label: "Vé Bao mức cao",
    icon: Layers,
    severity: OpsAlertSeverity.Warning,
    summary: "Board Bao 6–15 số chính có giá ≥ ngưỡng 'Giá board Bao cao' (VND).",
    tip: "Ý nghĩa: playType nhóm mainCover6..mainCover15 có board với giá chuẩn (C(N,5) × unitPrice) ≥ ngưỡng → bắn alert cover_high_stake. · Ngưỡng liên quan: 'Giá board Bao cao (VND)' ở trên. · Nâng Critical khi playType = mainCover15 (mức cao nhất). · Tác động khi TẮT: không nổi bật vé Bao cao rủi ro cao.",
  },
  {
    type: Lotto535OpsAlertType.LargeBet,
    label: "Cược lớn",
    icon: Coins,
    severity: OpsAlertSeverity.Warning,
  ...
  {
    type: Lotto535OpsAlertType.ComboConcentration,
    label: "Dồn bộ số",
    icon: Users,
    severity: OpsAlertSeverity.Warning,
...
const opsFormSchema = z.object({
  ...
  coverHighStakeAmount: z.coerce.number().int().positive("Phải > 0"),
  specialSkewRatio: z.coerce.number().min(0, "0–1").max(1, "0–1"),
  specialSkewMinAmount: z.coerce.number().int().positive("Phải > 0"),
...
/**
 * Ô nhập tỷ trọng dạng % thân thiện — lưu nội bộ dạng thập phân 0–1 (khớp Zod server)
 * nhưng hiển thị/nhập cho user dạng % (0.35 ↔ "35"). Chỉ dùng cho `specialSkewRatio`.
 */
function RatioPercentField({
  form,
  name,
  label,
  tip,
          ...
          <FormControl>
            <div className="relative">
              <Input
                ...
                value={Math.round((field.value as number) * 100)}
                onChange={(e) => {
    ...
    onSave({
      ops: {
        alerts: {
          ...
          coverHighStakeAmount: values.coverHighStakeAmount,
          specialSkewRatio: values.specialSkewRatio,
          specialSkewMinAmount: values.specialSkewMinAmount,
              ...
              <div className="space-y-5 p-6">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Worker so ngưỡng này mỗi chu kỳ để sinh alert. Đổi có hiệu lực trong ~1 chu kỳ
                    worker (không cần deploy).
                  </p>
                </div>
                ...
                <IntField
                  ...
                  suffix="VND"
                  tip="Ý nghĩa: entry có tổng tiền ≥ giá trị này bị đánh dấu 'cược lớn' và tính vào alert large_bet. · Hợp lệ: số nguyên > 0. · Mặc định: 30.000.000. · Tác động: hạ ngưỡng → nhiều entry bị coi là lớn hơn; hiệu lực trong ~1 chu kỳ worker."
                />
                ...
                <IntField
                  ...
                  suffix="VND"
                  tip="Ý nghĩa: khi tổng worst-case payout giải cố định (fixedWorstCase) ≥ giá trị này → alert exposure_threshold. VND TUYỆT ĐỐI. · Hợp lệ: số nguyên > 0. · Mặc định: 500.000.000. · Tác động: giảm → cảnh báo sớm hơn khi rủi ro trả thưởng tăng."
                />

                <IntField
                  form={form}
                  name="coverHighStakeAmount"
                  label="Giá board Bao cao"
                  suffix="VND"
                  tip="Ý nghĩa: playType nhóm mainCover6..mainCover15 có board với giá chuẩn (C(N,5) × unitPrice) ≥ giá trị này → alert cover_high_stake. · Hợp lệ: số nguyên > 0. · Mặc định: 10.000.000 (board mainCover12 = 7,92tr CHƯA chạm; mainCover13 = 12,87tr ĐÃ chạm). · Tác động: giảm → nhạy hơn với vé Bao mức cao."
                />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <RatioPercentField
                    form={form}
                    name="specialSkewRatio"
                    label="Tỷ trọng dồn số ĐB"
                    tip="Ý nghĩa: 1 số ĐB có amount / Σamount(kind=special) ≥ giá trị này → alert special_skew (đặc thù Lotto 5/35, không gian ĐB chỉ 12 số). · Hợp lệ: 0–100%. · Mặc định: 35% (baseline lý thuyết đều ≈ 8,3%). · Tác động: giảm → nhạy hơn với dồn tiền vào 1 số ĐB."
                  />
                  <IntField
                    form={form}
                    name="specialSkewMinAmount"
                    label="Tiền cược ĐB tối thiểu"
                    suffix="VND"
                    tip="Ý nghĩa: chỉ đánh giá rule special_skew khi Σamount(kind=special) ≥ giá trị này — chống nhiễu kỳ vắng (mới mở bán, ít tiền, 1 số ĐB dễ 'chiếm 100%' giả tạo). · Hợp lệ: số nguyên > 0. · Mặc định: 50.000.000."
                  />
                </div>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <p className="inline-flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-muted-foreground">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent side="top" className="max-w-80 text-xs">
                          Chọn loại rủi ro worker sẽ giám sát và sinh alert mỗi chu kỳ. Tắt một loại
                          nghĩa là ngưng theo dõi rủi ro đó — dùng khi muốn giảm nhiễu, KHÔNG nên
                          tắt loại 'Nghiêm trọng' trừ khi có lý do rõ ràng.
                        </TooltipContent>
                      </Tooltip>
                    </p>
