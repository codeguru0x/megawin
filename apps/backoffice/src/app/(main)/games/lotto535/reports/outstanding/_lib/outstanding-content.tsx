...
/**
 * Wrapper cho Lotto 5/35 Outstanding — kết nối game-specific hooks vào shared OutstandingContent.
 *
 * toTenantUsername áp dụng tại mapPlayerRow: bỏ suffix @local / @tenantId.
 */
export function Lotto535OutstandingContent() {
  const {
    drawId,
    tenantId,
    playerId,
    ...
    navigateToTenant,
    navigateToPlayer,
  } = useLotto535OutstandingFilters();

  const [selectedEntry, setSelectedEntry] = useState<TicketEntryEntity | null>(null);

  // Level 1 — draws outstanding
  const drawsQuery = useLotto535Outstanding();

  // Level 2 — tenant breakdown cho draw đang drill
  const tenantQuery = useLotto535OutstandingDrawTenants(drawId);

  // Level 3 — player breakdown cho draw × tenant đang drill
  const playerQuery = useLotto535OutstandingTenantPlayers(drawId ?? "", tenantId);

  // Level 4 — entries của player đang drill
  const entryQuery = useLotto535OutstandingPlayerEntries(drawId ?? "", tenantId ?? "", playerId);
