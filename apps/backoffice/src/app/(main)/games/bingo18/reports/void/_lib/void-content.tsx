...
/** Map TicketEntryEntity → shared VoidEntryRow. Bingo 18: no lineCount. */
...
/**
 * Wrapper cho Bingo 18 Void Reports — kết nối game-specific hooks vào shared VoidContent.
 *
 * toTenantUsername áp dụng tại mapPlayerRow: bỏ suffix @local / @tenantId.
 */
export function Bingo18VoidReportsContent() {
  const {
    from,
    to,
    onDateChange,
    ...
    navigateToTenant,
    navigateToPlayer,
  } = useBingo18VoidFilters();

  const [selectedEntry, setSelectedEntry] = useState<TicketEntryEntity | null>(null);

  const drawsQuery = useBingo18VoidReports(from, to);
  const tenantQuery = useBingo18VoidDrawTenants(drawId);
  const playerQuery = useBingo18VoidTenantPlayers(drawId ?? "", tenantId);
  const entryQuery = useBingo18VoidPlayerEntries(drawId ?? "", tenantId ?? "", playerId);
