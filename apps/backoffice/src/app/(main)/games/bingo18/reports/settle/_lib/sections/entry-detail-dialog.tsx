...
function Die({
  ...
  const sizeClass =
    size === "lg"
      ? "size-11 text-xl font-extrabold"
      : size === "sm"
        ? "size-7 text-[11px]"
        : "size-8 text-sm";
  // lg (kết quả kỳ quay) dùng hình tròn; sm/md dùng bo vuông nhẹ
  const roundClass = size === "lg" ? "rounded-full" : "rounded-lg";
  const colorClass =
    variant === "matched"
  ...
  return (
    <span
      className={`inline-flex items-center justify-center font-bold tabular-nums ${roundClass} ${sizeClass} ${colorClass}`}
    >
      {n}
    </span>
  );
                ...
                <div className="rounded-lg border p-4">
                  ...
                  <div className="space-y-2">
                    {allBoardSnapshots.map((snapshot, i) => {
                      ...
                      let selectionContent: React.ReactNode;
                      if (
                        snapshot.playType === "singleNum" ||
                        snapshot.playType === "doubleMatch"
                      ) {
                        const num = snapshot.number;
                        selectionContent =
                          num != null ? (
                            <Die n={num} size="sm" variant="default" />
                          ) : (
                            <span className="text-muted-foreground">—</span>
                          );
                      } else if (snapshot.playType === "tripleMatch") {
