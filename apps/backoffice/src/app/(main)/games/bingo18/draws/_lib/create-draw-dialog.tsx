...
import { useBingo18CreateDraw, useBingo18PreviewDraws } from "./use-draws";
...
export function Bingo18CreateDrawDialog() {
  const [open, setOpen] = useState(false);
  const [count, setCount] = useState(10);
  const today = getTodayISO();

  const preview = useBingo18PreviewDraws(open ? today : "", open ? count : 0);
  const createDraw = useBingo18CreateDraw();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Tạo kỳ quay Bingo 18 – {today}</DialogTitle>
          <DialogDescription>
            Thời gian quay và đóng bán tính tự động theo cấu hình game (mỗi 6 phút).
          </DialogDescription>
        </DialogHeader>
