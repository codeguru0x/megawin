...
export function useTenantConfigs() {
  return useQuery({
    ...
    queryFn: () => apiClient.get<ListTenantConfigsOutput>("/bingo18/tenant-config").then((r) => r.configs),
  });
}
...
export function useTenantConfig(tenantId: string | null) {
  return useQuery({
    ...
    queryFn: () => apiClient.get<GetTenantConfigOutput>(`/bingo18/tenant-config/${tenantId}`).then((r) => r.config),
    enabled: !!tenantId,
  });
}
