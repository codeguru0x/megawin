...
import { ExternalLink, Percent, Settings2, ShieldAlert, Trophy } from "lucide-react";
...
import { useBingo18GameConfig, useUpdateBingo18GameConfig } from "./_lib/use-game-config";
...
export default function Bingo18ConfigPage() {
  const { data: config, isLoading, isError, error } = useBingo18GameConfig();
  const mutation = useUpdateBingo18GameConfig();
  ...
  return (
    <div className="@container/main flex flex-col gap-2">
      <div className="flex items-center gap-2.5">
        <div className="from-profit to-game-mega645 flex size-8 items-center justify-center rounded-lg bg-linear-to-br shadow-sm">
          <Settings2 className="size-4 text-white" />
        </div>
        <div>
          <h1 className="text-foreground text-base font-semibold tracking-tight">Bingo 18 — Cấu hình</h1>
          {config && (
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
          <TabsList variant="line" className="w-full justify-start gap-0 border-b px-0">
            ...
            <TabsTrigger value="ops" className="gap-1.5">
              <ShieldAlert className="text-loss size-4" />
              Vận hành
            </TabsTrigger>
