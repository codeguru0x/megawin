...
function DrawList() {
  ...
  const totals = {
    ...
    netProfit: data.data.reduce((s, r) => s + r.netProfit, 0),
  };
...
function DrawTenantBreakdown({ drawId }: { drawId: string }) {
  const { navigateToPlayer } = useBingo18ReportFilters();
...
function PlayerBreakdown({ drawId, tenantId }: { drawId: string; tenantId: string }) {
  ...
  if (!players?.length) return <EmptyCard icon="ticket" message="Không có người chơi nào." />;
