...
import { useBingo18OutstandingDrawTenants } from "../../financial-reports/_lib/use-report-queries";
...
/**
 * Level 2 — tenant breakdown cho 1 draw outstanding Bingo 18.
 *
 * Tự gọi useBingo18OutstandingFilters() để lấy navigateToTenant.
 * Click bất kỳ cell nào trong row → drill vào Player Breakdown (Level 3).
 * Bingo 18 KHÔNG có lineCount — 5 cột thay vì 6 cột của Power655.
 */
export function OutstandingTenantBreakdown({ drawId }: OutstandingTenantBreakdownProps) {
  const { navigateToTenant } = useBingo18OutstandingFilters();
  const { data, isLoading, error, refetch } = useBingo18OutstandingDrawTenants(drawId);
