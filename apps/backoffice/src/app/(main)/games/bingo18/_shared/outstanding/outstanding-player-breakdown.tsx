...
import { useBingo18OutstandingTenantPlayers } from "../../financial-reports/_lib/use-report-queries";
...
/**
 * Level 3 — player breakdown cho 1 draw × 1 tenant outstanding Bingo 18.
 *
 * Tự gọi useBingo18OutstandingFilters() để lấy navigateToPlayer.
...
export function OutstandingPlayerBreakdown({ drawId, tenantId }: OutstandingPlayerBreakdownProps) {
  const { navigateToPlayer } = useBingo18OutstandingFilters();
  const { data, isLoading, error, refetch } = useBingo18OutstandingTenantPlayers(drawId, tenantId);
