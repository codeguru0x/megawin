...
/**
 * KPI strip cho Bingo 18 Outstanding — 4 cards (KHÔNG có lineCount).
 * Bingo 18 không expand boards thành lines nên không cần card lineCount.
 */
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      <KpiCard
        ...
        // ~6 phút/kỳ — tần suất quay Bingo 18
        sub="kỳ quay chưa settle (~6 phút/kỳ)"
      />
      <KpiCard
        icon={Ticket}
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          onClick={onRefresh}
          className="flex items-center gap-1.5 rounded-md px-1.5 py-1 transition-colors hover:bg-muted/60"
...
/**
 * Orchestrator cho Bingo 18 Outstanding drill-down.
 *
 * Render đúng level component dựa trên URL state từ useBingo18OutstandingFilters.
 * KpiStrip chỉ hiển thị ở Level 1 — 4 cards (KHÔNG có lineCount).
 * Breadcrumb hiển thị từ Level 2 trở lên.
 */
export function OutstandingContent() {
  const { drawId, tenantId, playerId, playerName, level } = useBingo18OutstandingFilters();

  const { data, isLoading, error, isFetching, refetch } = useBingo18Outstanding();
