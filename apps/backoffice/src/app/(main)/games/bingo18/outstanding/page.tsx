...
function KpiStrip({ data }: KpiStripProps) {
  ...
  // Bingo18 không có lineCount — 4 KPI cards
  return (
...
function OutstandingTable() {
  const { data, isLoading, error, dataUpdatedAt, isFetching, refetch } = useBingo18Outstanding();
  ...
  // Tổng cho footer row — Bingo18 không có lineCount
                ...
                <TableBody>
                  {filteredRows.map((row) => (
                    <TableRow key={row.drawId}>
                      ...
                      <TableCell>
                        <Link
                          href={`/games/bingo18/operations?draw=${row.drawId}`}
