...
import {
  BINGO18_DIRECTION_LABELS,
  BINGO18_OPS_ALERT_TYPE_LABELS,
  describeHighBucket,
} from "../../ops-constants";
...
function describeAlert(type: string, payload: Record<string, unknown>): AlertDescription {
  switch (type) {
    ...
    case Bingo18OpsAlertType.ExposureThreshold: {
      ...
      const numbers = Array.isArray(payload.worstNumbers)
        ? (payload.worstNumbers as number[]).join("-")
        : "";
      return {
    ...
    case Bingo18OpsAlertType.SidebetSkew: {
      ...
      const dir =
        BINGO18_DIRECTION_LABELS[dirKey as keyof typeof BINGO18_DIRECTION_LABELS] ?? dirKey;
      return {
    ...
    case Bingo18OpsAlertType.BucketConcentration: {
      ...
      const bucket = describeHighBucket(
        String(payload.playType ?? ""),
        String(payload.bucketKey ?? ""),
      );
      return {
