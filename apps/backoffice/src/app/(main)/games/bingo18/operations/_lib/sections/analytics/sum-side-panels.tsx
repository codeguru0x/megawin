      ...
      <TooltipTrigger asChild>
        <div className="flex h-full flex-1 cursor-help flex-col items-center gap-1">
          <div className="flex w-full flex-1 flex-col justify-end">
            <span
              className={cn(
                "mb-0.5 text-center text-xs font-semibold tabular-nums",
            ...
            <div
              className={cn(
                ...
                overThreshold ? "bg-warning" : isHighMultiplier ? "bg-loss/80" : "bg-info/80",
              )}
          ...
          <span
            className={cn(
              "flex h-6 w-full items-center justify-center rounded-md text-xs font-bold tabular-nums",
              isHighMultiplier ? "bg-loss text-loss ring-loss/60 ring-1" : "bg-muted text-muted-foreground",
            )}
          >
            {sum}
          </span>
      ...
      <CardHeader className="px-5 pt-4 pb-1">
        <div className="flex items-center gap-2">
          <div className="bg-info flex size-7 shrink-0 items-center justify-center rounded-lg">
            <Sigma className="text-info size-3.5" />
          </div>
          <div>
...
export function SideBetCard({
  ...
  return (
    <Card className="gap-0 py-0 shadow-sm">
      <CardHeader className="px-5 pt-4 pb-1">
        <div className="flex items-center gap-2">
          <div className="bg-game-mega645 flex size-7 shrink-0 items-center justify-center rounded-lg">
            <Scale className="text-game-mega645 size-3.5" />
          </div>
          ...
          {skewed && (
            <span className="bg-warning text-warning shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold">
        ...
        <div className="bg-muted flex h-2.5 w-full overflow-hidden rounded-full">
          {dirs.map((d) => {
            ...
            return (
              <div
                key={d.key}
                className={cn("h-full transition-all", isTopSkewed ? "bg-warning" : SEGMENT_COLORS[d.key])}
                style={{ width: `${pct}%` }}
              />
            );
          })}
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isTopSkewed ? "border-warning/70 bg-warning/60" : "border-border/50 bg-muted/10",
                )}
              >
