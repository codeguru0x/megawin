...
/**
 * Path bulk — `Void` vẫn có trong `BulkActionKind` (để `Exclude` ở dialog giống Keno) nhưng
 * Bingo18 Hub KHÔNG có route bulk-void (p1-04 §6.4). UI không bao giờ gọi `Void`.
 */
const BULK_PATH: Record<BulkActionKind, string> = {
  [BulkActionKind.Settle]: "/bingo18/draws/bulk-settle",
  [BulkActionKind.Void]: "/bingo18/draws/bulk-void",
  [BulkActionKind.CloseSales]: "/bingo18/draws/bulk-close-sales",
  [BulkActionKind.OpenSales]: "/bingo18/draws/bulk-open-sales",
};
...
/** Input gửi lên — chỉ `drawIds` (server tự lấy `actor` từ session). Bingo18 không có bulk-void. */
export type BulkActionInput = Omit<BulkDrawIdsInput, "actor">;
...
function applyBulkResult(
  ...
  void ctx.qc.invalidateQueries({ queryKey: bingo18Keys.opsHub() });
}
 ...
 * job nhiều lô. `SETTLE_SFN_ARN` KHÔNG gửi từ client (secret hạ tầng server-side, route tự
 * đọc từ `env` — xem `bulk-settle/route.ts`). Type input ở client CHỈ có `drawIds`.
 */
 ...
 * gì về Bingo18), áp `applyBulkResult` cho MỖI lô (selection/rowErrors cập nhật ngay theo từng
