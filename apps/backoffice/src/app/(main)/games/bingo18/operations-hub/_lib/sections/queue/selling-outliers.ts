 ...
 * 4. `exposureRaw ≥ p95` trong các kỳ CÓ exposure > 0 — percentile trong ngày. Bingo18 **không
 *    có** `payoutCaps`; `exposureWarnPct` trên DTO map từ `exposureWarnRevenuePct` (% doanh thu)
 *    — so percentile với nhau là proxy nội-ngày, không phải so "raw vs sau-cap" như Keno.
 */
