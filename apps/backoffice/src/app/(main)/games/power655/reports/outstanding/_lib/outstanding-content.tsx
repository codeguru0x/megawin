...
import {
  usePower655Outstanding,
  usePower655OutstandingDrawTenants,
  usePower655OutstandingPlayerEntries,
  usePower655OutstandingTenantPlayers,
} from "../../settle/_lib/use-report-queries";
import { usePower655OutstandingFilters } from "./use-outstanding-filters";
...
/**
 * Wrapper cho Power 6/55 Outstanding — kết nối game-specific hooks vào shared OutstandingContent.
 *
 * toTenantUsername áp dụng tại mapPlayerRow: bỏ suffix @local / @tenantId.
 */
export function Power655OutstandingContent() {
  const {
    drawId,
    tenantId,
    playerId,
    ...
    navigateToTenant,
    navigateToPlayer,
  } = usePower655OutstandingFilters();

  const [selectedEntry, setSelectedEntry] = useState<TicketEntryEntity | null>(null);

  // Level 1 — draws outstanding
  const drawsQuery = usePower655Outstanding();

  // Level 2 — tenant breakdown cho draw đang drill
  const tenantQuery = usePower655OutstandingDrawTenants(drawId);

  // Level 3 — player breakdown cho draw × tenant đang drill
  const playerQuery = usePower655OutstandingTenantPlayers(drawId ?? "", tenantId);

  // Level 4 — entries của player đang drill
  const entryQuery = usePower655OutstandingPlayerEntries(drawId ?? "", tenantId ?? "", playerId);
