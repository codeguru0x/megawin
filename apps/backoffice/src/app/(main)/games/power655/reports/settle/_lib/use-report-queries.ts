...
export function usePower655VoidDrawTenants(drawId: string | null) {
  return useQuery({
    queryKey: power655Keys.voidDrawTenants(drawId ?? ""),
    queryFn: () =>
      apiClient
        .get<{
          data: Array<{
        ...
        }>(`/power655/reports/void/draws/${drawId}/tenants`)
        .then((r) => r.data),
    enabled: !!drawId,
  });
}
...
export function usePower655VoidTenantPlayers(drawId: string, tenantId: string | null) {
  return useQuery({
    queryKey: power655Keys.voidTenantPlayers({ drawId, tenantId: tenantId ?? "" }),
    queryFn: () =>
      apiClient
        .get<{
          data: Array<{
        ...
        }>(`/power655/reports/void/draws/${drawId}/${tenantId}/players`)
        .then((r) => r.data),
    enabled: !!(drawId && tenantId),
  });
}
