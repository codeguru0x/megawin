...
import { PrizeTier, type EntryPayoutTier, type TicketEntryEntity } from "@megawin/game-power655/entities";
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-4xl">
        {entry ? (
          <Power655EntryDetailContent entry={entry} />
        ) : (
          <EntryDetailDialogLoading title="Phiếu cược — Power 6/55" />
        )}
      </DialogContent>
    </Dialog>
  );
...
/** Nội dung chi tiết thật — chỉ render khi `entry` đã fetch xong (xem `Power655EntryDetailDialog`). */
function Power655EntryDetailContent({ entry }: { entry: TicketEntryEntity }) {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-0.5">
                        <span className="text-foreground text-xs leading-tight font-semibold">{playLabel}</span>
                  ...
                  {boards.map((board, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center gap-1">
                          {board.mainNumbers.map((n: string) => (
                            <PowerMatchBall key={n} n={n} size="sm" variant="default" />
                          ))}
                        </div>
                      </div>
                    );
