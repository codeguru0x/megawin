...
import { usePower655Players, usePower655TenantDraws, usePower655TenantList } from "../use-report-queries";
...
function PlayerBreakdown({ drawId, tenantId }: { drawId: string; tenantId: string }) {
  const { navigateToEntriesFromTenant } = usePower655ReportFilters();
...
export function ByTenantTab() {
  const { level, tenantId, drawId, accountId, playerName } = usePower655ReportFilters();
