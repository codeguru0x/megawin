...
function DrawList() {
  const { from, to, page, setPage, navigateToDraw } = usePower655ReportFilters();
  const { data: summary } = usePower655DrawSummary(from, to);
  const { data, isLoading, error } = usePower655DrawList(from, to, page);
  ...
  return (
    <div className="space-y-4">
      {summary && <GameDrawKpiStrip data={summary} drawCountSub="kỳ đã settle · 3 kỳ/tuần" />}
      <GameDrawReportTable
        rows={data.data}
...
function DrawTenantBreakdown({ drawId }: { drawId: string }) {
  const { navigateToPlayersInDraw } = usePower655ReportFilters();
  const { data, isLoading } = usePower655DrawTenants(drawId);
...
function PlayerBreakdown({ drawId, tenantId }: { drawId: string; tenantId: string }) {
  const { navigateToEntries } = usePower655ReportFilters();
  const { data: players, isLoading } = usePower655Players(drawId, tenantId);
...
export function ByDrawTab() {
  const { level, drawId, tenantId, accountId, playerName } = usePower655ReportFilters();
