...
import { usePower655Outstanding } from "../financial-reports/_lib/use-report-queries";
...
function OutstandingTable() {
  const { data, isLoading, error, dataUpdatedAt, isFetching, refetch } = usePower655Outstanding();
  ...
  // Tổng cho footer row
  const totalPlayers = filteredRows.reduce((s, r) => s + r.playerCount, 0);
  const totalTenants = filteredRows.reduce((s, r) => s + r.tenantCount, 0);
  const totalEntries = filteredRows.reduce((s, r) => s + r.entryCount, 0);
  const totalLines = filteredRows.reduce((s, r) => s + r.lineCount, 0);
          ...
          {filteredRows.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {filteredRows.map((row) => (
                    <TableRow key={row.drawId}>
                      ...
                      <TableCell>
                        <Link
                          href={`/games/power655/operations?draw=${row.drawId}`}
                ...
                {/* Summary Footer Row — chỉ hiển thị khi >1 kỳ, bỏ tổng player/tenant vì không có nghĩa cross-draw */}
