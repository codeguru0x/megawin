...
function KpiStrip({ data }: { data: OutstandingDrawReport[] }) {
  ...
  const cards = [
    { label: "Kỳ đang active", value: String(data.length), sub: "kỳ quay chưa settle" },
    {
      label: "Pending Entries",
      value: formatNumber(totalEntries),
      sub: "entries chưa settle",
...
function OutstandingTable() {
  const { data, isLoading, error, dataUpdatedAt } = usePower655Outstanding();
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-xs">
                Tự động refresh mỗi 60 giây · Power 6/55 có tối đa ~3 kỳ outstanding
...
function Power655OutstandingContent() {
  return (
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Entries chưa settle · Snapshot TTL 15 phút · Tự động refresh mỗi 60s
          </p>
        </div>
      </div>
