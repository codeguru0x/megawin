...
export default function Power655TenantConfigPage() {
  ...
  function handleCreateTenant(tenantId: string) {
    if (tenantConfigs?.some((c) => c.tenantId === tenantId)) {
      return;
    }
    ...
    <div className="@container/main flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-foreground text-lg font-semibold tracking-tight">Power 6/55 — Cấu hình đại lý</h1>
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-0.5 p-1.5">
              {filtered.map((t) => (
                <button
                  ...
                  className={`hover:bg-muted/60 flex w-full items-center gap-3 rounded-lg px-3 py-3 text-left transition-colors ${selected === t.tenantId ? "bg-primary/10 ring-primary/30 ring-1" : ""}`}
                >
        ...
        <DialogFooter>
          ...
          <Button onClick={() => selected && onCreateTenant(selected)} disabled={!selected || isCreating}>
            {isCreating ? <Spinner className="mr-2" /> : null}Tạo cấu hình
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
