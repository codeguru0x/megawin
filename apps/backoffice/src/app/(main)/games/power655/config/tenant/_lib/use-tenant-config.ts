...
export function useTenantConfigs() {
  return useQuery({
    queryKey: LIST_KEY,
    queryFn: () => apiClient.get<{ configs: TenantConfig[] }>("/power655/tenant-config").then((r) => r.configs),
  });
}
...
export function useCreateTenantConfig() {
  ...
  return useMutation({
    mutationFn: (tenantId: string) =>
      apiClient.put<{ config: TenantConfig; version: number }>(`/power655/tenant-config/${tenantId}`, {
        isEnabled: true,
      }),
...
export function useUpdateTenantConfig(tenantId: string) {
  ...
  return useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      apiClient.put<{ config: TenantConfig; version: number }>(`/power655/tenant-config/${tenantId}`, data),
    onSuccess: (res) => {
      queryClient.setQueryData(detailKey(tenantId), res.config);
      queryClient.invalidateQueries({ queryKey: LIST_KEY });
