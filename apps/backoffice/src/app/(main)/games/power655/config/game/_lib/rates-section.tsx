            ...
            <div className="mx-6 mb-2 space-y-1.5">
              <div className="h-3 w-full overflow-hidden rounded-full bg-muted">
                <div className="flex h-full">
                  <div className="h-full bg-blue-500 transition-all duration-300" style={{ width: `${Math.min(commissionRate, 100)}%` }} />
                  <div className="h-full bg-emerald-500 transition-all duration-300" style={{ width: `${Math.min(companyRate, 100 - Math.min(commissionRate, 100))}%` }} />
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
                <span className="flex items-center gap-1.5">
                  <span className="inline-block size-2 rounded-full bg-blue-500" />Hoa hồng: {commissionRate}%
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="inline-block size-2 rounded-full bg-emerald-500" />Thu công ty: {companyRate}%
                </span>
                {remainingRate > 0 && (
                  <span className="flex items-center gap-1.5">
                    <span className="inline-block size-2 rounded-full bg-muted-foreground/30" />Giải thưởng / quỹ: {remainingRate}%
                  </span>
                )}
              </div>
            </div>
                      ...
                      <div className="flex items-baseline gap-2">
                        <FormControl>
                          <MoneyInput
                            ...
                            isAllowed={({ floatValue }) => floatValue === undefined || (floatValue >= 0 && floatValue <= Math.max(maxCommission, 0))}
                          />
                        </FormControl>
                      ...
                      <div className="flex items-baseline gap-2">
                        <FormControl>
                          <MoneyInput
                            ...
                            decimalScale={1}
                            thousandSeparator={false}
                            isAllowed={({ floatValue }) => floatValue === undefined || (floatValue >= 0 && floatValue <= Math.max(maxCompany, 0))}
                          />
                        </FormControl>
                        <span className="text-lg font-semibold text-muted-foreground">%</span>
                      </div>
