...
/**
 * Power 6/55 Operations — Alerts Panel (tab Giám sát)
 *
 ...
 * tự bung (mirror Keno). Mặc định chỉ hiện alert `new`, phần đã ack gộp xuống 1 dòng
 * "N đã xử lý" mỗi nhóm.
 *
 ...
 * `bao_high_stake` (đặc thù Power 6/55). `describeAlert` format theo payload từng loại
...
function describeAlert(type: string, payload: Record<string, unknown>): AlertDescription {
  switch (type) {
    case Power655OpsAlertType.LargeBet: {
    ...
    case Power655OpsAlertType.ExposureThreshold: {
    ...
    case Power655OpsAlertType.ComboConcentration: {
    ...
    case Power655OpsAlertType.BaoHighStake: {
...
/** Đọc `payload.top` (mảng Power655TopPotential) an toàn — chỉ large_bet có. */
function readTopEntries(payload: Record<string, unknown>): Power655TopPotential[] {
  const top = payload.top;
  if (!Array.isArray(top)) {
    return [];
  }
  return top as Power655TopPotential[];
}
...
function AlertTopEntries({ drawId, payload }: { drawId: string; payload: Record<string, unknown> }) {
  ...
  return (
    <div className="border-border/50 divide-border/40 mt-1 divide-y overflow-hidden rounded-md border">
      {entries.map((e) => {
        const href = buildOutstandingHref(GameProduct.Power655, drawId, e.accountId, e.username);
        return (
...
function AlertItemRow({ alert }: { alert: Power655OpsAlertEntity }) {
 ...
 */
function AlertGroupContent({ items }: { items: Power655OpsAlertEntity[] }) {
 ...
 * tab Giám sát. Snapshot Power 6/55 KHÔNG mang severity → không phân biệt "critical" như
