...
export function NumberBadge({
  ...
  heatLevel?: HeatLevel;
  /** Đang được chọn trong chế độ tra cứu — badge tô đậm brand red. */
  selected?: boolean;
}) {
...
function NumberCell({
  ...
  totalRows: number;
  heatLevel: HeatLevel;
  /** Đang được chọn — ô hiện ring red nổi bật. */
  selected: boolean;
...
/** Số cột grid — layout UI (Power 6/55 55 số → 11 cột × 5 hàng). */
const COLS = 11;
const TOTAL = 55;
...
function MainGrid({
  ...
  /** Bộ số đang chọn — ô đã chọn có ring red. Bảng LUÔN cho click chọn. */
  selected: Set<string>;
  onToggle: (num: string) => void;
}) {
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-foreground text-xs font-semibold">Số chính (01–55)</span>
        </div>
 ...
 *
 * KHÁC KENO (chỉ 8/9/10) — Power 6/55 nhiều mức. Đây là hint client; server (`.refine`
 * `comboLookupQuerySchema`) là chốt chặn cuối.
 */
 ...
 *
 * Bảng 55 số cho chọn số tuỳ ý; dialog lấy bộ số đang chọn làm input, PlayType tự suy
...
function ComboLookupDialog({
  ...
  // Nhập CSV thủ công → parse & replace selection. Chỉ commit khi mọi token là số 01-55
  ...
  const handleCsvChange = (value: string) => {
    ...
    const parsed = value
      ...
      .map((n) => n.trim())
      .filter(Boolean);
    const valid = parsed.every((n) => VALID_MAIN_NUMBER_SET.has(n));
    if (valid && new Set(parsed).size === parsed.length) {
      onReplace(parsed);
    }
  };
  ...
  const handleLookup = () => {
    if (pt) {
      lookup.mutate({ playType: pt, numbers: selected });
    }
  };
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            Chọn 5, 6, 7–15 hoặc 18 số trên bảng (hoặc gõ trực tiếp) để xem có bao nhiêu người/bộ đang dồn cược bộ này
            trong kỳ. Kiểu chơi tự suy theo số lượng số.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {pt ? ` · ${POWER655_PLAY_TYPE_LABELS[pt]}` : " · cần 5, 6, 7–15 hoặc 18"}
            </span>
            {selected.length > 0 && (
              <button
                type="button"
...
/**
 * NumberHeatmap — Power 6/55. Grid 11×5 = 55 số chính. Theme red với 5-level heat
      ...
      <CardHeader className="px-5 pt-4 pb-2">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5">
            {selected.length > 0 && (
              <>
                <span className="text-muted-foreground text-xs tabular-nums">Đã chọn {selected.length} số</span>
                <Button
                  type="button"
                  variant="ghost"
                  size="icon"
