...
function EntryNumbers({ entry }: { entry: WinningEntryItem }) {
  ...
  const winningSet = new Set(entry.winningMain);
  const bonus = entry.bonusNumber;
  return (
    <div className="flex flex-col gap-1.5">
      {entry.boards.map((b) => (
        <div key={b.boardNo} className="flex items-start gap-1.5">
          ...
          <div className="flex items-center gap-0.5 flex-wrap max-w-90">
            {b.mainNumbers.map((n) => (
              <PowerMatchBall
                key={n}
                n={n}
                size="sm"
                variant={winningSet.has(n) ? "matched" : bonus && n === bonus ? "bonus" : "default"}
              />
            ))}
          </div>
        </div>
