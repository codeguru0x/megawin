...
export function useVietlottResult(drawId: string | undefined, drawPeriod: string, enabled: boolean) {
  return useQuery({
    queryKey: power655Keys.vietlottResult(drawId ?? "", drawPeriod),
    queryFn: () =>
      apiClient.get<GetVietlottResultOutput>(`/power655/draws/${drawId}/vietlott-result`, {
        params: { drawPeriod },
      }),
...
export function useTriggerSettle() {
  return useDrawAction((id) => `/power655/draws/${id}/trigger-settle`, "post", "Đã bắt đầu kết sổ.");
}
...
export function useVoidDraw() {
  return useDrawAction<{ reason: string }>((id) => `/power655/draws/${id}/void`, "post", "Đã huỷ kỳ quay.");
}
