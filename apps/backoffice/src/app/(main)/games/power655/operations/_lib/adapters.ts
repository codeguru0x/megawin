/**
 * Power 6/55 Operations — Adapters
 *
 ...
 * gọi trong `useMemo` ở section, dùng chung 1 nguồn. Mirror `keno/operations/_lib/adapters.ts`.
 *
...
type Stats = Power655DrawBettingStatsEntity;
...
/**
 * KPI strip — hợp nhất 2 nguồn (mirror Keno §5.3): kỳ `Settled` ưu tiên số CHÍNH THỨC từ
 * settle (`financial`/`stats` trên `DrawDoc`); live/Settling/Void dùng ops stats.
 * `uniquePlayers` luôn từ snapshot (`countDocuments` trên `power655_draw_account_stats`).
...
// ─── Number frequency (heatmap 55 số) ────────────────────────────────────────

/**
 * `power655_draw_number_stats` (≤55 doc) → NumberFreqItem[55] — điền "01".."55" đầy đủ
 * (số chưa có cược → sets/amount/boards = 0). Nguồn TÁCH RIÊNG collection (KHÁC Keno
 * nhúng trong stats doc) — xem JSDoc `Power655DrawNumberStatsDoc`.
 */
export function toNumberFreq(numberStats: Power655DrawNumberStatsEntity[]): NumberFreqItem[] {
 ...
 * (derive lúc đọc từ `power655_draw_combo_stats`, sort `sets desc` phía BE).
 */
export function toTopCombos(topCombos: Power655TopCombo[]): TopComboRow[] {
...
/**
 * Build ExposureView từ `snapshot.exposure` (Power655SnapshotExposure) — field cấp
 * snapshot, KHÔNG phải `stats.exposure` (đó chỉ có `fixedWorstCase`, thiếu jackpot vì
 * jackpot đọc snapshot pool lúc build response — xem JSDoc {@link Power655SnapshotExposure}).
 */
export function toExposureView(exposure: {
  ...
  return {
    ...
    jackpot1: exposure.jackpot1,
    jackpot2: exposure.jackpot2,
 ...
 * `power655_draw_account_stats`), BE đã sort tiền desc.
 */
