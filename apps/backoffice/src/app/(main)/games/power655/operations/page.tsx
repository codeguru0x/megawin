...
function LastUpdatedBadge({ drawId }: { drawId: string | undefined }) {
  ...
  useEffect(() => {
    function tick() {
      const state = qc.getQueryState(power655Keys.opsSnapshot(drawId ?? ""));
    ...
    <div className="@container/main flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-foreground text-lg font-semibold tracking-tight">Power 6/55 — Vận hành</h1>
      ...
      {/* Zone 1: Jackpot hero card — luôn hiện (đặc thù Power 6/55, ngoài tabs). */}
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="gap-6">
        ...
        {/* Tab Phân tích cược: unmount khi ở tab Giám sát (heatmap 55 cell không render). */}
      ...
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed py-20 text-center">
        ...
        <div className="mt-5 flex items-center gap-3">
          <Button variant="outline" size="sm" asChild>
            <Link prefetch={false} href="/games/power655/draws">
              Lịch sử kỳ quay
            </Link>
          </Button>
          {noData ? (
            ...
            <Button size="sm" asChild>
              <Link prefetch={false} href="/games/power655/operations">
                Về trang vận hành
              </Link>
            </Button>
          )}
