...
interface OutstandingDrawListProps {
  /** Danh sách draws outstanding (từ usePower655Outstanding). */
  data: OutstandingDrawReport[];
}

/**
 * Level 1 — bảng danh sách kỳ quay outstanding.
 *
 * Tự gọi usePower655OutstandingFilters() để lấy navigateToDraw — cùng pattern với financial reports.
 * Click bất kỳ cell nào trong row → drill vào Tenant Breakdown.
 * Icon ExternalLink hover-only trên cột drawId → link sang Operations page (page khác).
 */
export function OutstandingDrawList({ data }: OutstandingDrawListProps) {
  const { navigateToDraw } = usePower655OutstandingFilters();
