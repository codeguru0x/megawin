...
/**
 * POST /api/mega645/draws/[drawId]/resettle-reopen
 *
 ...
 * re-settle vì pool jackpot đổi — nhưng số quay của chúng không đổi. Luồng
 * publish-result thông thường return sớm khi result không đổi nên không mở được
 * `Settled → Published`. Endpoint này re-stamp `result.publishedAt` (GIỮ nguyên
 * winningNumbers) để vượt cổng `DRAW_NO_NEW_RESULT`, sau đó staff bấm "Kết sổ lại".
 *
 * Yêu cầu:
