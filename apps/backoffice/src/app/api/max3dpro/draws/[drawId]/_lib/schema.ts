...
const resultSchema = z.object({
  special: z
    .array(tripletSchema)
    .length(MAX3D_PRO_DRAW_COUNT_SPECIAL, `Giải Đặc Biệt phải có đúng ${MAX3D_PRO_DRAW_COUNT_SPECIAL} bộ ba số.`),
  first: z
    .array(tripletSchema)
    .length(MAX3D_PRO_DRAW_COUNT_FIRST, `Giải Nhất phải có đúng ${MAX3D_PRO_DRAW_COUNT_FIRST} bộ ba số.`),
  second: z
    .array(tripletSchema)
    .length(MAX3D_PRO_DRAW_COUNT_SECOND, `Giải Nhì phải có đúng ${MAX3D_PRO_DRAW_COUNT_SECOND} bộ ba số.`),
  third: z
    .array(tripletSchema)
    .length(MAX3D_PRO_DRAW_COUNT_THIRD, `Giải Ba phải có đúng ${MAX3D_PRO_DRAW_COUNT_THIRD} bộ ba số.`),
});
