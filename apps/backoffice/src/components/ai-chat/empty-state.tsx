    ...
    <ConversationEmptyState className="min-h-[60svh] justify-center gap-6">
      <div className="flex flex-col items-center gap-3">
        ...
        <div className="space-y-1">
          ...
          <h3 className="text-xl font-semibold tracking-tight">Xin chào, tôi là {AI_ASSISTANT_NAME}</h3>
          <p className="text-muted-foreground text-sm text-balance">Rất vui được hỗ trợ. Tôi có thể giúp gì cho bạn?</p>
        </div>
      </div>
      {suggestions.length > 0 && (
        ...
        <ul className="mx-auto flex w-full max-w-md flex-col">
          {suggestions.map((suggestion) => (
            <li key={suggestion}>
              <button
                className="hover:bg-accent focus-visible:bg-accent flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm transition-colors focus-visible:outline-none"
