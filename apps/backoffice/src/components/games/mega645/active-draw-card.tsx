...
> = {
  ...
  [DrawStatus.SalesOpen]: {
    ...
    accent: "from-game-mega645 via-profit to-profit",
    cardBg: "bg-linear-to-br from-game-mega645/60 via-card to-profit/30 dark:via-card",
    iconBg: "bg-game-mega645",
    iconColor: "text-game-mega645",
    pingColor: "bg-game-mega645",
    dotColor: "bg-game-mega645",
  },
...
export function Mega645PrimaryDrawCard({ draw }: { draw: CurrentDrawInfo }) {
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex cursor-default items-center gap-1.5 select-none">
                  <Unlock className="text-game-mega645 size-3.5" />
                  <span className="text-muted-foreground text-xs">Mở bán</span>
                  <span className="text-game-mega645 font-mono text-xs font-bold tabular-nums">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="dark:bg-card flex items-center gap-3 rounded-xl border bg-white/80 p-3">
              <div className="bg-game-mega645 flex size-8 items-center justify-center rounded-lg">
                <CircleDollarSign className="text-game-mega645 size-3.5" />
              </div>
              <div>
...
export function Mega645QueueDrawCard({ draw }: { draw: CurrentDrawInfo }) {
  const status = draw.status;
  const vis = STATUS_VISUALS[status] ?? DEFAULT_VISUAL;
  const drawTime = formatVNTime(new Date(draw.drawTime));
  const StatusIcon = getStatusIcon(status);
