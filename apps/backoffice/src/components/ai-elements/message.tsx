...
// Keyframes `sd-fadeIn`/`sd-blurIn`/`sd-slideUp` + rule `[data-sd-animate]` của streamdown. Đây là
// nơi DUY NHẤT định nghĩa animation cho từng từ mà `STREAM_TEXT_ANIMATION` bật lên — thiếu import
// này thì span `data-sd-animate` vẫn được sinh nhưng không có keyframes, text lại "nhảy" thành cục.
// Không nằm trong Tailwind nên `@source` không phủ được.
...
export const Message = ({ className, from, ...props }: MessageProps) => (
  <div
    className={cn(
      ...
      // User: bubble hẹp căn phải. Assistant: full width, KHÔNG bubble — câu trả lời có bảng số
      // liệu cần hết chiều rộng (đúng pattern ChatGPT/Claude/Grok, p0-04 §4.1).
      from === "user" ? "is-user ml-auto max-w-[85%] justify-end" : "is-assistant",
      className,
    )}
    {...props}
  />
...
export const MessageContent = ({ children, className, ...props }: MessageContentProps) => (
  <div
    className={cn(
      // `gap-3`: khoảng cách giữa các PART (text / tool card / reasoning) — trước dùng `gap-2`
...
export const MessageResponse = memo(
  ({ className, ...props }: MessageResponseProps) => (
    <Streamdown
      ...
      className={cn(
        "chat-md size-full [&>*:first-child]:mt-0 [&>*:last-child]:mb-0",
        // `chat-md-streaming`: cổng CSS để tắt `text-align: justify` cho ĐÚNG khối đang được viết
        // (xem `globals.css`). Đây là nguồn "giật" thứ hai, tách biệt với chuyện fade-in: đo thật
        // 17/08 bằng cách gỡ justify trên một đoạn đang stream → các từ dịch ngang tới 31px. Cơ chế:
        // spec `justify` KHÔNG dàn dòng cuối của đoạn, nên dòng đang gõ thì căn trái; ngay khi nó
        // wrap sang dòng mới, nó thành dòng-không-cuối và bị dàn đều ⇒ toàn bộ chữ vừa đọc nhảy
        // ngang một nhịp. Cứ mỗi lần xuống dòng lại nhảy một lần.
        props.isAnimating === true && "chat-md-streaming",
        className,
      )}
