...
export const PromptInputSubmit = ({
  ...
  if (isGenerating) {
    // CẢ `submitted` VÀ `streaming` đều là "agent đang làm việc" → phải hiện nút DỪNG bấm được.
    // Trước đây `submitted` hiện <Spinner/> trơ nên user tưởng không bấm được, dù handleClick
    // vẫn gọi onStop() — chính giai đoạn `submitted` (model đang thinking) là lúc cần dừng nhất.
  ...
  return (
    <InputGroupButton
      ...
      onClick={handleClick}
      size={size}
      title={isGenerating ? "Dừng tạo câu trả lời" : "Gửi tin nhắn"}
