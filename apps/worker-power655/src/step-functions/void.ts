 ...
 *  │     + build power655_void_draw_reports                   │
 *  └────────┬─────────────────────────────────────────────────┘
 *           ▼
 *  ┌──────────────────────────────────────────────────────────┐
 ...
 *  │     Bulk insert refund orders vào outbox │
 ...
 *
 * CRASH RECOVERY:
 *   Mỗi step idempotent. Entries đã void tự filter ra.
 *
 * REFUND SAU FINALIZE:
 *   EnqueueDispatchRefunds chạy sau FinalizeVoid — nhất quán với settle (EnqueueDispatchPayouts sau FinalizeSettle).
 *   Nếu enqueue fail → draw vẫn = void (không treo ở voiding). Admin retry thủ công.
 *
 * REFUND LOGIC:
