/**
 * Power 6/55 Resettle – Step Function Definition (ASL)
 *
 * ═══════════════════════════════════════════════════════════════════════
 * FLOW (CRASH-SAFE):
 * ═══════════════════════════════════════════════════════════════════════
 ...
 *    resettleContext: {
 *      resettleId, scenario, openingJp1, openingJp2,
 *      cycleDrawCountBefore, skipCycleUpdate
 *    }
 *  }
 ...
 *  │                              │  openingJp1/2 từ resettleContext thay vì
 ...
 *  └─────────────────────────┘
 *
 * KEY DIFFERENCES vs Max3D Resettle:
 *   - `resettleContext` chứa thêm openingJp1/2, cycleDrawCountBefore, skipCycleUpdate.
 *   - Settle SFN `PrepareSettle` đọc `resettleContext` (nếu có) để override
 *     jp1/2CurrentAmount từ ledger (không đọc activeCycle — đã thay đổi sau T).
 *   - `FinalizeSettle` dùng `skipCycleUpdate` để bỏ qua updateCycle khi TYPE_B1.
 *
 * KEY INVARIANTS (giống Max3D):
...
export const RESETTLE_STATE_MACHINE = {
  Comment: "Power 6/55 Resettle Step Function – Kết sổ lại kỳ quay (crash-safe, dual JP)",
  ...
  States: {
    ...
    // lockOwnerToken, lockKey } → `resettleContext` (openingJp1/2, scenario,
    ...
    //   openingJp1/2; FinalizeSettle đọc skipCycleUpdate.
