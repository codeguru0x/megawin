/**
 * Lambda: feed-scheduler (Max 3D Pro)
 *
...
import { AcquireFeedLockUseCase } from "@megawin/game-max3dpro-application/use-cases/feed";
...
export async function handler() {
  const executionName = `max3dpro-feed-sync-${Date.now()}`;
