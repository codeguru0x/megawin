/**
 * Lambda: sync-outstanding (Max 3D – Scheduled Job)
 *
 * EventBridge rule chạy mỗi 5 phút.
 * Aggregate entries WHERE { status: "scheduled" } group by drawId
 * → Upsert max3d_outstanding_draw_reports với snapshotAt = now (reset TTL).
 *
 * Khi draw settle/void, job không tạo doc mới → TTL tự xoá sau 15 phút.
 * Max 3D CÓ lineCount — aggregate lineCount (lines/pairs) từ entries.
 *
...
export async function handler() {
  ...
  // Max 3D CÓ lineCount — aggregateOutstandingByDraw trả lineCount (lines/pairs)
  const drawSnapshots = await entryRepo.aggregateOutstandingByDraw();

  // ── Bước 2: Upsert max3d_outstanding_draw_reports cho từng draw active ────
  ...
  const systemResult = await syncSystemUseCase.execute({
    ...
    outstandingDrawReportCollection: MAX3D_OUTSTANDING_DRAW_REPORTS,
  });
