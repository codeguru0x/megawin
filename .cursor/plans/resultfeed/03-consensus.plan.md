...
`resultfeed` không biết core đã settle hay chưa (D7) ⇒ nó chỉ đánh dấu `Conflict` + alert; việc đối
...
tự quyết settle/resettle. `resultfeed` **không** biết những việc đó.

### 6.1. Quyết định đã chốt (2026-09-01): cờ `RESULTFEED_AUTO_PUBLISH_UNVERIFIED` — sai khác CÓ CHỦ ĐÍCH so với bảng G6 ở trên, CHỈ dùng giai đoạn thử nghiệm

Bảng ở §6 mô tả **thiết kế production cuối cùng** (G6 cần đo được độ tin cậy từ G5 trước khi bật, và
yêu cầu `HumanVerified` — không phải `Agreed` của máy). Nhưng hiện tại `resultfeed` **chưa có G5** (core
chưa PULL gì cả) — dữ liệu đang ở giai đoạn **tham khảo/thử nghiệm chất lượng sản phẩm**, và quan trọng
nhất: **quyết định thật (áp dụng vào `DrawDoc`, trả thưởng) vẫn nằm ở phía core, do người duyệt khi core
cập nhật kết quả draw** — không phải tại `resultfeed`. Vì lớp chặn "người duyệt trước khi tiền chảy" vẫn
tồn tại (chỉ nằm ở core, không nằm ở đây), việc để máy tự publish `Agreed` sớm ở `resultfeed` trong giai
đoạn này an toàn hơn bảng G6 giả định (G6 giả định `resultfeed` là **nguồn duy nhất** trước khi tiền
chảy — hiện tại KHÔNG phải vậy vì G5 core PULL chưa được xây).

**Cơ chế:** biến môi trường `RESULTFEED_AUTO_PUBLISH_UNVERIFIED` (đọc trong `apps/worker-resultfeed`,
nơi `consensus-tick` Lambda chạy — KHÔNG phải field trong DB/backoffice UI, để tránh staff vô tình bật
nhầm qua UI; bật/tắt cờ này là quyết định **vận hành/deploy**, không phải quyết định nghiệp vụ hàng ngày).

- `true` ⇒ `ConsensusTickUseCase` set `publishedAt = now` NGAY khi máy quyết `state = Agreed` (cùng
  1 lần ghi với `applyMachineDecision`, KHÔNG cần chờ `HumanVerified`).
- Không set / `false` (mặc định) ⇒ đúng hành vi bảng gốc: `Agreed` không tự `publishedAt`, cần G5's
  manual publish (hoặc G6 sau này).
- **KHÔNG** đụng gì tới `HumanVerified`/`Rejected` — người vẫn verify/publish tay qua trang review bình
  thường, không xung đột với cờ này (verify tay luôn thắng, D6 vẫn giữ nguyên).
- **Điều kiện phải tắt trước khi sang G5 thật** (core bắt đầu PULL `resultfeed` để settle tiền thật):
  quay lại đúng bảng gốc ở §6 — bỏ cờ này (hoặc set `false`) trước khi bất kỳ ai gọi
  `GET /api/resultfeed/results` cho mục đích settle. Đây là lý do cờ nằm ở env (per-environment), không
  phải 1 setting global vĩnh viễn.
...
- [ ] Test: đúng 1 nguồn `Authoritative` duy nhất, `IntrinsicState.Passed`, không có nhóm nào khác lệch
      ⇒ **Agreed** (không kẹt ở Pending mãi — §3 bước 5 nhánh "KHÔNG" tự đúng khi chỉ có 1 nguồn).
- [ ] Test: `RESULTFEED_AUTO_PUBLISH_UNVERIFIED=true` + `state=Agreed` ⇒ `publishedAt` được set cùng
      lần ghi; `=false`/không set ⇒ `publishedAt` vẫn `null` (§6.1).
- [ ] Test: cờ auto-publish KHÔNG có tác dụng gì khi `state != Agreed` (Pending/Conflict/Rejected/HumanVerified).
