...
# ResultFeed — Data Model

DB **`megawin-resultfeed`** — **dùng CHUNG cluster/URI với core** (`MONGODB_URI`), chỉ khác `dbName`.
6 collection. Tuân thủ `mongodb.mdc` (`docPath`, repo types tách file, TTL index ưu tiên hơn cleanup
batch) và `entity-typesafe-mongodb.mdc`.

## 1. Wiring DB — reuse `MONGODB_URI`, chỉ thêm 1 `dbName` mới

Không cần cluster/env Mongo riêng. Tiền lệ có sẵn trong `packages/data/src/mongo/base-repos.ts`:
`AuditRepo`, `ReportRepo`, `IdentityRepo`, `TenantRepo`, `GameRepo` đều là DB **khác nhau** nhưng
**cùng cluster** — không ai truyền `mongoEnvKey` riêng, tất cả rơi về default `"MONGODB_URI"`
(`repository.ts:88-89`). `ResultFeedRepo` theo đúng tiền lệ này, không phải ngoại lệ.

### 1.1. `packages/data/src/mongo/constants.ts`

```typescript
Default: {
  // … các DB hiện có (DbName, GameDbName, IdentityDbName, MegawinTenantDbName, ReportDbName, AuditDbName) …
 ...
 *
 * Không tách cluster từ đầu: khối lượng ghi rất nhỏ (~440 request/ngày → §3 `00-overview.md`),
 * không có lý do kỹ thuật để trả thêm một connection string. Nếu sau này ResultFeed phục vụ
 * API public với tải đọc lớn và cần cách ly khỏi OLTP game — thêm `mongoEnvKey` riêng NGAY tại
 * đây (một tham số, không phải refactor gọi nơi khác — xem tiền lệ comment ở `ReportReadRepo`,
 * `base-repos.ts:102`).
 */
...
Không cần sửa `client.ts`/`repository.ts` — không truyền `mongoEnvKey` thì tự rơi về default
`"MONGODB_URI"` (`repository.ts:88-89`); `getMongoDatabase` cache theo `${mongoEnvKey}::${dbName}`
(`client.ts:107`) nên nhiều `dbName` share 1 client Mongo là cách dùng **đã có sẵn**, không phải mới.

### 1.3. Env — không thêm env Mongo mới

| Env | Ở đâu | Ghi chú |
| --- | --- | --- |
| `MONGODB_URI` | đã có sẵn ở mọi app (`apps/worker-resultfeed`, `apps/backoffice`, `apps/api-resultfeed`) | **Không thêm gì** — `ResultFeedRepo` dùng chung connection string này, chỉ khác `dbName` |
| `OXYLABS_USERNAME` · `OXYLABS_PASSWORD` | worker | Oxylabs Web Unblocker (**primary** — `00-overview.md` D1). Secret lấy từ SSM như `MONGODB_URI` (`serverless.yml:22`) |
| `CONTEXTDEV_API_KEY` | worker | context.dev Scrape HTML (**secondary/failover**). Cùng cơ chế SSM |

**Không có** `RESULTFEED_FETCH_PROVIDER`: provider được chọn **trong code** (registry theo
`sources[].providerId`), không qua env. Lý do: một env toàn cục sẽ ép mọi source dùng cùng vendor, trong
khi đúng mô hình là **mỗi source có thể dùng vendor khác nhau** — thứ vừa trở nên quan trọng khi ta phải
đổi vendor cho riêng `vietlott`.

### 1.4. Lock

Dùng `@megawin/worker-core` (`TickLoopWorker`/`SingleRunWorker`/`DistributedMutex`). Lock của
worker-core nằm ở DB **`megawin`** (`WorkerCoreBaseRepo extends SharedRepo`,
`packages/worker-core/src/infras/base-repo.ts:14`) — cùng cluster với `megawin-resultfeed` (§1),
chỉ khác `dbName`, nên đây **không phải** một dependency hạ tầng nặng, chỉ là dùng lại tick-loop/lock
đã có test sẵn thay vì viết lại.

Đặt tên lock: `resultfeed:fetch:<sourceId>:<gameKey>` và `resultfeed:consensus:<gameKey>`.

---

## 2. Enum — `const object as const`, KHÔNG string trần

`packages/resultfeed/src/entities/enums.ts`. Tuân thủ `code-quality-standards.mdc` §5.3.
...
/** Game mà ResultFeed thu thập. TỰ khai báo — KHÔNG import từ `@megawin/game-*` (overview §6). */
...
Tên collection **không cần prefix** `resultfeed_` vì đã ở DB riêng — prefix chỉ thêm nhiễu.
...
| Lotto535 | **thứ tự quay**, nhưng có 2 miền số tách biệt: 5 main (1-35) + 1 đặc biệt (1-12, đứng cuối) | `03 04 06 15 22 \| 09` |

Hai hash, **không được trộn**:

| Hash | Tính từ | Dùng để |
| --- | --- | --- |
| `payoutHash` | Số đã **canonical** + `gameKey` + `drawPeriod` | So **giữa các nguồn**. Trả thưởng độc lập thứ tự (trong từng miền số) ⇒ hai nguồn ghi khác thứ tự **KHÔNG phải** conflict |
| `displayHash` | Số **đúng thứ tự nguồn công bố** + `gameKey` + `drawPeriod` | Giữ dạng công bố để ghi ra ngoài. Với Bingo18/Lotto535 **khác** `payoutHash` |

```typescript
/**
 * Canonical hoá số của 1 game để so sánh CHÉO NGUỒN.
 *
 * KHÔNG còn "sort tăng dần cho MỌI game" — mỗi game canonical hoá theo đúng cách
 * trả thưởng của nó, tự khai theo `gameKey`:
 * - Keno/Bingo18: sort toàn mảng tăng dần (trả thưởng độc lập thứ tự toàn bộ).
 * - Lotto535: có 2 miền số KHÔNG được trộn (5 main 1-35 + 1 đặc biệt 1-12, đứng
 *   cuối) — chỉ sort riêng 5 phần tử main, giữ cố định phần tử thứ 6 (đặc biệt)
 *   ở cuối. Sort toàn mảng flat sẽ làm mất phân biệt main/đặc biệt.
 *
 * KHÔNG dùng kết quả hàm này để ghi ra ngoài: Bingo18/Lotto535 phải công bố đúng
 * thứ tự nguồn (`5,2,5` / main+đặc biệt), không phải thứ tự đã canonical.
 *
...
PULL**, không phải của `resultfeed` (D7). `resultfeed` không được biết quy ước `drawId` của MegaWin.

**Ba field là BẮT BUỘC — mọi thứ còn lại chỉ là hỗ trợ, có thể vắng mặt:** `drawPeriod`,
`drawDateSource`, và `numbersDisplay` (kết quả) là lý do `resultfeed` tồn tại — thiếu 1 trong 3 thì
observation vô giá trị. `claimedChecksums` **cố ý** là `Record<string, string | number>` generic,
không phải field cứng liệt kê `chan`/`le`/`lon`/`nho` — vì mỗi game công bố checksum khác nhau
(Keno: chẵn/lẻ/lớn/nhỏ; Bingo18: tổng + Lớn/Hòa/Nhỏ; game sau có thể không công bố gì hoặc công bố
kiểu khác hẳn). Thêm site/game mới mà nguồn có checksum lạ ⇒ **thêm key vào record đang có**, không
sửa schema `ObservationDoc`. Nguồn không công bố checksum nào ⇒ record rỗng `{}` + `intrinsicState =
NotAvailable` (§3) — đây là trạng thái **hợp lệ bình thường**, không phải lỗi thiếu dữ liệu.
...
export interface SourceCursorDoc extends BaseEntity {
  ...
  /** Không fetch trước mốc này (tôn trọng `minIntervalMs` + backoff, hoặc lịch quay cố định — xem 05-lotto535-and-schedule.plan.md §2). */
  nextFetchAt: Date;
...
**Không còn `needsReanchor`.** Thiết kế ban đầu (đọc trang list để "re-anchor" khi lệch kỳ) chưa
từng chạy được và tạo nguy cơ deadlock (`05-lotto535-and-schedule.plan.md` §1). Khi kỳ trả về LỆCH
`nextExpectedPeriod` (`period_gap`), cơ chế mới: ghi alert Warning rồi **tự nhận kỳ thực tế làm
anchor mới** (`recordSuccess` với `lastConfirmedPeriod = actual`) và tiếp tục — không block, không
cần đọc trang list.

### 4.6. `alerts` — ops alert RIÊNG, không dùng `ops_alerts` của game

`resultfeed` không import `game-*` (overview §6) ⇒ có bảng alert riêng. Loại alert theo `as const`:
`fetch_failing`, `parse_failed`, `intrinsic_failed`, `consensus_conflict`, `period_gap`,
`source_stale`, `cost_spike`, `human_review_backlog`.

---

## 5. Index — source of truth ở `packages/resultfeed/src/indexes/index.ts`
...
- [ ] `ResultFeedDbName` + `ResultFeedRepo` thêm vào `@megawin/data`, có JSDoc giải thích vì sao cluster riêng.
- [ ] Mọi enum dùng `const object as const` + type dẫn xuất — không string literal trần.
- [ ] `canonicalizeNumbers` có test: bất biến 1–4 ở §3, đặc biệt **Bingo18 không dedupe**, **Lotto535 không trộn 2 miền số** (main vs đặc biệt).
...
- [ ] Không file nào trong `packages/resultfeed*` import `@megawin/game-*`.