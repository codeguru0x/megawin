...
| [`p2-02-view-transitions-polish`](./p2-02-view-transitions-polish.plan.md) | P2 | ✅ done | `(main)/template.tsx` + `<ViewTransition>` + CSS. Lỗi `startTime` từng nghi do transition → đã truy ra là **React DevTools extension** (`installHook.js` / `devToolsReportSoftNavs`), không phải app | Không — thuần CSS/UX |
