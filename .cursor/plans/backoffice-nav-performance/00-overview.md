...
| [`p2-02-view-transitions-polish`](./p2-02-view-transitions-polish.plan.md) | P2 | ✅ done | `(main)/template.tsx` + `<ViewTransition>` + CSS. Lỗi `startTime` từng nghi do transition → đã truy ra là **bug Chrome DevTools Live Metrics** (script inject vào isolated world, vá upstream 31/08/2026), không phải app | Không — thuần CSS/UX |
