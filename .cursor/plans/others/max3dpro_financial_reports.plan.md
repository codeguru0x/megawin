---
name: Max3DPro Financial Reports
overview: Triển khai financial reporting cho Max 3D Pro (per-game). KHÔNG CÓ jackpot, CÓ lineCount (pairs). Gần giống Max 3D.
todos:
  - id: entity-max3dpro
    content: Tạo packages/game-max3dpro/src/entities/report.ts + export
    status: pending
  - id: repo-max3dpro
...
# Financial Reports: Max 3D Pro Implementation
...
## Đặc thù Max 3D Pro — KHÔNG CÓ JACKPOT, CÓ lineCount (pairs)
...
- **CÓ lineCount** trên entry (pairs per board — multiNumber hoặc multiDigit)
- **KHÔNG CÓ ApplyPayoutCaps, KHÔNG CÓ CheckJackpotWinner**
- GameProduct: `GameProduct.Max3dpro`
- Collection prefix: `max3dpro`
- Entry collection: `max3dpro_ticket_entries`
- SettleFinancials: `{ totalRevenue, totalFixedPrizes, totalAgentCommission, profit }`
- Old report collection: `max3dProDailyReports` (extends AbstractReportRepository)
...
Tạo `packages/game-max3dpro/src/entities/report.ts`:

- Copy từ lotto535, đổi prefix `LOTTO535_` → `MAX3DPRO_`, `lotto535_` → `max3dpro_`
- `lineCount` optional field — Max3DPro CÓ lineCount (pairs)
- Export qua `index.ts`

## Phase 2: Repository Layer

Tạo trong `packages/game-max3dpro-application/src/infras/repos/`:

4 repo files — copy từ lotto535, đổi import + collection:

- `settle-draw-report-repo.ts` → `MAX3DPRO_SETTLE_DRAW_REPORTS`
- `settle-tenant-report-repo.ts` → `MAX3DPRO_SETTLE_TENANT_REPORTS`
- `void-report-repo.ts` → `MAX3DPRO_VOID_DRAW_REPORTS`
- `outstanding-report-repo.ts` → `MAX3DPRO_OUTSTANDING_DRAW_REPORTS`

Export qua `repos/index.ts`.

## Phase 3: Entry Repository — Aggregation Methods

Thêm vào `packages/game-max3dpro-application/src/infras/repos/entry-repo.ts`:
...
Sửa `packages/game-max3dpro-application/src/use-cases/void/types.ts`:

- Thêm `financialDate: string`

Sửa `packages/game-max3dpro-application/src/use-cases/void/prepare-void.ts`:
...
```typescript
  // Max3DPro KHÔNG có Jackpot — KHÔNG có field jackpotContribution trong upsert
  // companyTake = profit
...
- `settle/publish-settle-daily.ts` — GameProduct.Max3dpro, MAX3DPRO_SETTLE_*
