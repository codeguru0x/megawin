...
# Plan: Lotto 5/35 — Thêm betCount multiplier
...
1. **Game rule**: `.cursor/rules/lotto535-game-rules.mdc`
2. **Code quality**: `.cursor/rules/code-quality-standards.mdc`
3. **Entity files**: Đọc toàn bộ `packages/game-lotto535/src/entities/` trước khi sửa
4. **Plan mẫu**: `.cursor/plans/keno_bet_count.plan.md` — plan gốc cho Keno
...
| Field                                     | Ý nghĩa                                                                                          | Lotto535 (plan này)                                   |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------ | ----------------------------------------------------- |
...
- **Lotto535**: ⬜ plan này
- **Mega645, Power655**: ⬜ tương lai (cấu trúc tương tự Lotto535)

---

## Phân tích hiện trạng Lotto 5/35

### Đặc thù Lotto 5/35 (khác Keno/Max3D)

- **CÓ Line entity** (`TicketLineDoc`) — lines tạo khi settle, mỗi line match độc lập
- **CÓ Jackpot** + Split Cycle — settle pipeline phức tạp hơn Keno
- **KHÔNG có side bets** — chỉ có boards (đơn giản hơn Keno)
- **KHÔNG cần rename** — Lotto535 chưa có field `betCount` nào trên entry, không conflict
- **Board có `derived.expandedLines`** — lineCount per board đã có sẵn
- **Có payout caps** — KHÔNG (khác Keno). Payout tính theo tier cố định

### Thuật ngữ cho Lotto535
...
Settle pipeline Lotto 5/35 có 9 steps:

```
PrepareSettle → SettleEntries (loop) → CalculateFinancials → CheckPrizeRoute →
  ├─ PatchJackpotPrize (4a, có JP winner)
  ├─ ApplySplitBonuses (4b, split cycle)
  └─ (skip, kỳ thường)
...
- **Split Bonuses**: bonusPerWinner tính per-unit, nhân betCount khi patch
...
- Tiền thưởng per line = `unitWinAmount × betCount` (của board chứa line đó)
...
- Trúng tier3 (500.000 VND): thưởng = 500.000 × 3 = 1.500.000 VND

**MainCover 6 (6 lines), betCount = 2:**

- 1 board × 6 lines × 2 betCount = 12 bet units → tiền cược = 120.000 VND
- Trúng 5 chính + ĐB: 1 line Jackpot + 5 lines tier2
- Thưởng tier2 mỗi line = 5.000.000 × 2 = 10.000.000 VND
- Tổng tier2 = 5 × 10.000.000 = 50.000.000 VND + Jackpot × 2
...
| 9   | PatchJackpotPrize: winnerCount đếm **số lines** (không nhân betCount), jackpotPerWinner per-unit → nhân betCount khi patch | 1 line trúng JP betCount=3 vẫn = 1 winner, nhưng nhận 3× prize |
| 10  | ApplySplitBonuses: bonusPerWinner per-unit → nhân betCount khi patch                                                       | Tương tự JP                                                    |
| 11  | `PlayRules` thêm `minBetCount` (default 1) + `maxBetCount` (default 10)                                                    | Config chuẩn cross-game                                        |
| 12  | `BoardDerived` thêm `betCount` (snapshot)                                                                                  | Truy xuất nhanh trên ticket                                    |
...
### 1.1 Game Config — `packages/game-lotto535/src/entities/types.ts`
...
**File**: `packages/game-lotto535/src/rules/jackpot.ts` — `DEFAULT_LOTTO535_CONFIG`

Thêm `minBetCount: 1`, `maxBetCount: 10` vào `DEFAULT_LOTTO535_CONFIG.play`.

### 1.2 Ticket — `packages/game-lotto535/src/entities/ticket.ts`
...
### 1.3 Entry — `packages/game-lotto535/src/entities/entry.ts`
...
### 1.4 Line — `packages/game-lotto535/src/entities/line.ts`
...
- `jackpot.ts` — `DEFAULT_LOTTO535_CONFIG.play` thêm 2 fields
...
### 1.5.1 API Schema — `apps/backoffice/src/app/api/lotto535/config/_lib/schema.ts`

Thêm `minBetCount: positiveInt`, `maxBetCount: positiveInt` vào `playSchema`.

### 1.5.2 UI Form — `play-rules-section.tsx`
...
### 2.1 API Schema — `apps/api-player/src/handlers/lotto535/place-bet.ts`
...
### 2.2 DTO — `packages/game-lotto535-application/src/use-cases/place-bet/dto/place-bet.dto.ts`
...
### 2.3 Use Case — `packages/game-lotto535-application/src/use-cases/place-bet/place-bet.ts`
...
const lineDocs: Array<Omit<TicketLineDoc, "_id">> = lines.map((line, i) => {
  ...
  return {
    ...
    matchResult: {
      ...
      tier: perLine.tier,
      // Jackpot: winAmount = 0 tạm thời (PatchJackpotPrize step 4a).
...
> **LƯU Ý quan trọng**: `EntryPayoutTier.hitCount` = số LINES trúng (không nhân betCount). `amount` = tổng thưởng đã nhân betCount. Nếu board A (betCount=2) có 3 lines trúng tier3 → hitCount=3, amount=3×500K×2=3.000.000.

### 3.2 PatchJackpotPrize — Step 4a

**File**: `packages/game-lotto535-application/src/use-cases/settle/patch-jackpot-prize.ts`

Hiện tại: `jackpotPerWinner = floor(totalPool / jackpotEntries.length)`.

**QUY TẮC VIETLOTT**: "Giải Độc Đắc được chia đều **theo tỷ lệ giá trị tham gia dự thưởng** của người trúng thưởng."

→ Jackpot KHÔNG chia đều per line/entry, mà **chia theo tỷ lệ betCount** (= số lần tham gia dự thưởng).

**VÍ DỤ**:

- Player A trúng JP 1 line, betCount = 3 → tham gia 3 đơn vị
- Player B trúng JP 1 line, betCount = 1 → tham gia 1 đơn vị
- Tổng = 4 đơn vị
...
// entry.payout.tiers[jackpot].amount = Σ(jackpotPerUnit × line.betCount) cho JP lines của entry
```

**LƯU Ý**: Khi betCount = 1 cho tất cả (backward compat), totalBetUnits = số lines = hiện tại. Kết quả giống nhau.

**Thay đổi aggregateSettleSummary**: `tierWinnerCounts` phải tổng hợp CẢ betCount:

- Thêm `tierBetUnitCounts` = Σ(hitCount × betCount) per tier — dùng cho split distribution
- Giữ `tierWinnerCounts` = Σ(hitCount) per tier — dùng cho thống kê/display

### 3.3 ApplySplitBonuses — Step 4b

**File**: `packages/game-lotto535-application/src/use-cases/settle/apply-split-bonuses.ts`

Tương tự PatchJackpotPrize. Quy tắc: "giá trị lĩnh thưởng = số lần tham gia dự thưởng × giá trị giải".

**Thay đổi**: `winnerCount` cho split distribution phải đếm **tổng bet units** (= Σ hitCount × betCount), KHÔNG phải lines.

```typescript
// CalculateFinancials: khi tính split distribution
// winnerCountPerTier = Σ(hitCount × betCount) per tier — tổng đơn vị tham gia
// bonusPerUnit = pool / totalBetUnits — tiền bonus cho 1 đơn vị tham gia

// Khi patch entry:
// bonusAmount = bonusPerUnit × hitCount × betCount (của entry đó cho tier đó)
```

**VÍ DỤ Split tier3**:

- Entry A: 2 lines trúng tier3, betCount = 3 → 6 bet units
- Entry B: 1 line trúng tier3, betCount = 1 → 1 bet unit
- Total = 7 bet units
- bonusPerUnit = floor(pool_tier3 / 7)
- Entry A nhận: bonusPerUnit × 2 × 3 = bonusPerUnit × 6
- Entry B nhận: bonusPerUnit × 1 × 1 = bonusPerUnit × 1

**LƯU Ý**: Khi betCount = 1 cho tất cả, totalBetUnits = hitCount = hiện tại. Backward compat.

**Thay đổi CalculateFinancials**: Truyền `tierBetUnitCounts` thay vì `tierWinnerCounts` cho `calculateSplitDistribution`.

**Thay đổi `calculateSplitDistribution`** (`jackpot.ts`):

- Param `winnerCountPerTier` đổi ý nghĩa thành "bet unit count per tier"
- Hoặc thêm param mới `betUnitCountPerTier` và dùng nó để tính bonusPerWinner
...
**KHÔNG CẦN SỬA** vì `entry.payout.winAmount` đã bao gồm betCount (từ SettleEntries).
...
- ApplySplitBonuses: **winnerCount = tổng bet units** (Σ hitCount × betCount per tier)
- ApplySplitBonuses: bonusPerUnit = pool / totalBetUnits, bonus × betCount khi patch
- CalculateFinancials: thêm `tierBetUnitCounts` aggregate (Σ hitCount × betCount)
- calculateSplitDistribution: dùng betUnitCounts thay vì winnerCounts
...
Review tất cả aggregate queries — Lotto535 KHÔNG rename `betCount`, chỉ thêm `betUnitCount`:
...
**File**: `packages/game-lotto535-application/src/use-cases/operations/dto/live-entries.dto.ts`
...
**File**: `.cursor/rules/lotto535-game-rules.mdc`

Cập nhật:

- Section 1: thêm "betCount = số lần tham gia dự thưởng per board"
- Section 3: ghi rõ "giá vé = expandedLines × betCount × unitPrice"
- Section 9: cập nhật entity fields mới
- Section 11: thêm quy tắc betCount

### 6.2 Tests

**File**: `packages/game-lotto535-application/test/use-cases/`
...
- PatchJackpotPrize: winnerCount = lines (không nhân betCount), prize × betCount
- ApplySplitBonuses: bonus × betCount
...
| 3. Settle          | `settle-entries.ts`, `patch-jackpot-prize.ts`, `apply-split-bonuses.ts` | **CRITICAL** |
...
| Aspect               | Keno                                      | Lotto 5/35                                                                    |
| -------------------- | ----------------------------------------- | ----------------------------------------------------------------------------- |
| **Rename betCount**  | CẦN RENAME `betCount` → `selectionCount`  | KHÔNG cần rename (chưa có field cũ)                                           |
| Board structure      | `BasicBoard` (không có lines)             | `Board` + `BoardDerived.expandedLines` (có lines)                             |
| Side bets            | Có `SideBet.betCount`                     | Không có side bets                                                            |
| Line entity          | Không có                                  | CÓ `TicketLineDoc` → cần thêm `betCount`                                      |
| betUnitCount         | `Σ(board.betCount) + Σ(sideBet.betCount)` | `Σ(expandedLines × betCount)`                                                 |
| Settle nhân betCount | Per board/sideBet payout                  | Per LINE doc (mỗi line có betCount từ board)                                  |
| Payout caps          | Có (Keno bậc 8/9/10)                      | KHÔNG có                                                                      |
| Jackpot              | Không có                                  | CÓ — PatchJackpotPrize **chia theo tỷ lệ betCount** (không chia đều per line) |
| Split Cycle          | Không có                                  | CÓ — ApplySplitBonuses: winnerCount = tổng bet units, bonus × betCount        |
| buildPayoutTiers     | Từ matchResult × betCount                 | Từ lineDocs đã nhân betCount (multi-board)                                    |
