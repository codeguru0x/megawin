...
import { AUDIT_ACTIONS, AuditCategory, AuditActorType } from "@megawin/audit/entities";

// ... sau khi void thành công ...
AuditLogger.record({
  actorId: input.voidedBy ?? "system",
  actorType: input.voidedBy ? AuditActorType.Company : AuditActorType.System,
  actorName: input.voidedByName ?? "system",
  actorRoles: [],
  tenantId: "",
...
> Actor info (id/name/roles) cần được truyền xuống use-case từ API route
> (session). Các use-case hiện tại như `VoidDrawUseCase` đã có `voidedBy` —
> đợt tích hợp sẽ mở rộng input để nhận thêm actor context khi cần.

Action target tích hợp ban đầu (đợt 4): `void-draw`, `publish-result` cho 1-2
game làm chuẩn mực, sau đó nhân rộng.
...
| **2** | `AuditLogger` (record fire-and-forget + recordAndWait) + AuditEventInput | Logger dùng được |
| **3** | `ListAuditLogsUseCase` + `GetAuditLogUseCase` (NextApiUseCase) | Use-case query |
| **4** | Gọi thủ công trong 2-3 use-case mẫu (void, publish-result) | Pattern chuẩn mực |
