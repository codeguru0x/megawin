...
| p1-01-combo-transparency | P1 | ✅ done | ☐ chưa review | 02, 03 | Minh bạch chia jackpot cho player (analysis §3.10): endpoint `combo-popularity` ownership-gated + player-sdk `getComboPopularity` + `jackpotUnits` cho bộ 6 số standard. Unit test 7 case đã viết (`get-combo-popularity.test.ts`) — check-types + lint pass |
