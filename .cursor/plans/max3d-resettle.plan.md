...
### Scope
- ✅ Republish result (chỉ `numbers`) cho draw đang `Settled`.
- ✅ Update `vietlottRef` riêng (không đụng numbers).
...
| 4 | Tách 3 endpoint: `publish-result` (full), `republish-result` (chỉ numbers), `vietlott-ref` (chỉ ref) | Phân quyền & audit log rõ ràng. Initial publish vẫn giữ combined cho backward compat |
...
| 10 | `settledAt` là high-water mark | UI dùng để biết draw đã từng settled, gate resettle action |
...
```
[BO User]
   │ click "Republish Result"
   ▼
[BO API] PUT /api/max3d/draws/:drawId/republish-result
   │ body: { result: { special, first, second, third } }
   │ - check status === Settled
   │ - update draw.result, giữ status = Settled
   ▼
[BO User]
   │ click "Trigger Resettle"
   │ enter reason
   ▼
...
[SFN] Max3dResettleStateMachine
   ...
   │     - draw.status: Settled → Settling
   │     - clear financial, stats, settleSummary
...
**Thêm transition `Settled → Settling` để hỗ trợ resettle:**

```typescript
export const VALID_TRANSITIONS: Record<DrawStatus, Set<DrawStatus> | undefined> = {
  // ... existing ...
  [DrawStatus.Settled]: new Set([DrawStatus.Settling]), // ← NEW: cho resettle
};
```
...
#### 5.1.1 `republishResult(drawId, result)` — chỉ update `result`

```typescript
/**
 * Republish kết quả của draw đã settled.
 * 
 * CHỈ update `result.numbers` (special/first/second/third). KHÔNG đụng `vietlottRef`.
 * Idempotent: chạy lại nhiều lần với cùng input → cùng kết quả.
 * 
 * @returns updated draw hoặc null nếu draw không ở trạng thái Settled.
 */
async republishResult(
  drawId: string,
...
#### 5.1.2 `updateVietlottRef(drawId, vietlottRef)` — chỉ update `vietlottRef`

```typescript
/**
 * Update vietlottRef cho draw đã có result.
 * 
 * Chấp nhận draws ở status `Published`, `Settling`, hoặc `Settled`.
 * Idempotent.
 */
...
#### 5.1.3 `triggerResettle(drawId)` — `Settled → Settling`

```typescript
/**
 * Kích hoạt resettle: settled → settling.
 * 
 * Atomic, idempotent. Clear financial/stats/settleSummary để chuẩn bị tính lại.
...
> {
  ...
  protected async execute(input: ResetDrawForResettleInput): Promise<ResetDrawForResettleOutput> {
    ...
    const updated = await this.drawRepo.triggerResettle(drawId);
    if (!updated) {
      const draw = await this.drawRepo.getDrawById(drawId);
...
### 7.5 `republish-result.ts` (use-cases/draws/)

Wraps `DrawRepository.republishResult`. Throw `businessRuleViolation` nếu draw không ở `Settled`.

### 7.6 `update-vietlott-ref.ts` (use-cases/draws/)

Wraps `DrawRepository.updateVietlottRef`. Chấp nhận `Published/Settling/Settled`.

### 7.7 `trigger-resettle.ts` (use-cases/draws/)
...
export class TriggerResettleUseCase extends InternalUseCase<TriggerResettleInput, TriggerResettleOutput> {
  ...
  protected async execute(input: TriggerResettleInput): Promise<TriggerResettleOutput> {
    ...
    if (draw.status !== DrawStatus.Settled) {
      throw AppException.businessRuleViolation(
        `Draw ${drawId} status = "${draw.status}". Chỉ resettle được khi đã Settled.`,
      );
    }
...
### 7.8 Export barrel `use-cases/draws/index.ts`

```typescript
export * from "./republish-result";
export * from "./trigger-resettle";
export * from "./update-vietlott-ref";
```
...
### 10.3 `apps/backoffice/src/app/api/max3d/draws/[drawId]/_lib/schema.ts` (NEW)

Tách schema dùng chung cho 3 endpoint, theo pattern Bingo18:
...
### 10.4 Refactor `apps/backoffice/src/app/api/max3d/draws/[drawId]/publish-result/route.ts`

Import `publishResultSchema` từ `_lib/schema.ts`. Body và logic giữ nguyên.

### 10.5 `apps/backoffice/src/app/api/max3d/draws/[drawId]/republish-result/route.ts` (NEW)

```typescript
import { NextRequest, NextResponse } from "next/server";
import { handleApiError } from "@/lib/api/error-handler";
import { requireBackofficeAuth } from "@/lib/api/auth";
import { RepublishResultUseCase } from "@megawin/game-max3d-application";
...
export async function PUT(
  ...
  try {
    await requireBackofficeAuth(req, ["max3d:operations:republish"]);
    const { drawId } = await ctx.params;
    const body = await req.json();
    const parsed = republishResultSchema.parse(body);

    const result = await new RepublishResultUseCase().run({
      drawId,
...
### 10.6 `apps/backoffice/src/app/api/max3d/draws/[drawId]/vietlott-ref/route.ts` (NEW)

```typescript
import { NextRequest, NextResponse } from "next/server";
import { handleApiError } from "@/lib/api/error-handler";
import { requireBackofficeAuth } from "@/lib/api/auth";
import { UpdateVietlottRefUseCase } from "@megawin/game-max3d-application";
...
export async function PUT(
  ...
  try {
    await requireBackofficeAuth(req, ["max3d:operations:update-vietlott-ref"]);
    const { drawId } = await ctx.params;
    const body = await req.json();
    const parsed = updateVietlottRefSchema.parse(body);

    const result = await new UpdateVietlottRefUseCase().run({
      drawId,
...
### 10.7 `apps/backoffice/src/app/api/max3d/draws/[drawId]/resettle/route.ts` (NEW)
...
### 11.1 `draw-actions/republish-result-action.tsx` (NEW)

Modal:
- Pre-fill form từ `draw.result` (4 mảng triplets: special/first/second/third).
- Validate y hệt `republishResultSchema` (4 mảng đúng length).
- Chỉ enable khi `draw.status === Settled`.
- Submit → `PUT /api/max3d/draws/:drawId/republish-result`.
- Toast success/error → invalidate query draw detail.

### 11.2 `draw-actions/update-vietlott-ref-action.tsx` (NEW)

Modal:
- 2 fields: `drawPeriod` (text), `drawDate` (date picker, format YYYY-MM-DD).
- Pre-fill từ `draw.vietlottRef` nếu có.
- Enable khi `status ∈ {Published, Settling, Settled}`.
- Submit → `PUT /api/max3d/draws/:drawId/vietlott-ref`.

### 11.3 `draw-actions/trigger-resettle-action.tsx` (NEW)
...
- Chỉ enable khi `draw.status === Settled` AND user có permission `max3d:operations:resettle`.
- Submit → `POST /api/max3d/draws/:drawId/resettle`.
- Toast với `resettleId` để user track.

### 11.4 Update `draw-actions/publish-result-action.tsx`

- Combined publish: vẫn cho phép truyền `result` + `vietlottRef` cùng lúc trên initial publish (status `SalesClosed/Published`).
- Add note: "Đây là lần publish đầu tiên. Để đổi result sau khi đã settled, dùng Republish Result."

### 11.5 Update `draw-actions/index.ts`

```typescript
export * from "./publish-result-action";
export * from "./republish-result-action";    // NEW
export * from "./update-vietlott-ref-action"; // NEW
...
### 11.6 Update `draw-command-center.tsx`

Render conditionally theo `draw.status`:

| Status | Buttons |
|---|---|
| `SalesClosed` / `Published` (chưa Settled) | Publish Result, Trigger Settle |
| `Settling` / `Voiding` | (no actions, show progress) |
| `Settled` | Republish Result, Update Vietlott Ref, **Trigger Resettle** |
| `Voided` / `Cancelled` | (no actions) |

Hiển thị `settledAt` badge nếu có.

### 11.7 Update `_lib/use-operations.ts`

Thêm hooks SWR + mutation:
- `useRepublishResult(drawId)` — `useSWRMutation` PUT.
- `useUpdateVietlottRef(drawId)` — `useSWRMutation` PUT.
- `useTriggerResettle(drawId)` — `useSWRMutation` POST.

Mỗi mutation success → `mutate()` để re-fetch draw detail.

### 11.8 Update `_lib/sections/draw-management/index.tsx`

Mount các action modals trong `DrawCommandCenter`. Wire user permissions từ session.

### 11.9 Update `get-draw-selector.ts` use-case + DTO
...
| Event | Payload |
|---|---|
| `max3d.draw.republish-result` | `{ drawId, oldResult, newResult, actorId }` |
| `max3d.draw.update-vietlott-ref` | `{ drawId, oldRef, newRef, actorId }` |
...
- [ ] Add `Settled → Settling` to `VALID_TRANSITIONS`
- [ ] `DrawRepository.republishResult / updateVietlottRef / triggerResettle`
...
- [ ] `use-cases/resettle/reset-draw-for-resettle.ts`
- [ ] `use-cases/resettle/index.ts` (barrel)
- [ ] `use-cases/draws/republish-result.ts`
- [ ] `use-cases/draws/update-vietlott-ref.ts`
- [ ] `use-cases/draws/trigger-resettle.ts`
- [ ] Update `use-cases/draws/index.ts` barrel
- [ ] Update `use-cases/draws/dto/draw.dto.ts` (settledAt)
...
- [ ] `api/max3d/draws/[drawId]/_lib/schema.ts` (NEW)
- [ ] Refactor `api/max3d/draws/[drawId]/publish-result/route.ts` (use shared schema)
- [ ] `api/max3d/draws/[drawId]/republish-result/route.ts` (NEW)
- [ ] `api/max3d/draws/[drawId]/vietlott-ref/route.ts` (NEW)
- [ ] `api/max3d/draws/[drawId]/resettle/route.ts` (NEW)

### Phase 5: Backoffice UI
- [ ] `draw-actions/republish-result-action.tsx`
- [ ] `draw-actions/update-vietlott-ref-action.tsx`
- [ ] `draw-actions/trigger-resettle-action.tsx`
- [ ] Update `draw-actions/publish-result-action.tsx`
- [ ] Update `draw-actions/index.ts`
- [ ] Update `draw-command-center.tsx` (conditional buttons)
- [ ] Update `_lib/use-operations.ts` (3 mutation hooks)
- [ ] Update `_lib/sections/draw-management/index.tsx` (mount modals)

### Phase 6: Audit & Permissions
- [ ] Add permissions: `max3d:operations:republish`, `max3d:operations:update-vietlott-ref`, `max3d:operations:resettle`
- [ ] Audit log events (5 events)
...
`DrawDoc.settledAt` set lần đầu khi finalize-settle. Resettle KHÔNG cập nhật field này (dùng `version` để track resettle generations). UI dùng `settledAt != null && status = Settled` để gate Resettle action.
...
- ✅ **Vietlott split**: tách 3 endpoints (`publish-result`, `republish-result`, `vietlott-ref`); initial publish vẫn combined.
