/**
 * Lotto 5/35 – Financial Report Entities
 *
 * Per-game report collections cho Lotto 5/35.
 * 4 collections với prefix `lotto535_`:
 *   lotto535_settle_draw_reports      — 1 doc/draw đã settle
 *   lotto535_settle_tenant_reports    — 1 doc/tenant/draw đã settle
 *   lotto535_void_draw_reports        — 1 doc/draw đã void
 *   lotto535_outstanding_draw_reports — 1 doc/draw active (TTL 15 phút)
 *
 * Nguyên tắc:
...
/**
 * Báo cáo settle theo draw cho Lotto 5/35.
 * Unique index: { drawId: 1 }
 * Query index:  { financialDate: 1 }
 */
export const LOTTO535_SETTLE_DRAW_REPORTS = "lotto535_settle_draw_reports";

/**
 * Báo cáo settle theo tenant × draw cho Lotto 5/35.
 * Unique index: { drawId: 1, tenantId: 1 }
 * Query index:  { financialDate: 1, tenantId: 1 }
 */
export const LOTTO535_SETTLE_TENANT_REPORTS = "lotto535_settle_tenant_reports";

/**
 * Báo cáo void theo draw cho Lotto 5/35.
 * Unique index: { drawId: 1 }
 * Query index:  { financialDate: 1 }
 */
export const LOTTO535_VOID_DRAW_REPORTS = "lotto535_void_draw_reports";

/**
 * Snapshot outstanding draw cho Lotto 5/35. TTL auto-expire.
 * Unique index: { drawId: 1 }
 * TTL index:    { snapshotAt: 1 }, expireAfterSeconds: 300
 */
export const LOTTO535_OUTSTANDING_DRAW_REPORTS = "lotto535_outstanding_draw_reports";
...
export interface SettleDrawReport {
  ...
  /** Số tenant tham gia. */
  tenantCount: number;
  /** Số dòng (lines) — Lotto 5/35 có lines. */
  lineCount: number;
...
/**
 * MongoDB document type cho lotto535_settle_draw_reports.
...
/**
 * MongoDB document type cho lotto535_settle_tenant_reports.
 * Thêm `id` (hex string từ _id ObjectId) theo yêu cầu của BaseEntity.
 */
export type SettleTenantReportEntity = SettleTenantReport & { id: string };

/**
 * MongoDB document type cho lotto535_void_draw_reports.
 * Thêm `id` (hex string từ _id ObjectId) theo yêu cầu của BaseEntity.
 */
export type VoidDrawReportEntity = VoidDrawReport & { id: string };

/**
 * MongoDB document type cho lotto535_outstanding_draw_reports.
 * Thêm `id` (hex string từ _id ObjectId) theo yêu cầu của BaseEntity.
 */
