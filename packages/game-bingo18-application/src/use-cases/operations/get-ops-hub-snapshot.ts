 ...
 * `processingStuckSec`).
 *
 * TODO(follow-up): thêm các field này vào `OpsAlertsConfig`/`OpsConfig` per-game để staff
 * chỉnh qua backoffice UI, giống `largeBetAmount`/`exposureWarnRevenuePct`. Tới lúc đó dùng
...
/**
 * Snapshot vận hành ĐA KỲ — nguồn duy nhất cho trang Ops Hub Bingo18.
 *
 * Khác `GetOpsSnapshotUseCase` (1 kỳ, chi tiết sâu: heatmap, topAccounts…): use-case này
 * đọc NHIỀU kỳ nhưng MỎNG, chỉ đủ dựng 1 dòng bảng. Trang chi tiết 1 kỳ vẫn gọi
 * `GetOpsSnapshotUseCase` như cũ — không nhân bản logic.
 *
 ...
 * cho ra số KHÔNG khớp bảng người vận hành đang nhìn.
 *
 * Khác Keno: `exposureRaw` tính lúc build row qua `computeBingo18Exposure` (Bingo18 không
 * lưu exposure trên stats doc). Ngưỡng `exposureWarnPct` map từ `alerts.exposureWarnRevenuePct`.
 *
 * 2 tầng `Promise.all` (không phải 1): tầng 2 cần `drawIds` từ tầng 1. Không dùng `$lookup`
 * gộp thành 1 round-trip vì nó buộc materialize doc stats đầy đủ, mất lợi ích projection.
 */
export class GetOpsHubSnapshotUseCase extends UseCase<OpsHubSnapshotInput, OpsHubSnapshotOutput> {
  ...
  protected async execute(input: OpsHubSnapshotInput): Promise<OpsHubSnapshotOutput> {
    ...
    // biết `drawIds` cho tầng 2 (stats + alertCounts) — waterfall 2 tầng có chủ đích.
    ...
    // 0 kỳ chưa hoàn thành → khỏi gọi 2 query còn lại.
    ...
    // Doc cũ chưa có section ops → fallback default (giống GetOpsSnapshotUseCase).
    const ops = config.ops ?? DEFAULT_BINGO18_CONFIG.ops;
    const { alerts, stats: statsConfig } = ops;
    const { play } = config;
    ...
    return {
      rows: rows.map((row) => this.buildRow(row, statsByDrawId.get(row.drawId), alertCounts.get(row.drawId), prizes)),
      serverNow: new Date().toISOString(),
      // `>=` (không `===`) để an toàn nếu repo trả nhiều hơn limit vì lý do nào đó.
      truncated: rows.length >= limit,
      // Bingo18 dùng `exposureWarnRevenuePct` — map sang DTO field `exposureWarnPct` (FE mirror Keno).
      thresholds: this.buildThresholds(play.drawIntervalMinutes, alerts.largeBetAmount, alerts.exposureWarnRevenuePct),
   ...
   *
   * `exposureRaw` = `computeBingo18Exposure(...).worstCase.amount` — cùng prize shape với
   * `GetOpsSnapshotUseCase`. Kỳ chưa có / partial `byPlayType` → `normalizeByPlayType` bù
   * đủ 38 bucket (không chỉ `?? createEmpty` — object partial vẫn truthy).
   */
  private buildRow(
    draw: HubDrawRow,
    ...
    // Doc Mongo chỉ `$inc` nhánh đã cược → partial; normalize trước khi duyệt 216 outcome.
    const exposureRaw = stats
      ? computeBingo18Exposure(normalizeByPlayType(stats.byPlayType), prizes).worstCase.amount
      : 0;
  ...
  /**
   * Ngưỡng `AwaitingResult` tính động = 2×/4× `drawIntervalMinutes` (Bingo18 6 phút).
   * Field còn lại dùng default cứng cho tới khi `OpsConfig` có field tương ứng
   * (xem {@link DEFAULT_STAGE_THRESHOLDS}).
   */
