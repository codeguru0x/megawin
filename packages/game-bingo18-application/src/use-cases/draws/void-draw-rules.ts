 ...
 *
 * FE dùng hàm này để check "draw này có void được không" ở client — KHÔNG hardcode lại
 * danh sách status, tránh lệch khi domain đổi.
 *
 ...
 * module graph. Client Component cần gọi hàm này — bundler Next.js không thể resolve
 * `mongodb`/AWS SDK cho target browser → build lỗi. File này CHỈ import `DrawStatus`
 * (domain thuần, không I/O) nên an toàn bundle vào client.
 *
 * Hub Ops Bingo18 KHÔNG dùng void (đã bỏ khỏi bulk/single-row từ p1-07 bên Keno) — file
 * này vẫn tồn tại để giữ convention client-safe và phục vụ trang operations đơn kỳ.
 */
