/**
 * Use Case: Republish Result (Bingo 18) — bước 1 của workflow Resettle.
 *
 ...
 *
 * CHỈ nhận `numbers` (3 xúc xắc 1-6). Sửa `vietlottRef` (metadata tham chiếu
 * Vietlott) thuộc endpoint riêng `UpdateVietlottRefUseCase` — KHÔNG kéo theo
 * resettle.
 *
 * Validate input (numbers length + range) thực hiện ở route layer qua Zod
 * schema `republishResultSchema` — use-case không validate lại để tránh duplicate.
 *
 * Side effects:
...
> {
  ...
  protected async execute(input: RepublishResultInput): Promise<RepublishResultOutput> {
    ...
    // sum tính lại từ numbers mới — đồng nhất với PublishResultUseCase.
    ...
    if (!updated) {
      throw AppException.internal(`Republish kết quả kỳ ${input.drawId} thất bại..`);
    }
