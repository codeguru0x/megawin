 ...
 * jackpot rollover nên giá trị kỳ T không phụ thuộc kỳ T-1 — giải cố định hoàn
 * toàn, rollup daily re-aggregate toàn bộ theo financialDate (CAS trên `version`,
 * xem `SystemPublishSettleDailyUseCase`). Nhiều kỳ settle SONG SONG là hợp lệ và
 * cần thiết (~158 kỳ/ngày với `drawIntervalMinutes = 6` — tần suất cao nhất hệ
 * thống; 1 kỳ tắc có thể làm cả loạt kỳ sau đó không settle được — xem
 * `keno-bingo18-sequential-settle-guard.analysis.md`). Chống double-trigger vẫn
 * đủ 3 lớp: CAS status published→settling, deterministic SFN execution name, và
