 ...
 * batch, mỗi batch chạm hàng chục kỳ ở chặng `PendingClose`.
 *
 * Rẻ nhất trong 3 action Bingo18 Hub: `CloseSalesUseCase` chỉ 1 DB write có điều kiện status
 * (repo tự filter `SalesOpen`), KHÔNG đọc draw trước, KHÔNG start SFN. Kỳ không ở `SalesOpen`
 * sẽ nhận `DRAW_INVALID_TRANSITION` từ repo — đúng hành vi idempotent mong muốn.
 */
