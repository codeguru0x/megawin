/**
 * Use Case: Trigger Resettle (Bingo 18) — bước 2 của workflow Resettle.
 *
 ...
 *   5. Acquire business lock `bingo18:resettle:{drawId}` TTL 300s qua
 *      `DistributedMutex`.
...
> {
  ...
  protected async execute(input: TriggerResettleInput): Promise<TriggerResettleOutput> {
    ...
    const lockKey = buildResettleLockKey(GameProduct.Bingo18, drawId);
    ...
    try {
      ...
      // vì throw `ExecutionAlreadyExists` (AWS giữ name unique trong 90 ngày).
      //
