/**
 * Use Case: Prepare Bingo 18 Resettle (Step 1 của Resettle SFN).
 *
 * Trách nhiệm:
 ...
 *      Bingo18 KHÔNG có `hasCappablePrize`) để Settle SFN replay với kết quả mới.
 *
...
export interface PrepareResettleInput {
   ...
   * `buildResettleLockKey(GameProduct.Bingo18, drawId)` trong BO API.
   */
  lockKey: string;
}
...
> {
  ...
  protected async execute(input: PrepareResettleInput): Promise<PrepareResettleOutput> {
    ...
    // Bingo 18 KHÔNG có `hasCappablePrize` → repo method tự giới hạn
