/**
 * Use Case: Enqueue Bingo 18 Reversal Orders (Step 2 của Resettle SFN).
 *
...
export class EnqueueReversalsUseCase extends InternalUseCase<EnqueueReversalsInput, EnqueueReversalsOutput> {
  ...
  protected async execute(input: EnqueueReversalsInput): Promise<EnqueueReversalsOutput> {
    ...
    const reversalBatchKey = buildResettleBatchKey(GameProduct.Bingo18, drawId, resettleId, "reversal");
    ...
    while (true) {
      ...
      const orders = entries.map((e) =>
        buildReversalOrder({
          ...
          roundIds: [drawId],
          description: `Thu hồi Bingo 18 kỳ ${drawId} (resettle, vé ${e.ticketNo})`,
