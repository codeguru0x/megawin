/**
 * Cache module: Tenant Config (Bingo18).
 *
...
import { BINGO18_CACHE_KEYS } from "./keys";
...
const fetcher = createCachedFetcher<string, TenantConfigEntity | null>(
  (tenantId) => getRepo().getTenantConfig(tenantId),
  {
    ...
    keyPrefix: BINGO18_CACHE_KEYS.tenantConfig,
    ttlSec: 60 * 10, // 10 minutes
  },
);

/**
 * Cache read-through cho tenant config Bingo18 (per tenantId).
 *
