/**
 * Power 6/55 – Draw Combo Stats (chi tiết combo theo BOARD người chơi chọn)
 *
 * Collection: power655_draw_combo_stats — 1 document / (draw × combo).
 *
 ...
 *
 * Keno combo = bộ số người chơi chọn cho basic play (pick numbers). Power 6/55
 * combo = BOARD (`playType:sortedMainNumbers`) — vé Bao 18 (18 số đã chọn) = 1
 ...
 * Chi tiết per-account tách sang {@link Power655DrawComboAccountDoc}. `accountCount`
 * là counter vô hướng → index được cho rule `combo_concentration`
 * (`{drawId, accountCount: {$gte: n}}`), copy pattern Keno.
 */
 ...
 *
 * Chi tiết từng account nằm ở {@link Power655DrawComboAccountDoc} (1 doc/account) —
 ...
 */
export interface Power655DrawComboStatsDoc extends DeltaAccumulatedDoc {
  /** MongoDB ObjectId. */
  _id: unknown;
  /** drawId dạng `YYYY-MM-DD.NNN`. */
  drawId: string;
  /**
   * Khoá combo theo BOARD: `${playType}:${sortedMainNumbers.join(",")}`.
   * Unique cùng `drawId`. VD: `"bao7:01,05,12,23,34,45,55"`.
   */
  comboKey: string;
  /**
   ...
   * combo standard, xem index `{drawId, playType, mainNumbers}`).
   */
  playType: PlayType;
  /**
   * Bộ số đã sort (zero-padded `"01".."55"`) — hiển thị trực tiếp, không parse
   ...
   * {@link Power655DrawComboAccountDoc}.
   */
  accountCount: number;
  createdAt: Date;
  updatedAt: Date;
...
export interface Power655DrawComboStatsEntity extends Omit<Power655DrawComboStatsDoc, "_id"> {
 ...
 *
 * Collection: power655_draw_combo_accounts. Tách khỏi {@link Power655DrawComboStatsDoc}
 ...
 */
export interface Power655DrawComboAccountDoc extends DeltaAccumulatedDoc {
  /** MongoDB ObjectId. */
  _id: unknown;
  /** drawId dạng `YYYY-MM-DD.NNN`. */
  drawId: string;
  /** Khoá combo — cùng convention `Power655DrawComboStatsDoc.comboKey`. */
  comboKey: string;
...
/** Application-layer entity sau khi qua mapper. ObjectId → id string. */
export interface Power655DrawComboAccountEntity extends Omit<Power655DrawComboAccountDoc, "_id"> {
  /** MongoDB ObjectId đã chuyển sang hex string. */
  id: string;
}
