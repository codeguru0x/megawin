/**
 * Power 6/55 – Financial Report Entities
 *
 * Per-game report collections cho Power 6/55.
 * 4 collections với prefix `power655_`:
 *   power655_settle_draw_reports      — 1 doc/draw đã settle
 *   power655_settle_tenant_reports    — 1 doc/tenant/draw đã settle
 *   power655_void_draw_reports        — 1 doc/draw đã void
 *   power655_outstanding_draw_reports — 1 doc/draw active (TTL 15 phút)
 *
 * Nguyên tắc:
 ...
 *
 * Đặc thù Power 6/55:
 *   - CÓ DUAL Jackpot (JP1 + JP2): jackpotContribution = JP1 + JP2 contribution tổng
 *   - CÓ lineCount (expandedLines per board)
 *   - companyTake = actualCompanyTake từ SettleFinancials
 */
...
/**
 * Báo cáo settle theo draw cho Power 6/55.
 * Unique index: { drawId: 1 }
 * Query index:  { financialDate: 1 }
 */
export const POWER655_SETTLE_DRAW_REPORTS = "power655_settle_draw_reports";

/**
 * Báo cáo settle theo tenant × draw cho Power 6/55.
 * Unique index: { drawId: 1, tenantId: 1 }
 * Query index:  { financialDate: 1, tenantId: 1 }
 */
export const POWER655_SETTLE_TENANT_REPORTS = "power655_settle_tenant_reports";

/**
 * Báo cáo void theo draw cho Power 6/55.
 * Unique index: { drawId: 1 }
 * Query index:  { financialDate: 1 }
 */
export const POWER655_VOID_DRAW_REPORTS = "power655_void_draw_reports";

/**
 * Snapshot outstanding draw cho Power 6/55. TTL auto-expire.
 * Unique index: { drawId: 1 }
 * TTL index:    { snapshotAt: 1 }, expireAfterSeconds: 300
 */
export const POWER655_OUTSTANDING_DRAW_REPORTS = "power655_outstanding_draw_reports";
 ...
 *   jackpotContribution = jackpot1Contribution + jackpot2Contribution (tổng cả 2 pool)
 */
export interface SettleDrawReport {
  ...
  /** Số tenant tham gia. */
  tenantCount: number;
  /** Số dòng (lines) — Power 6/55 có lines (expandedLines per board). */
  lineCount: number;
   ...
   * Power 6/55 DUAL Jackpot: = jackpot1Contribution + jackpot2Contribution.
   */
...
export interface OutstandingDrawReport {
  ...
  /** Số tenant tham gia. */
  tenantCount: number;
  /** Số dòng (lines) pending — Power 6/55 có lineCount. */
  lineCount: number;
...
/**
 * MongoDB document type cho power655_settle_draw_reports.
...
/**
 * MongoDB document type cho power655_settle_tenant_reports.
 * Thêm `id` (hex string từ _id ObjectId) theo yêu cầu của BaseEntity.
 */
export type SettleTenantReportEntity = SettleTenantReport & { id: string };

/**
 * MongoDB document type cho power655_void_draw_reports.
 * Thêm `id` (hex string từ _id ObjectId) theo yêu cầu của BaseEntity.
 */
export type VoidDrawReportEntity = VoidDrawReport & { id: string };

/**
 * MongoDB document type cho power655_outstanding_draw_reports.
 * Thêm `id` (hex string từ _id ObjectId) theo yêu cầu của BaseEntity.
 */
