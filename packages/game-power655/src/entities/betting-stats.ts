/**
 * Power 6/55 – Draw Betting Stats (pre-aggregated realtime stats)
 *
 * Collection: power655_draw_betting_stats — 1 document / draw.
 *
 ...
 * place-bet.
 *
 * Port từ Keno (`packages/game-keno/src/entities/betting-stats.ts`) — KHÁC Keno ở:
 * KHÔNG có `numberFreq` (tách riêng {@link Power655DrawNumberStatsDoc} — quyết định
 * user 05/08, xem `number-stats.ts`), exposure 2 phần (fixed + jackpot, xem
 * {@link Power655Exposure}), `byPlayType` 12 key cố định thay side-bet.
 *
 * Xem `.cursor/analysis/power655-operations-risk-control.analysis.md` §3.4, §3.6.
 */
...
export interface Power655PlayTypeStat {
...
/**
 * Exposure — cấu trúc KHÁC KENO CĂN BẢN (game jackpot, không có bảng maxPrize/cap).
 *
 * Chỉ lưu phần FIXED (cộng dồn `$inc`, an toàn). Phần JACKPOT KHÔNG lưu ở đây —
 * đọc snapshot pool (`DrawJackpot`/jackpot cycle hiện hành) lúc build response/eval
 * alert, vì jackpot bị chặn bởi pool (không nhân số vé như giải cố định). Xem
 * analysis §3.6.
 */
export interface Power655Exposure {
  /**
   ...
   * (`ops.alerts.fixedExposureWarnAmount`) vì Power 6/55 không có `maxPerDraw` để
   * tính %.
   */
...
export interface Power655TopPotential {
  /** ID entry (hex string). */
  entryId: string;
...
/**
 * Thống kê realtime 1 kỳ Power 6/55 — 1 document / draw, worker cập nhật async.
 *
 * Extends `DrawBettingStatsBase` (trừ `lastEntryId` — siết type) — thêm field đặc thù:
 * `byPlayType` (12 kiểu chơi cố định), `exposure` (2 phần: fixed nhúng + jackpot đọc
 * lúc build response), `topPotential`.
 *
 * ## KHÔNG có `numberFreq`, `topAccounts`, `topCombos` trong doc
 *
 * - `numberFreq` tách riêng {@link Power655DrawNumberStatsDoc} ngay từ đầu (KHÁC
 *   Keno — Keno nhúng 80 key trong doc). Quyết định user 05/08: chừa đường thêm
 *   chỉ số unbounded per số trong tương lai mà không refactor stats doc (analysis §3.3).
 * - `topAccounts`/`topCombos` là top-K theo metric TÍCH LUỸ — derive lúc đọc từ
 *   `power655_draw_account_stats`/`power655_draw_combo_stats` (`sort().limit(K)`),
...
export interface Power655DrawBettingStatsDoc extends Omit<DrawBettingStatsBase, "lastEntryId">, DeltaAccumulatedDoc {
  ...
  byPlayType: Record<PlayType, Power655PlayTypeStat>;

  /** Exposure 2 phần: fixed nhúng ở đây, jackpot đọc lúc build response (analysis §3.6). */
   ...
   */
  topPotential: Power655TopPotential[];
}

/** Application-layer entity sau khi qua mapper. ObjectId → id string. */
export interface Power655DrawBettingStatsEntity extends Omit<Power655DrawBettingStatsDoc, "_id"> {
