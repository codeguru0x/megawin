...
/**
 * Mega 6/45 audit-log helpers — nhóm free functions ghi audit log cho domain
 * Mega 6/45.
 *
 * Đóng băng sẵn `game`/`category`/`targetType`/`action` của Mega 6/45 → use-case
 * chỉ truyền payload nghiệp vụ tối thiểu (actor + id + diff).
 *
 * Phân tầng: `@megawin/audit/logger` cung cấp `record()` (low-level, generic).
 * Module này là tầng high-level riêng Mega 6/45 — là **domain service** ("ghi lại
 * chuyện gì đã xảy ra trong domain Mega 6/45"), nên đặt trong `services/` cùng các
 * domain service khác. Phụ thuộc domain type Mega 6/45 (status, winningNumbers…) nên
...
const GAME = GameProduct.Mega645;

type PublishResultArgs = {
  actor: AuditActor;
  drawId: string;
  /** 6 số chính trúng thưởng (zero-padded `"01"`–`"45"`). */
 ...
 *
 * Bộ số trúng (winningNumbers) là NỘI DUNG của thay đổi (before: chưa có kết quả
 * → after: có kết quả) nên ghi vào `changes`, KHÔNG phải `metadata.extra`.
 *
...
function recordPublishResult(
  ...
  record({
    ...
    targetLabel: `Kỳ ${args.drawId}`,
    changes: {
      after: { winningNumbers: args.winningNumbers },
    },
    metadata: {
      extra: dropUndefined({
...
/**
 * Audit công bố kết quả kỳ quay Mega 6/45 (publish lần đầu / sửa trước settle).
 *
 ...
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
 * @param args.winningNumbers - 6 số chính (zero-padded `"01"`–`"45"`).
...
/**
 * Audit công bố LẠI kết quả kỳ quay Mega 6/45 — sửa kết quả sau khi kỳ đã settle
...
/**
 * Audit staff BẤM NÚT kết sổ kỳ quay Mega 6/45 (settle lần đầu) từ BO.
 *
...
/**
 * Audit staff BẤM NÚT kết sổ LẠI kỳ quay Mega 6/45 (resettle) từ BO — sau khi
 ...
 *
 * Riêng game có jackpot (Mega 6/45). Cascade B2 sửa kết quả kỳ T kéo theo các
 * kỳ T+1…T+n đã settle phải re-settle vì pool jackpot đổi — số quay KHÔNG đổi.
 * Reopen re-stamp `publishedAt`, chuyển `settled → published` để mở cổng
...
/**
 * Audit staff mở bán kỳ quay Mega 6/45 từ BO (`scheduled|sales_closed → sales_open`).
 *
 * @param args.actor - Chủ thể thực hiện.
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
...
/**
 * Audit staff đóng bán kỳ quay Mega 6/45 từ BO (`sales_open → sales_closed`).
 *
 * @param args.actor - Chủ thể thực hiện.
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
...
/**
 * Audit staff cập nhật lịch bán/quay của kỳ quay Mega 6/45 từ BO.
 *
...
/**
 * Audit cập nhật cấu hình game Mega 6/45 toàn cục.
 *
...
/**
 * Audit cập nhật cấu hình Mega 6/45 riêng của 1 tenant (upsert).
 *
