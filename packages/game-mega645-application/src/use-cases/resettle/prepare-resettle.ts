/**
 * Use Case: Prepare Mega 6/45 Resettle (Step 1 của Resettle SFN).
 *
 * Trách nhiệm:
 ...
 *      Mega 6/45 KHÔNG có `hasCappablePrize`) để Settle SFN replay với kết quả mới.
 *
...
import type { EntryReversal } from "@megawin/game-mega645/entities";
...
export interface PrepareResettleInput {
  ...
  /** Lock key (`mega645:resettle:{drawId}`) — propagate để `FinalizeSettle` release. */
  lockKey: string;
}
 ...
 * `StartSettleExecution`) thực sự đọc: `drawId`, `resettleId`, `lockOwnerToken`,
 * `lockKey`. Metric như `reversalCount` / `resetCount` được log qua CloudWatch,
 * KHÔNG đưa vào output để tránh pollute SFN state.
 */
...
export class PrepareResettleUseCase extends InternalUseCase<PrepareResettleInput, PrepareResettleOutput> {
  ...
  protected async execute(input: PrepareResettleInput): Promise<PrepareResettleOutput> {
    ...
    // Mega 6/45 KHÔNG có `hasCappablePrize` → repo method $unset đúng fields.
    const resetCount = await this.entryResettleRepo.resetEntriesForResettle(drawId);

    console.info("[PrepareResettle Mega645] done", {
      drawId,
      resettleId,
      reversalCount,
      resetCount,
