/**
 * Use Case: Enqueue Mega 6/45 Reversal Orders (Step 2 của Resettle SFN).
 *
...
export interface EnqueueReversalsInput {
  ...
  /** Lock key propagate xuôi SFN tới `FinalizeSettle` (`mega645:resettle:{drawId}`). */
  lockKey: string;
}
...
export class EnqueueReversalsUseCase extends UseCase<EnqueueReversalsInput, EnqueueReversalsOutput> {
  ...
  protected async execute(input: EnqueueReversalsInput): Promise<EnqueueReversalsOutput> {
    ...
    while (true) {
      ...
      const orders = entries.map((e) =>
        buildReversalOrder({
          ...
          roundIds: [drawId],
          description: `Thu hồi Mega 6/45 kỳ ${drawId} (resettle, vé ${e.ticketNo})`,
    ...
    console.info("[EnqueueReversals Mega645] done", { drawId, resettleId, enqueuedTotal });
