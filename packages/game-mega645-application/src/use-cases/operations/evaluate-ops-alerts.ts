...
export class EvaluateOpsAlertsUseCase extends TickLoopWorker<void, EvaluateOpsAlertsResult> {
  ...
  protected async beforeLoop(): Promise<void> {
    ...
    this.alertCtx = {
      ...
      alerts: config.ops?.alerts ?? DEFAULT_MEGA645_CONFIG.ops.alerts,
  ...
  protected async runTick(): Promise<TickOutcome> {
    ...
    for (const stats of docs) {
      ...
      try {
        ...
        logError("mega645:ops-alerts", error, { drawId: stats.drawId });
  ...
  private async evaluateDoc(stats: Mega645DrawBettingStatsEntity): Promise<void> {
    const combos = this.alertCtx.alerts.enabled[Mega645OpsAlertType.ComboConcentration]
