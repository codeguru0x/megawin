 ...
 *      là trạng thái cuối, không được huỷ (Mega 6/45 không có resettle).
...
export class VoidDrawUseCase extends NextApiUseCase<VoidDrawInput, VoidDrawOutput> {
  ...
  protected async execute(input: VoidDrawInput): Promise<VoidDrawOutput> {
    ...
    if (!alreadyVoiding) {
      // Kỳ đã từng kết sổ (settledAt là high-water mark) → CẤM void. Mega 6/45
      ...
      if (!updated) {
        throw AppException.internal("Huỷ kỳ quay thất bại – vui lòng thử lại sau.");
      }
