/**
 * Mega 6/45 – Entry Resettle Repository
 *
 * Collection: mega645_ticket_entries (chỉ entries trong phiên resettle)
 *
...
export class EntryResettleRepository extends BaseRepo<TicketEntryEntity, EntryMapper> {
   ...
   *
   * Fields bị $unset (xoá hoàn toàn data settle cũ — Mega 6/45):
   * - `payout` — winAmount, payoutAmount, tiers, payoutTx, settledAt.
   * - `outcome` — "win" / "loss".
   * - `result` — snapshot kết quả quay (winningNumbers, publishedAt).
   *
   * Mega 6/45 KHÔNG có `hasCappablePrize` → chỉ unset 3 field trên.
   *
  ...
  async resetEntriesForResettle(drawId: string): Promise<number> {
    ...
    const result = await this.updateMany(
      { drawId, status: EntryStatus.Settled },
      {
        ...
        // Xoá các field được set trong bulkSettleEntries (Mega645 không có hasCappablePrize)
        $unset: {
          payout: "",
          outcome: "",
          result: "",
