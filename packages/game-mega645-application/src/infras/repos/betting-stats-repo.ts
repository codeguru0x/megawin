/**
 * Mega 6/45 – Draw Betting Stats Repository
 *
 * Collection: mega645_draw_betting_stats — 1 doc/draw, pre-aggregated ops stats.
 *
 ...
 * 6/55: `exposure` cũng chỉ 1 field `fixedWorstCase` (Mega 6/45 chỉ 1 Jackpot đơn, càng
 * đơn giản hơn nữa).
 *
...
export class BettingStatsRepository extends BaseRepo<Mega645DrawBettingStatsEntity, BettingStatsMapper> {
  ...
  async findByDrawId(drawId: string): Promise<Mega645DrawBettingStatsEntity | null> {
  ...
  async applyDelta(drawId: string, delta: DrawStatsDelta, batchMaxId: string, topPotentialK: number): Promise<boolean> {
    ...
    if (delta.topPotential.length > 0) {
      update.$push = {
        [f("topPotential")]: {
          $each: delta.topPotential satisfies Mega645TopPotential[],
   ...
   */
  async findChangedSince(since: Date, limit: number): Promise<Mega645DrawBettingStatsEntity[]> {
   ...
   *
   * KHÔNG có `resetFinal` đối xứng — kiến trúc `$inc` + watermark khiến reset không tính
   * lại được gì, mà có rủi ro cộng đôi nếu sửa sai.
   */
