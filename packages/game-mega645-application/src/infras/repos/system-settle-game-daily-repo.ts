/**
 * System Settle Game Daily Repository — Mega 6/45
 *
 * Kế thừa base SystemSettleGameDailyRepository từ game-core.
 * Thêm perGameColl trỏ vào mega645_settle_draw_reports.
 *
 * aggregateDrawsFromPerGame():
 *   Query mega645_settle_draw_reports WHERE { financialDate }
  ...
  async aggregateDrawsFromPerGame(financialDate: string): Promise<SettleGameDailyAggregateResult> {
    ...
    if (result.length === 0) {
      return {
        ...
        totalCommission: 0,
        netProfit: 0,
      } as SettleGameDailyAggregateResult;
    }
