/**
 * Mega 6/45 – Draw Account Stats Repository
 *
 * Collection: mega645_draw_account_stats — 1 doc/(draw × account).
 *
 * ĐỌC:
 ...
 *
 * Port nguyên kiến trúc từ Power 6/55 (`account-stats-repo.ts`) — vì sao không nuôi mảng
 * `topAccounts` trong stats doc: metric tích luỹ không seed lại chính xác được.
 *
 * ## Retention
 *
...
import {
  Mega645Collections,
  type Mega645DrawAccountStatsDoc,
  type Mega645DrawAccountStatsEntity,
...
export class AccountStatsRepository extends BaseRepo<Mega645DrawAccountStatsEntity, AccountStatsMapper> {
   ...
   */
  async findTopByAmount(drawId: string, k: number): Promise<Mega645DrawAccountStatsEntity[]> {
