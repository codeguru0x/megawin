/**
 * Max 3D Pro – Financial Report Entities
 *
 * Per-game report collections cho Max 3D Pro.
 * 4 collections với prefix `max3dpro_`:
 *   max3dpro_settle_draw_reports      — 1 doc/draw đã settle
 *   max3dpro_settle_tenant_reports    — 1 doc/tenant/draw đã settle
 *   max3dpro_void_draw_reports        — 1 doc/draw đã void
 *   max3dpro_outstanding_draw_reports — 1 doc/draw active (TTL 15 phút)
 *
 * Nguyên tắc:
 ...
 *   - Max 3D Pro KHÔNG CÓ Jackpot: KHÔNG có field jackpotContribution.
 *   - Max 3D Pro CÓ lineCount (pairs per board — multiNumber hoặc multiDigit).
 */
...
/**
 * Báo cáo settle theo draw cho Max 3D Pro.
 * Unique index: { drawId: 1 }
 * Query index:  { financialDate: 1 }
 */
export const MAX3DPRO_SETTLE_DRAW_REPORTS = "max3dpro_settle_draw_reports";

/**
 * Báo cáo settle theo tenant × draw cho Max 3D Pro.
 * Unique index: { drawId: 1, tenantId: 1 }
 * Query index:  { financialDate: 1, tenantId: 1 }
 */
export const MAX3DPRO_SETTLE_TENANT_REPORTS = "max3dpro_settle_tenant_reports";

/**
 * Báo cáo void theo draw cho Max 3D Pro.
 * Unique index: { drawId: 1 }
 * Query index:  { financialDate: 1 }
 */
export const MAX3DPRO_VOID_DRAW_REPORTS = "max3dpro_void_draw_reports";

/**
 * Snapshot outstanding draw cho Max 3D Pro. TTL auto-expire.
 * Unique index: { drawId: 1 }
 * TTL index:    { snapshotAt: 1 }, expireAfterSeconds: 300
 */
export const MAX3DPRO_OUTSTANDING_DRAW_REPORTS = "max3dpro_outstanding_draw_reports";
...
export interface SettleDrawReport {
  ...
  /** Số tenant tham gia. */
  tenantCount: number;
  /** Số pairs đã settle. Max 3D Pro CÓ lineCount. */
  lineCount: number;
...
/**
 * MongoDB document type cho max3dpro_settle_draw_reports.
...
/**
 * MongoDB document type cho max3dpro_settle_tenant_reports.
 * Thêm `id` (hex string từ _id ObjectId) theo yêu cầu của BaseEntity.
 */
export type SettleTenantReportEntity = SettleTenantReport & { id: string };

/**
 * MongoDB document type cho max3dpro_void_draw_reports.
 * Thêm `id` (hex string từ _id ObjectId) theo yêu cầu của BaseEntity.
 */
export type VoidDrawReportEntity = VoidDrawReport & { id: string };

/**
 * MongoDB document type cho max3dpro_outstanding_draw_reports.
 * Thêm `id` (hex string từ _id ObjectId) theo yêu cầu của BaseEntity.
 */
