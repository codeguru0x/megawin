/**
 * Mutex cho **business-level distributed lock** xuyên qua nhiều process.
 *
 * Khác với {@link SingleRunWorker} (single-invocation: acquire → run → release
 * trong cùng 1 Lambda), `DistributedMutex` phục vụ pattern **cross-process
 * lock** điển hình:
 *
 ...
 *
 * ## `kind` doc — luôn `"business"`, không cấu hình được
 *
 * `acquire()` hardcode `kind: WorkerLockKind.Business` khi gọi `tryAcquire` — mọi lock qua
 * mutex này tự động thuộc `kind: "business"`, không có param để đổi. Trang BO Workers
 * health CHỈ đọc `kind: "worker"` (`ListWorkersHealthUseCase.listByKind`) nên lock tạo ở đây
 * KHÔNG hiện trên trang đó — xem bảng so sánh 2 kind ở JSDoc `WorkerLockKind`
 * (`../../entities/worker-lock.ts`). Doc `kind: "business"` cũng KHÔNG vĩnh viễn — có TTL index
 * riêng trên `acquiredAt` (`../../indexes/index.ts`, `partialFilterExpression: { kind: "business" }`).
 *
 * ## Vì sao là 1 abstraction riêng?
 *
 * - `SingleRunWorker` ép semantic "skip-if-locked" + cursor + kill-switch
 ...
 * - Mutex centralize: business layer chỉ thấy `acquire`/`release` semantic,
 ...
 *   các bước sau (transition status, start SFN) thất bại — tránh user phải đợi
 *   đủ TTL retry.
 *
 ...
 * } catch (err) {
 *   await mutex.releaseOnRollback(lockKey, ownerToken, err);
 *   throw err;
 * }
 * ```
 *
...
export interface ReleaseBusinessLockOptions {
  ...
  /**
   * Nếu business operation thất bại, truyền error vào — mutex sẽ ghi
...
/**
 * Mutex cho cross-process business lock. Stateless — có thể tái sử dụng
