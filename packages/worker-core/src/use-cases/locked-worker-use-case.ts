/**
 * Base use case cho worker cần distributed lock qua MongoDB.
 *
 ...
 * - Skip (`locked`/`disabled`) KHÔNG throw — return `LockedWorkerSkipped`.
 ...
 * @example
 * ```ts
 * class ProcessMainUseCase extends LockedWorkerUseCase<Input, Output> {
...
export abstract class LockedWorkerUseCase<I, O> extends InternalUseCase<I, LockedWorkerResult<O>> {
  ...
  protected async execute(input: I): Promise<LockedWorkerResult<O>> {
    ...
    // Ghi lastSuccessAt / lastError + clear ownerToken vào cùng 1 $set.
