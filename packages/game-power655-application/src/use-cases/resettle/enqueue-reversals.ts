/**
 * Use Case: Enqueue Power 6/55 Reversal Orders (Step 2 của Resettle SFN).
 *
 ...
 *   2. Build `TenantDispatchOrderInput` qua `buildReversalOrder`.
...
export interface EnqueueReversalsInput {
  ...
  /** Lock key propagate xuôi SFN tới `FinalizeSettle` (`power655:resettle:{drawId}`). */
  lockKey: string;
}
...
export class EnqueueReversalsUseCase extends UseCase<EnqueueReversalsInput, EnqueueReversalsOutput> {
  ...
  protected async execute(input: EnqueueReversalsInput): Promise<EnqueueReversalsOutput> {
    ...
    const reversalBatchKey = buildResettleBatchKey(GameProduct.Power655, drawId, resettleId, "reversal");
    ...
    while (true) {
      ...
      const orders = entries.map((e) =>
        buildReversalOrder({
          ...
          roundIds: [drawId],
          description: `Thu hồi Power 6/55 kỳ ${drawId} (resettle, vé ${e.ticketNo})`,
    ...
    console.info("[EnqueueReversals Power655] done", { drawId, resettleId, enqueuedTotal });
