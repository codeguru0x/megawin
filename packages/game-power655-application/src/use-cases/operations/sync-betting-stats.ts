/**
 * Power 6/55 – Sync Betting Stats Worker
 *
 * Cập nhật `power655_draw_betting_stats` (+ number/account/combo stats) async cho mọi kỳ
 * chưa `final` — KHÔNG đụng hot path place-bet (analysis §3.1–3.3). Extends
 * {@link TickLoopWorker}: 1 worker toàn hệ (`power655:stats-sync`), vòng lặp/budget/cadence
 * nằm ở base class (packages/worker-core) — xem JSDoc `TickLoopWorker` cho cơ chế loop.
 *
 * Port nguyên kiến trúc Keno (`sync-betting-stats.ts`, xem JSDoc gốc cho lý giải đầy đủ
 * "hàng đợi việc theo trạng thái công việc, không theo status draw" + "vì sao ghi theo
 * batch, không gom cả tick vào RAM"). KHÁC Keno:
 *
 * - Ghi thêm `power655_draw_number_stats` (tách riêng — Power655 KHÔNG nhúng numberFreq
 *   trong stats doc như Keno, xem `Power655DrawNumberStatsDoc`).
 * - `PrizeContext` chỉ cần `{ unitPrice, tier1, largeBetAmount }` (không có bảng
 *   basic/bigSmall/evenOdd của Keno — Power655 không có side bet).
 * - `MAX_ENTRIES_PER_DRAW_PER_TICK`/`MAX_DRAWS_PER_TICK` GIỮ NGUYÊN hằng số Keno (Power
 *   6/55 thường chỉ 1 kỳ active — giữ để đồng nhất codebase, không có ý nghĩa scale riêng).
 *
 * ## Mỗi invocation
 *
 ...
 * `power655:ops-alerts`, tách theo analysis §5.1: lỗi rule alert không làm chậm sync,
 ...
 *
 * GIỮ NGUYÊN giá trị Keno (analysis: Power 6/55 thường chỉ 1 kỳ active/ngày, trần này
...
/**
 * Trần số kỳ xử lý trong 1 tick — GIỮ NGUYÊN giá trị Keno (xem JSDoc `MAX_ENTRIES_PER_DRAW_PER_TICK`).
 *
...
export class SyncBettingStatsUseCase extends TickLoopWorker<void, SyncBettingStatsResult> {
  ...
  protected async beforeLoop(): Promise<void> {
    ...
    // `DEFAULT_POWER655_CONFIG.ops` khi thiếu, tránh crash `beforeLoop` (R2 p0-01, R7 p0-02).
    this.statsConfig = config.ops?.stats ?? DEFAULT_POWER655_CONFIG.ops.stats;
    this.counters = { entriesApplied: 0, finalized: 0, failed: 0 }; // reset — container reuse

    // Enroll 1 lần/invocation (đồng bộ lý luận Keno): draws được staff tạo batch cho cả
    // ngày; kỳ tạo GIỮA invocation chờ tối đa ~55s để vào hàng đợi — vô nghĩa so với chu kỳ
    // draw (1 kỳ/ngày). Nhờ vậy `runTick` còn đúng 1 câu chuyện: lấy hàng đợi → hút delta →
    // đóng dấu.
  ...
  private async syncDraw(
    ...
    while (true) {
      ...
      const acc = new Power655StatsAccumulator(drawId, prize);
      ...
      if (!ok) {
        throw new LockTakenOverError("power655:stats-sync");
      }
   ...
   * `power655_draw_combo_accounts` rồi `$set` tuyệt đối, nên tự hội tụ sau mọi crash (khác
   * `$inc` theo "account mới trong tick" — mất là mất vĩnh viễn).
   */
  private async writeBatch(
    drawId: string,
    acc: Power655StatsAccumulator,
    batchMaxId: string,
    stats: OpsStatsConfig,
  ): Promise<void> {
