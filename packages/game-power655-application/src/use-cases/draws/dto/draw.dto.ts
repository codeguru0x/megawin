...
import type { DrawEntity, DrawNo } from "@megawin/game-power655/entities";
...
export interface GetVietlottResultOutput {
  ...
  found: boolean;
  /** Dàn số trúng thưởng dạng flat (7 phần tử, phần tử cuối = bonus) — mapping sang field form do frontend tự làm. `null` khi `found = false`. */
  numbers: string[] | null;
...
export interface GetDrawDetailOutput {
  /** Entity đầy đủ của kỳ quay Power 6/55 — Date fields đã serialize thành ISO string qua JSON response. */
