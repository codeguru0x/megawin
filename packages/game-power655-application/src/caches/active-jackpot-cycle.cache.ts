/**
 * Cache module: Active Jackpot Cycle (Power 6/55) — CHỈ cho read path HIỂN THỊ.
 *
 ...
 *
 * Policy: read-through TTL 60s — jackpot chỉ đổi khi settle (Power 6/55 quay 3 kỳ/tuần)
...
import type { JackpotCycleEntity } from "@megawin/game-power655/entities";
...
const fetcher = createCachedFetcher<void, JackpotCycleEntity | null>(
  ...
  async () => await getRepo().getActiveCycle(),
  {
    ...
    keyPrefix: POWER655_CACHE_KEYS.activeJackpotCycle,
    ttlSec: 60,
    negativeTtlSec: 5,
  },
);
...
/**
 * Cache read-through cho jackpot cycle đang active của Power 6/55.
 *
