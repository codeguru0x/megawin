/**
 * Power 6/55 – Ops Alert Repository
 *
 * Collection: power655_ops_alerts — 1 doc/(draw × dedupeKey). Alert-driven ops.
 *
 ...
 *
 * Port nguyên kiến trúc từ Keno (`ops-alert-repo.ts`) — KHÁC Keno: `countByStatus` nhận
 * `drawId` và trả breakdown theo status CỦA KỲ ĐÓ (dùng cho badge trang ops 1 kỳ cụ thể),
 * thay vì đếm toàn cục theo 1 status (Keno dùng cho badge global toàn hệ thống — Power655
 * chưa cần badge global ở P0).
 *
...
export class OpsAlertRepository extends BaseRepo<Power655OpsAlertEntity, OpsAlertMapper> {
  constructor() {
    super({
      collName: Power655Collections.OpsAlerts,
   ...
   */
  async bulkUpsertByDedupe(alerts: Omit<Power655OpsAlertDoc, "_id">[]): Promise<void> {
    if (alerts.length === 0) {
      return;
    }
