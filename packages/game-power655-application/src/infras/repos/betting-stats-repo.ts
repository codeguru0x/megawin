/**
 * Power 6/55 – Draw Betting Stats Repository
 *
 * Collection: power655_draw_betting_stats — 1 doc/draw, pre-aggregated ops stats.
 *
 * ĐỌC: `getByDrawId` → O(1) cho backoffice ops dashboard.
 * GHI: `applyDelta` — `$inc` theo path cố định + `$set lastEntryId` trong CÙNG 1 lệnh.
 *
 * Port nguyên kiến trúc từ Keno (`betting-stats-repo.ts`, xem JSDoc gốc cho lý giải đầy đủ
 * `$inc` theo path thay `$set` full snapshot + idempotent watermark). KHÁC Keno:
 * `byPlayType` chỉ 1 tầng lá (12 key cố định, không có slot lồng bigSmall/evenOdd);
 * `exposure` chỉ 1 field `fixedWorstCase` (jackpot đọc snapshot pool lúc build response,
 * KHÔNG lưu ở đây — xem `Power655Exposure` JSDoc, analysis §3.6).
 *
...
import {
  Power655Collections,
  type Power655DrawBettingStatsDoc,
  type Power655DrawBettingStatsEntity,
  type Power655PlayTypeStat,
...
export class BettingStatsRepository extends BaseRepo<Power655DrawBettingStatsEntity, BettingStatsMapper> {
  constructor() {
    super({
      collName: Power655Collections.DrawBettingStats,
      dataMapper: new BettingStatsMapper(),
    });
  }

  /** Đọc stats 1 kỳ — O(1) theo unique index `{ drawId }`. */
  async findByDrawId(drawId: string): Promise<Power655DrawBettingStatsEntity | null> {
  ...
  async applyDelta(drawId: string, delta: DrawStatsDelta, batchMaxId: string, topPotentialK: number): Promise<boolean> {
    ...
    if (delta.topPotential.length > 0) {
      update.$push = {
        [f("topPotential")]: {
          $each: delta.topPotential satisfies Power655TopPotential[],
   ...
   */
  async findChangedSince(since: Date, limit: number): Promise<Power655DrawBettingStatsEntity[]> {
...
/** `$inc` 3 field của 1 `Power655PlayTypeStat` tại `basePath`. */
function incPlayTypeStat(inc: Record<string, number>, basePath: string, stat: Power655PlayTypeStat): void {
