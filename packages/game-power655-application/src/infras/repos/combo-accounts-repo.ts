/**
 * Power 6/55 – Draw Combo Accounts Repository
 *
 * Collection: power655_draw_combo_accounts — 1 doc/(draw × combo × account).
 *
 * Tách khỏi `power655_draw_combo_stats` để mảng người chơi không nằm trong document —
 * xem JSDoc `Power655DrawComboAccountDoc` cho lý giải đầy đủ (port kiến trúc Keno
...
import {
  Power655Collections,
  type Power655DrawComboAccountDoc,
  type Power655DrawComboAccountEntity,
...
export class ComboAccountsRepository extends BaseRepo<Power655DrawComboAccountEntity, ComboAccountMapper> {
   ...
   * phái sinh ở `power655_draw_combo_stats`).
   *
