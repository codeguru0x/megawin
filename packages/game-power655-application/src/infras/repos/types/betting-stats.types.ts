 ...
 * Port từ Keno (`packages/game-keno-application/src/infras/repos/types/betting-stats.types.ts`).
 */
...
export interface EntryBoardForStats {
  playType: string;
  /** Số chính đã chọn ("01".."55") — KHÔNG expand thành lines. */
  mainNumbers: string[];
  /** Số line sau khi expand (standard=1, bao5=50, bao7-18=C(N,6)). */
 ...
 * (tránh đọc baseline 33KB × D kỳ mỗi tick, xem `betting-stats-repo.ts`).
 */
 ...
 */
export type PartialPlayTypeDelta = Partial<Record<PlayType, Power655PlayTypeStat>>;

/**
 * Delta tần suất 1 số trong 1 kỳ — ghi `power655_draw_number_stats` bằng `$inc` upsert.
 *
 * Đếm theo `board.mainNumbers` (KHÔNG expand lines) — xem JSDoc `Power655DrawNumberStatsDoc`.
 */
...
/**
 * Delta tích luỹ 1 account trong 1 kỳ — ghi `power655_draw_account_stats` bằng `$inc` upsert.
 *
 ...
 * `power655_draw_combo_accounts` bằng `$inc` upsert. `sets`/`amount` luôn ≥ 0.
 */
 ...
 *
 * `comboKey = `${playType}:${sortedMainNumbers.join(",")}``. Track MỌI play type (không
 * chỉ cappable — Power 6/55 không có cap) vì doc chỉ sinh cho combo thực sự có người cược.
 */
