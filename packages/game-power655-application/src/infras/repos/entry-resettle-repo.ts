/**
 * Power 6/55 – Entry Resettle Repository
 *
 * Collection: power655TicketEntries (chỉ entries trong phiên resettle)
 *
...
import { Power655Collections, type EntryReversal, type TicketEntryEntity } from "@megawin/game-power655/entities";
...
export class EntryResettleRepository extends BaseRepo<TicketEntryEntity, EntryMapper> {
   ...
   *
   * Fields bị $unset (xoá hoàn toàn data settle cũ — Power 6/55):
   * - `payout` — winAmount, payoutAmount, tiers, payoutTx, settledAt.
   * - `outcome` — "win" / "loss".
   * - `result` — snapshot kết quả quay (winningMain, bonusNumber, publishedAt).
   *
   * Power 6/55 KHÔNG có `hasCappablePrize` → chỉ unset 3 field trên.
   *
  ...
  async resetEntriesForResettle(drawId: string): Promise<number> {
    ...
    const result = await this.updateMany(
      { drawId, status: EntryStatus.Settled },
      {
        ...
        // Xoá các field được set trong bulkSettleEntries (Power655 không có hasCappablePrize)
        $unset: {
          payout: "",
          outcome: "",
          result: "",
