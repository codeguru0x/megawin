...
// ─────────────────────────────────────────────
import { PlayMode, PrizeTier } from "@megawin/game-max3dpro/entities/enums";
...
describe("expandSelectionToPairs – multiNumber → P(n,2) ordered pairs", () => {
  ...
  it("kịch bản thực tế: player chọn ['096','389','683'], Special=['096','389'] → 1 pair trúng ĐB, 1 pair trúng phụ ĐB", () => {
    ...
    for (const p of pairs) {
      ...
      if (tierNames(m).includes(PrizeTier.Special)) {
        hasSpecial = true;
      }
      if (tierNames(m).includes(PrizeTier.SpecialSub)) {
        hasSpecialSub = true;
      }
    }
