/**
 * Max 3D Pro – Entry Resettle Repository
 *
 * Collection: max3d_pro_ticketEntries (chỉ entries trong phiên resettle)
 *
...
export class EntryResettleRepository extends BaseRepo<TicketEntryEntity, EntryMapper> {
   ...
   *
   * Fields bị $unset (xoá hoàn toàn data settle cũ — Max 3D Pro):
   * - `payout` — winAmount, payoutAmount, tiers, payoutTx, settledAt.
   * - `outcome` — "win" / "loss".
   * - `result` — snapshot kết quả quay (special, first, second, third).
   *
   * Max 3D Pro KHÔNG có `hasCappablePrize` (không có payout cap) → chỉ unset 3 field trên.
   *
  ...
  async resetEntriesForResettle(drawId: string): Promise<number> {
    ...
    const result = await this.updateMany(
      { drawId, status: EntryStatus.Settled },
      {
        ...
        // Xoá các field được set trong bulkSettleEntries (Max3DPro không có hasCappablePrize)
        $unset: {
          payout: "",
          outcome: "",
          result: "",
