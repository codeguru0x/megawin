/**
 * Aggregate result types cho outstanding drill-down queries — Max 3D Pro.
 *
 ...
 * Max 3D Pro có lineCount (multiNumber: P(n,2) ordered pairs, multiDigit: perms×perms).
 */
export interface OutstandingTenantBreakdownRow {
  ...
  playerCount: number;
  /** Tổng số cặp số pending. Max 3D Pro: P(n,2) hoặc perms(front)×perms(back). */
  lineCount: number;
