...
/**
 * Max 3D Pro audit-log helpers — nhóm free functions ghi audit log cho domain
 * Max 3D Pro.
 *
 * Đóng băng sẵn `game`/`category`/`targetType`/`action` của Max 3D Pro → use-case
 * chỉ truyền payload nghiệp vụ tối thiểu (actor + id + diff).
 *
 * Phân tầng: `@megawin/audit/logger` cung cấp `record()` (low-level, generic).
 * Module này là tầng high-level riêng Max 3D Pro — là **domain service** ("ghi lại
 * chuyện gì đã xảy ra trong domain Max 3D Pro"), nên đặt trong `services/` cùng các
 * domain service khác. Phụ thuộc domain type Max 3D Pro (status, drawResult…) nên
 ...
 * class `Max3dproTicketLookupService`). Bỏ suffix `Service` ở tên để phân biệt với
 ...
 *
 * Max 3D Pro KHÔNG có Jackpot tích luỹ → KHÔNG có `auditReopenForCascade`.
 */

const GAME = GameProduct.Max3dpro;

type PublishResultArgs = {
  actor: AuditActor;
  drawId: string;
  /**
   * Kết quả quay 20 bộ ba số chia 4 hạng (2 ĐB + 4 Nhất + 6 Nhì + 8 Ba). Là
   * NỘI DUNG của thay đổi (chưa có kết quả → có kết quả) nên ghi vào `changes`.
   */
 ...
 *
 * Kết quả 4 hạng (special/first/second/third) là NỘI DUNG của thay đổi (before:
 * chưa có kết quả → after: có kết quả) nên ghi vào `changes`, KHÔNG phải
 * `metadata.extra`. Flatten theo dot-path (`special`, `first`, …) để UI hiển thị
 * từng hạng dễ đọc.
 *
...
/**
 * Audit công bố kết quả kỳ quay Max 3D Pro (publish lần đầu / sửa trước khi settle).
 *
...
/**
 * Audit công bố LẠI kết quả kỳ quay Max 3D Pro — sửa kết quả sau khi kỳ đã settle
...
/**
 * Audit staff BẤM NÚT kết sổ kỳ quay Max 3D Pro (settle lần đầu) từ BO.
 *
...
/**
 * Audit staff BẤM NÚT kết sổ LẠI kỳ quay Max 3D Pro (resettle) từ BO — sau khi đã
 * republish kết quả.
 *
 * @param args.actor - Chủ thể thực hiện.
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
...
/**
 * Audit staff mở bán kỳ quay Max 3D Pro từ BO (`scheduled|sales_closed → sales_open`).
 *
 * @param args.actor - Chủ thể thực hiện.
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
...
/**
 * Audit staff đóng bán kỳ quay Max 3D Pro từ BO (`sales_open → sales_closed`).
 *
 * @param args.actor - Chủ thể thực hiện.
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
...
/**
 * Audit staff cập nhật lịch bán/quay của kỳ quay Max 3D Pro từ BO.
 *
...
/**
 * Audit cập nhật cấu hình game Max 3D Pro toàn cục.
 *
 ...
 *
 * Nhóm lồng sâu (defaultPrizes) được flatten theo dot-path để giữ **chính xác giá
 * trị sâu** — VD `defaultPrizes.standard.special = 1e9`, không tóm tắt chung chung.
 *
 * @param args.actor - Chủ thể thực hiện.
...
/**
 * Audit cập nhật cấu hình Max 3D Pro riêng của 1 tenant (upsert).
 *
