/**
 * Use Case: Sync Outstanding (Max 3D Pro)
 *
 ...
 *   3. Merge kết quả → bulk upsert max3d_pro_outstanding_draw_reports (1 DB call)
...
export class SyncOutstandingUseCase extends InternalUseCase<void, SyncOutstandingResult> {
  ...
  protected async execute(_input: void): Promise<SyncOutstandingResult> {
    ...
    // Max 3D Pro quay T3, T5, T7 — thường có 1-2 draws active.
    ...
    // Aggregate toàn bộ max3d_pro_outstanding_draw_reports → upsert system snapshot.
