/**
 * Use Case: Get Vietlott Suggestion (Max 3D Pro)
 *
 ...
 *
 * Max 3D Pro dùng schedule kiểu C: `drawDaysOfWeek` + NHIỀU giờ quay/tuần
 * (`play.drawTimes: string[]`). Max 3D Pro có dải `drawPeriod` RIÊNG so với Max3D dù
 * cùng kiểu lịch — mỗi game một neo riêng.
 *
 * Config DB thiếu `vietlott` (neo chưa cấu hình) → trả `reason: NoAnchor`, KHÔNG
 * fallback `DEFAULT_MAX3D_PRO_CONFIG` (P0.0.1).
 */
