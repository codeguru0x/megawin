/**
 * Use Case: Trigger Resettle (Max 3D Pro) — bước 2 của workflow Resettle.
 *
 ...
 *   5. Acquire business lock `max3dpro:resettle:{drawId}` TTL 600s qua
 ...
 *
 * TTL 600s cho Max 3D Pro (cao hơn Bingo 18 = 300s) vì pipeline Max 3D Pro có thêm
...
const RESETTLE_LOCK_TTL_SECONDS = 600; // 10 phút — Max3DPro pipeline có thêm reporting steps.
...
> {
  ...
  protected async execute(input: TriggerResettleInput): Promise<TriggerResettleOutput> {
    ...
    const lockKey = buildResettleLockKey(GameProduct.Max3dpro, drawId);
