...
import type { Board, DrawEntity, EntryBoardSnapshot, TicketDoc, TicketEntryDoc } from "@megawin/game-max3dpro/entities";
...
export class PlaceBetUseCase extends UseCase<PlaceBetInput, PlaceBetOutput> {
  ...
  protected async execute(input: PlaceBetInput): Promise<PlaceBetOutput> {
    ...
    const debitInput = {
      ...
      roundIds: drawIds,
      description: `Đặt cược Max 3D Pro ${drawCount} kỳ ${drawIds[0]}${drawCount > 1 ? `→${drawIds[drawCount - 1]}` : ""}`,
      metadata: { ticketNo },
    };
