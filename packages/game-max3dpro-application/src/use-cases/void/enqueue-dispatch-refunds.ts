/**
 * Use Case: Enqueue Dispatch Refunds (Max 3D Pro).
 *
...
> {
  ...
  protected async execute(input: VoidContext): Promise<EnqueueDispatchRefundsOutput> {
    const { drawId } = input;
    const batchKey = `${GameProduct.Max3dpro}:void:${drawId}:refund`;
    ...
    while (Date.now() - startTime < MAX_EXECUTION_MS) {
      ...
      const orders = entries.map((e) =>
        buildRefundOrder({
          ...
          roundIds: [drawId],
          description: `Hoàn tiền Max 3D Pro kỳ ${drawId} – kỳ bị huỷ`,
