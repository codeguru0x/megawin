/**
 * Use Case: Dispatch Payout Batch (Max 3D Pro)
 *
...
export interface DispatchPayoutBatchResult {
  /** ID kỳ quay. */
  drawId: string;
  /** true nếu đã hết entries cần payout. */
  done: boolean;
  /** Số entries đã gửi payout thành công. */
  dispatched: number;
  /** Số entries gửi payout thất bại. */
  failed: number;
  /** Số entries bỏ qua (vượt quá retry limit). */
  skipped: number;
  /** Chi tiết kết quả payout theo từng tenant. */
  tenantResults: Array<{
    /** ID tenant. */
    tenantId: string;
    ...
    failed: number;
    /** Tổng tiền trả thưởng cho tenant (VND). */
    totalAmount: number;
  }>;
}
...
async function dispatchToTenant(
  ...
  for (const batch of batches) {
    const items: PayoutItem[] = batch.map((e: any) => ({
      ...
      description: `Trả thưởng Max 3D Pro kỳ ${drawId}`,
    }));
