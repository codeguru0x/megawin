/**
 * Use Case: Build Settle Report (Max 3D Pro)
 *
 * ═══════════════════════════════════════════════════════════════════════
 ...
 *   2. Upsert SettleTenantReport[] vào max3dpro_settle_tenant_reports
 *   3. SUM tenant reports + financials từ context → Upsert SettleDrawReport
 *
 * KHÁC BIỆT SO VỚI LOTTO535/MEGA645/POWER655:
 *   - KHÔNG có jackpotContribution (Max 3D Pro không có Jackpot — field không tồn tại)
 *   - companyTake = financials.companyTake (công ty thu toàn bộ phần dư)
 *   - CÓ lineCount (pairs per board — multiNumber hoặc multiDigit)
 *
 * CRASH-SAFE:
...
/**
 * Xây dựng per-game settle reports cho Max 3D Pro.
 *
 ...
 * Max 3D Pro KHÔNG có Jackpot: KHÔNG có field jackpotContribution. companyTake = financials.companyTake.
 * Max 3D Pro CÓ lineCount: aggregate lineCount (pairs) từ entries.
 */
...
> {
  ...
  protected async execute(input: SettleContext): Promise<BuildSettleReportResult> {
    ...
    //   - tenantAgg: group by { tenantId } → metrics tài chính + lineCount (pairs)
    ...
    // Max 3D Pro CÓ lineCount — thêm lineCount vào tenant reports
    ...
    // Max 3D Pro không có Jackpot quỹ tích luỹ → không cần chia phần dư
