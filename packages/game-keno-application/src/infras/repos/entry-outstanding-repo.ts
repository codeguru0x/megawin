/**
 * Keno – Entry Outstanding Repository
 *
 * Collection: keno_ticket_entries (status = "scheduled" only)
 *
 ...
 * Pipeline KHÔNG $sum lineCount — khác Power 6/55 và Mega 6/45.
 *
...
export class EntryOutstandingRepository extends BaseRepo<TicketEntryEntity, EntryMapper> {
   ...
   *
   * Keno KHÔNG $sum lineCount — không có lineCount trên TicketEntryDoc.
   ...
   *
   * Trả về full entity để dialog có thể hiển thị chi tiết boards (unified boards).
   * Keno boards có thể là pick1-pick10, bigSmall, hoặc evenOdd — dialog xử lý tất cả.
   * Index: { drawId: 1, tenantId: 1, accountId: 1 }
