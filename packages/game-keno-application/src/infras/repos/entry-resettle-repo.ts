 ...
 *
 * Collection: kenoTicketEntries (chỉ entries trong phiên resettle)
 *
...
export class EntryResettleRepository extends BaseRepo<TicketEntryEntity, EntryMapper> {
   ...
   * rất lớn (hàng vạn entries cho 1 draw Keno) → caller (use case) tự loop
   * cho đến khi `result.length < limit`.
   *
   ...
   * - `outcome` — "win" / "loss".
   * - `result` — snapshot kết quả quay (winningNumbers, bigCount, ...).
   * - `hasCappablePrize` — flag bậc 8/9/10 dùng cho ApplyPayoutCaps.
   *
   * KHÔNG $unset `reversal` — đã được set ở `bulkSetReversal` step trước,
   * giữ lại để EnqueueReversals đọc; FinalizeSettle (resettle path) sẽ clear.
   *
   ...
   *   - Tenant feed nhận event "vé thắng 100K → quay về chưa quay, payout=0"
   ...
   * mới" — version được bump ở `bulkSettleEntries` (re-settle) và (nếu có)
   * `bulkApplyPayoutCap`. SettleEntries query toàn bộ entries Scheduled (cả
   * thắng và thua) → mọi entry đều được bump version đúng 1 lần ở re-settle,
   * không có entry nào bị kẹt version cũ.
   *
  ...
  async resetEntriesForResettle(drawId: string): Promise<number> {
    ...
    const result = await this.updateMany(
      { drawId, status: EntryStatus.Settled },
      {
        ...
        // Xoá các field được gọi trong bulkSettleEntries (xem entry-repo.ts dòng 147-160)
        $unset: {
          payout: "",
          outcome: "",
          result: "",
