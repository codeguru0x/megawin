/**
 * Repository aggregate player daily stats từ keno_ticket_entries.
 *
...
/**
 * Aggregate player stats từ keno_ticket_entries cho PublishPlayerDaily.
 *
 ...
 *
 * Keno dùng "$tenantId" trực tiếp (flat field) — khác các game khác dùng "$tenant.tenantId" (nested).
 */
  ...
  implements PlayerDailyPublisher
{
   ...
   *
   * Keno dùng "$tenantId" flat — khác các game khác dùng "$tenant.tenantId" nested.
   * Index: { financialDate: 1, status: 1 }
   */
  async aggregatePlayersFromEntries(financialDate: string): Promise<PlayerDailyAggregateResult[]> {
    const result = await this.aggregate([
      ...
      // Keno: "$tenantId" là flat field, không nested trong "$tenant.tenantId"
      {
        $group: {
          _id: {
            tenantId: "$tenantId",
