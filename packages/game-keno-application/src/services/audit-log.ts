...
/**
 * Keno audit-log helpers — nhóm free functions ghi audit log cho domain Keno.
 *
 * Đóng băng sẵn `game`/`category`/`targetType`/`action` của Keno → use-case chỉ
 * truyền payload nghiệp vụ tối thiểu (actor + id + diff).
 *
 * Phân tầng: `@megawin/audit/logger` cung cấp `record()` (low-level, generic).
 * Module này là tầng high-level riêng Keno — là **domain service** ("ghi lại
 * chuyện gì đã xảy ra trong domain Keno"), nên đặt trong `services/` cùng các
 * domain service khác. Phụ thuộc domain type Keno (status, winningNumbers…) nên
 ...
 * class `KenoTicketLookupService`). Bỏ suffix `Service` ở tên để phân biệt với
 ...
 *
 * Bộ số trúng là NỘI DUNG của thay đổi (before: chưa có kết quả → after: có kết
 * quả) nên ghi vào `changes`, KHÔNG phải `metadata.extra`. UI hiển thị dạng
 * "01, 56, 03, …" dễ đọc.
 *
...
function recordPublishResult(
  ...
  record({
    ...
    targetId: args.drawId,
    targetLabel: `Kỳ ${args.drawId}`,
    changes: { after: { winningNumbers: args.winningNumbers } },
    metadata: {
      http: args.meta,
      extra: dropUndefined({
...
/**
 * Audit công bố kết quả kỳ quay Keno (publish lần đầu / sửa trước khi settle).
 *
 ...
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
 * @param args.winningNumbers - Bộ số trúng (zero-padded `"01"`–`"80"`).
...
/**
 * Audit công bố LẠI kết quả kỳ quay Keno — sửa kết quả sau khi kỳ đã settle
...
/**
 * Audit staff BẤM NÚT kết sổ kỳ quay Keno (settle lần đầu) từ BO.
 *
...
/**
 * Audit staff BẤM NÚT kết sổ LẠI kỳ quay Keno (resettle) từ BO — sau khi đã
 * republish kết quả.
 *
 * @param args.actor - Chủ thể thực hiện.
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
...
/**
 * Audit staff mở bán kỳ quay Keno từ BO (`scheduled|sales_closed → sales_open`).
 *
 * @param args.actor - Chủ thể thực hiện.
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
...
/**
 * Audit staff đóng bán kỳ quay Keno từ BO (`sales_open → sales_closed`).
 *
 * @param args.actor - Chủ thể thực hiện.
 * @param args.drawId - Id kỳ quay (`YYYY-MM-DD.NNN`).
...
/**
 * Audit staff cập nhật lịch bán/quay của kỳ quay Keno từ BO.
 *
...
/**
 * Audit cập nhật cấu hình game Keno toàn cục.
 *
 * Chỉ ghi **giá trị mới** (`changes.after`) của các nhóm field đã đổi
 * (rates/basicPrizes/…). KHÔNG ghi `before`: config append-only + có `version`,
 * muốn biết đổi gì thì trace ngược record version trước.
 *
 * Nhóm lồng sâu (basicPrizes) được flatten theo dot-path để giữ **chính xác giá
 * trị sâu** — VD `basicPrizes.pick8.match5 = 10`, không tóm tắt chung chung.
 *
 * @param args.actor - Chủ thể thực hiện.
...
/**
 * Audit cập nhật cấu hình Keno riêng của 1 tenant (upsert).
 *
...
export function auditUpdateTenantConfig(args: {
  ...
  record({
    ...
    targetLabel: `Cấu hình Keno tenant ${args.tenantId}`,
