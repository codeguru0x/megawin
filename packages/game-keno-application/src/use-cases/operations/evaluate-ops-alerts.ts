...
/** Ngữ cảnh đánh giá alert — ngưỡng động + cap kỳ, đọc 1 lần/invocation. */
interface AlertContext {
  /** Ngưỡng động `ops.alerts` từ GlobalConfig. */
  alerts: OpsAlertsConfig;
  /** Cap trả thưởng kỳ — mẫu số exposure %, ngưỡng cap_sets_near. */
  caps: PayoutCaps;
}
...
export class EvaluateOpsAlertsUseCase extends TickLoopWorker<void, EvaluateOpsAlertsResult> {
  ...
  protected async beforeLoop(): Promise<void> {
    ...
    this.alertCtx = {
      ...
      caps: config.payoutCaps,
    };

    this.tickMs = config.ops.stats.tickSeconds * 1000; // dùng CHUNG nhịp với sync (analysis §5.1)
  ...
  private async evaluateDoc(stats: KenoDrawBettingStatsEntity): Promise<void> {
    const combos = this.alertCtx.alerts.enabled[KenoOpsAlertType.ComboConcentration]
