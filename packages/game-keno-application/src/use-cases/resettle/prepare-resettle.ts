 ...
 *   3. Clear reversal phiên cũ (idempotent, $unset reversal field).
 ...
 *      hasCappablePrize) để Settle SFN replay với kết quả mới.
 *
 ...
 *     `tx` đảm bảo nếu reversalTx_v1 đã enqueue rồi thì v2 KHÔNG enqueue trùng
 *     nhờ idempotency `tx`. Tuy nhiên đây là edge case rất hiếm; giải pháp
 *     bền vững hơn là dùng `resettleId + entryId` derive `reversalTx` (nhưng
 *     phá tính ngẫu nhiên UUIDv7).
 *
...
export interface PrepareResettleInput {
   ...
   * `TriggerResettleUseCase` qua SFN tới `FinalizeSettle`. Build qua
   * `buildResettleLockKey(GameProduct.Keno, drawId)` trong BO API.
   */
  lockKey: string;
}
 ...
 * - `drawId`, `resettleId`, `lockOwnerToken`, `lockKey` — pass-through xuôi.
 *
 * `reversalBatchKey` KHÔNG còn trả về — `EnqueueReversalsUseCase` tự build từ
 * `drawId + resettleId` để giữ ctx gọn (xem JSDoc convention naming ở use
 * case đó).
 *
 * Metric như `reversalCount` / `resetCount` được log qua CloudWatch, KHÔNG
 * đưa vào output để tránh pollute SFN state — Choice `CheckHasReversals` đã
 * bị bỏ ở refactor trước, không còn ai consume `reversalCount` ở SFN layer.
 */
export interface PrepareResettleOutput {
  ...
  lockOwnerToken: string;
  /** Propagate cho `FinalizeSettle` để release lock đúng key. */
  lockKey: string;
}
...
export class PrepareResettleUseCase extends InternalUseCase<PrepareResettleInput, PrepareResettleOutput> {
  ...
  protected async execute(input: PrepareResettleInput): Promise<PrepareResettleOutput> {
    ...
    while (true) {
      ...
      // Update cursorId to the last entryId in the batch
      cursorId = candidates[candidates.length - 1]!.id;

      // Break if batch is less than BATCH_SIZE
    ...
    // Log metric cho CloudWatch — KHÔNG đưa vào output để giữ SFN state gọn.
