/**
 * Use Case: Get Vietlott Suggestion (Keno)
 *
 ...
 * fallback `DEFAULT_KENO_CONFIG` (P0.0.1).
 */
...
export class GetVietlottSuggestionUseCase extends UseCase<GetVietlottSuggestionInput, GetVietlottSuggestionOutput> {
  ...
  protected async execute(input: GetVietlottSuggestionInput): Promise<GetVietlottSuggestionOutput> {
    ...
    return {
      suggestedPeriod,
      reason,
      // draw.drawDate đã LÀ ngày VN của drawTime (gán lúc tạo kỳ — xem create-draw.ts) → dùng
      // thẳng, không tính lại qua formatVNDate để tránh 2 nguồn có thể lệch nhau.
