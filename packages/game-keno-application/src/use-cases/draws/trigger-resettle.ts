/**
 * Use Case: Trigger Resettle (Keno) — bước 2 của workflow Resettle.
 *
 ...
 *   5. Acquire business lock `keno:resettle:{drawId}` TTL 300s qua
 ...
 *      `settledAt` chỉ thay đổi khi FinalizeSettle ghi lại lúc kết thúc một
 *      phiên resettle → 2 phiên resettle khác nhau có 2 execution name khác nhau.
 ...
 *   - Outbox FIFO per tenant tự đảm bảo Reversal (createdAt T0) chạy TRƯỚC
 *     Payout (createdAt T1>T0) cùng player. Reversal mới enqueue chỉ xếp SAU
 *     order cũ chưa dispatch — vẫn FIFO đúng, không double-debit.
 *   - Tenant offline → block resettle vô thời hạn không tự giải. Trách nhiệm
 *     của ops dispatch dashboard, KHÔNG phải BO API.
 *   - Đồng nhất với `TriggerSettleUseCase` (settle lần đầu cũng không check).
 *
...
const RESETTLE_LOCK_TTL_SECONDS = 300; // 5 phút — đủ cho 1 phiên resettle Keno.
...
> {
  ...
  protected async execute(input: TriggerResettleInput): Promise<TriggerResettleOutput> {
    ...
    //
    // BusinessLockCoordinator wrap WorkerLockRepository — không lộ
    // ownerToken/expiresAt detail ra business layer. Coordinator throw
    // AppException(heldErrorCode, heldErrorMessage) trực tiếp khi acquire fail
    // → Next.js middleware tự render HTTP 409. ownerToken được trả về để
    // propagate qua SFN context cho FinalizeSettle release đúng owner.
    const lockKey = buildResettleLockKey(GameProduct.Keno, drawId);
    const lockOwnerToken = await this.lockCoordinator.acquire({
      ...
      heldErrorMessage: `Kỳ quay ${drawId} đang được resettle bởi phiên khác. Vui lòng đợi ~5 phút hoặc liên hệ admin.`,
    });
    ...
    try {
      ...
      // ở mức StartExecution: retry cùng phiên khiến AWS ném `ExecutionAlreadyExists`
      // (AWS giữ name unique trong 90 ngày) → catch bên dưới coi như thành công.
      //
      ...
      //
      // Hình dung token là chữ ký của "phiên resettle xảy ra SAU lần kết sổ
      // hoàn tất tại settledAt = T":
      //   - Lần đầu resettle: T = settledAt của Settle lần đầu (T1).
      //   - Resettle thành công → FinalizeSettle ghi settledAt = T2.
      //   - Republish lần 2 + Resettle lần 2 → token = T2 ≠ T1 → name mới.
      //
      ...
      // tiếp theo rơi vào nhánh "đã Settling, skip transition" + dùng cùng
      // execution name deterministic → AWS idempotent. Rollback về Published
      // chỉ thêm 1 DB write thừa, không tăng safety.
