 ...
 * thường xuất hiện thành DÃY kỳ liền nhau (`ops-hub-page-layout.guideline.md` §1.5).
 *
 ...
 * ra kỳ không nhận được vé nào (`p0-04-bulk-settle-void-api.plan.md` §1.4). Kỳ quá hạn trả
 * `DRAW_SALES_WINDOW_CLOSED`, KHÔNG gọi `OpenSalesUseCase`.
 *
...
export class BulkOpenSalesUseCase extends UseCase<BulkDrawIdsInput, BulkDrawActionOutput> {
  ...
  protected async execute(input: BulkDrawIdsInput): Promise<BulkDrawActionOutput> {
    ...
    // 1 query duy nhất cho cả lô — KHÔNG N lần getDrawById (plan §2.5).
    ...
    // đúng thứ tự uniqueDrawIds ở bước cuối, tránh chia nhóm rồi gộp sai thứ tự (plan §2.5
    // bước 5 — chỗ dễ sai nhất).
