 ...
 * Server:
 *   1. Validate: batch ≤ KENO_CREATE_DRAW_BATCH_MAX, drawDate ≥ hôm nay (giờ VN), drawTime hợp lệ
...
export class CreateDrawUseCase extends UseCase<CreateDrawInput, CreateDrawOutput> {
  ...
  protected async execute(input: CreateDrawInput): Promise<CreateDrawOutput> {
    ...
    if (inputDraws.length > KENO_CREATE_DRAW_BATCH_MAX) {
      throw AppException.badRequest(`Không thể tạo quá ${KENO_CREATE_DRAW_BATCH_MAX} kỳ một lúc.`);
    }
    ...
    // trước nhiều ngày như các game khác; số lượng đã bị `KENO_CREATE_DRAW_BATCH_MAX` chặn.
    const today = todayVN();
    ...
    for (const drawDate of dateOrder) {
      ...
      for (const [i, { index, drawTime: drawTimeIso, openNow }] of group.entries()) {
        ...
        // financialDate tính 1 LẦN/kỳ rồi dùng lại cho cả doc lưu DB và output. KHÔNG hoist ra
        // ngoài vòng lặp theo ngày: mốc tài chính là 11:00 VN, còn firstDrawTime sớm hơn nên
        // các kỳ đầu ngày thuộc financialDate của NGÀY HÔM TRƯỚC — financialDate KHÔNG hằng
        // định trong 1 drawDate.
