...
> Mega 6/45 là **single jackpot** (6/6 số), KHÔNG bonus, KHÔNG JP2. Mọi winner JP đều đóng cycle.
...
Mà tính lại số tiền cho từng kỳ chính là việc `Settle` đã làm tự động — miễn là `openingJp` của mỗi kỳ đúng. Cycle Ledger lưu `closing` từng kỳ, và hệ thống áp **bất biến `opening(K) = closing(K-1)`**: khi resettle kỳ K, `trigger-resettle` đọc `closingJp` của kỳ K-1 (vừa resettle xong) làm opening của K, rồi `FinalizeSettle` ghi đè lại `opening` trong ledger(K) cho liên tục. Vì vậy ta **cascade tuần tự**:

- Worker lo **payout + kết sổ lại** cho TỪNG kỳ (giống hệt luồng B1, `skipCycleUpdate=true`), tự lấy opening đúng từ closing kỳ trước.
- DBA chỉ **chốt jackpot cycle** (`mega645JackpotCycles`) sau mỗi kỳ — checkpoint cho cycle structure (đóng/mở cycle khi có/gỡ JP winner). DBA **không** phải sửa `openingJp` trong ledger — hệ thống tự lo.
...
2. **DBA (checkpoint)**: cập nhật `mega645JackpotCycles` phản ánh đúng winner/pool kỳ vừa rồi (đóng/mở cycle khi JP winner thay đổi). KHÔNG cần sửa `openingJp` trong ledger — worker đã tự ghi đúng.
...
sequenceDiagram
    ...
    DBA->>DB: Chốt mega645JackpotCycles theo winner/pool kỳ K (cycle structure)
...
1. **Backup MongoDB**: `mega645Draws`, `mega645TicketEntries`, `mega645JackpotCycles`, `mega645JackpotCycleEntries`.
...
```
POST /api/mega645/draws/<DRAW_ID_K>/resettle-reopen
Body: { "dbaConfirmed": true }
```

`ReopenForCascadeUseCase` re-stamp `result.publishedAt = now` (GIỮ NGUYÊN `winningNumbers`), chuyển `settled → published`, `$unset financial/stats/settleSummary` — y hệt Cách 2 nhưng idempotent + có guard (`DRAW_NEVER_SETTLED`, `DRAW_INVALID_TRANSITION`, `RESETTLE_REQUIRES_DBA`). Sau khi mở, bấm **"Kết sổ lại"**.
...
```
POST /api/mega645/draws/<DRAW_ID_K>/trigger-resettle
Body: { "dbaConfirmed": true }
```

Hệ thống tự: reversal payout cũ → reset entries → re-settle. `openingJp` của kỳ K được `trigger-resettle` resolve = `closingJp` của kỳ K-1 trong ledger (kỳ trước vừa resettle xong). `FinalizeSettle` upsert lại `ledger(K)` với opening (ghi đè) + closing mới, **không** đụng cycle.

#### Bước B — DBA chốt cycle cho kỳ K

Sau khi kỳ K `settled`. **Chỉ cập nhật `mega645JackpotCycles`** (cycle structure) — KHÔNG đụng `mega645JackpotCycleEntries` (worker đã ghi đúng opening/closing ledger).

```js
// 1. Đọc ledger entry mới của kỳ K (chỉ để THAM CHIẾU số liệu, không sửa)
const ledgerK = db.mega645JackpotCycleEntries.findOne({ drawId: "<DRAW_ID_K>" })
// → { cycleNo, seq, openingJp, jpContribution, closingJp, hasJpWinner }

// 2. Cập nhật active cycle (mega645JackpotCycles) theo winner/pool kỳ K —
...
//    - Không winner   → currentAmount = closingJp, drawCount = seq.
```

Chi tiết các trường hợp đóng/mở cycle: dùng nguyên **type-b1.md → Bước 4 (A/C)**. B2 chỉ khác là lặp lại cho từng kỳ.

> **Không cần sửa ledger opening**: opening kỳ K+1 sẽ do `trigger-resettle` tự đọc từ `ledger(K).closingJp` khi resettle kỳ K+1. DBA chỉ lo `mega645JackpotCycles`.
...
- **KHÔNG** bỏ qua bước DBA chốt cycle (`mega645JackpotCycles`) giữa các kỳ — cycle structure (đóng/mở khi JP winner thay đổi) cần đúng để báo cáo/jackpot hiển thị chính xác.
- Opening kỳ sau do **hệ thống tự resolve** từ `ledger(kỳ trước).closing` — DBA KHÔNG cần sửa `openingJp` trong `mega645JackpotCycleEntries`. Worker ghi đè opening/closing ledger mỗi kỳ.
...
Xử lý: **dừng resettle, không tự tái tính**. Báo đội kỹ thuật kiểm tra `mega645_jackpot_cycle_entries` + log `FinalizeSettle` của kỳ T. Chỉ resettle lại sau khi entry đã được khôi phục đúng. Guard này chủ yếu phòng crash (đọc `seq`/opening trên `null`). Xem [troubleshooting.md](./troubleshooting.md).
