# Mega 6/45 — Hướng dẫn Resettle (Kết sổ lại)
...
> ### ✅ Cross-cycle (gỡ JP winner) ⇒ vẫn là TYPE_B2
>
> Mega 6/45 chỉ **đóng cycle** khi có JP winner. Khi kỳ T có winner, cycle T đóng và cycle `cycleNo + 1` mở ngay.
>
> **Tình huống:** kỳ T cũ **CÓ JP winner** (đã đóng cycle), nhưng kết quả mới **gỡ bỏ JP winner** — TRONG KHI cycle kế đã có ≥1 kỳ kết sổ. Lúc này cycle T đáng lẽ **không đóng** → các kỳ đã kết sổ ở cycle kế phải **gộp ngược** vào cycle T.
>
> Chain được phát hiện theo `drawId` (`findSettledChainAfterDraw`, **xuyên cycle**) nên các kỳ ở cycle kế **vẫn nằm trong chain B2** → cascade tuần tự. Worker tự re-settle entries + payout từng kỳ; **DBA chỉ tái cấu trúc cycle metadata** (gộp/đổi `cycleNo`, reopen cycle T) giữa mỗi bước, dựa trên ledger. `opening` mỗi kỳ đọc từ `closing` kỳ liền trước theo thời gian (`findClosingJpBeforeDraw`) — đúng cả khi gộp xuyên cycle.

> ### ⚠️ Mega 6/45 là SINGLE JACKPOT
>
> Mega 6/45 chỉ có **một** giải Jackpot (trúng cả 6/6 số), **KHÔNG** có số thưởng (bonus), **KHÔNG** có cấu trúc split cycle như Power 6/55. Cycle Jackpot chỉ **đóng** khi có winner JP, nên bất biến cycle là:
>
> ```
> openingJp(T) = seedAmount + Σ contribution các kỳ < T (trong cùng cycle)
> ```
...
> Khi kết quả cũ có winner JP, cycle cũ đã **ĐÓNG** và JP đã **reset về seed**, một cycle mới đã mở. Nếu sửa thành "không winner" mà vẫn auto (TYPE_A), `FinalizeSettle` sẽ chạy với `getActiveCycle()` hiện tại (cycle mới sau khi đóng) → tính sai: jackpot bị **reset oan**, cycle structure sai. Đáng lẽ cycle cũ **không được đóng** và JP phải tiếp tục tích luỹ.
>
> Trạng thái winner CŨ đọc từ `ledgerEntry.hasJpWinner` (đã ghi lúc settle trước). Trạng thái winner MỚI lấy từ pre-flight re-match.
...
{
  "proposedWinningNumbers": ["05", "12", "23", "34", "40", "45"]
}
```
...
```
POST /api/mega645/draws/{drawId}/publish-result
{
  "winningNumbers": ["05", "12", "23", "34", "40", "45"],
...
```
POST /api/mega645/draws/{drawId}/trigger-resettle
{
