# Troubleshooting — Resettle Power 6/55
...
db.power655Draws.findOne({ drawId: "<DRAW_ID>" }, { status: 1, settledAt: 1, updatedAt: 1 })
```
...
Nếu SFN execution **đang chạy** (RUNNING): đợi nó hoàn tất (tối đa 15-20 phút với Power 6/55 pipeline).
...
Kỳ T đã `settled` nhưng không tìm thấy ledger entry trong `power655_jackpot_cycle_entries`.
...
2. Báo **đội kỹ thuật** kiểm tra:
   - `db.power655_jackpot_cycle_entries.findOne({ drawId: "<T>" })` → xác nhận entry thật sự mất.
...
## Cross-cycle (TYPE_B2) — gỡ JP1 winner ở kỳ T

Kỳ T cũ có **JP1 winner** (đã đóng cycle), nhưng kết quả mới **gỡ bỏ JP1 winner**, TRONG KHI cycle kế đã có ≥1 kỳ kết sổ.

> Power 6/55 chỉ **JP1 winner mới đóng cycle** (JP2 reset-only). Gỡ JP1 winner làm cycle T đáng lẽ **không đóng** → các kỳ đã kết sổ ở cycle kế phải **gộp ngược** vào cycle T. Trước đây bị CHẶN (`RESETTLE_CYCLE_RESTRUCTURE`); **nay đã bỏ chặn** — chain phát hiện theo `drawId` nên các kỳ này nằm trong chain B2 → resettle tuần tự, DBA chỉ tái cấu trúc cycle metadata giữa mỗi bước.

### Chẩn đoán

```js
// Ledger entry kỳ T — xác nhận cũ có JP1 winner (đóng cycle)
db.power655JackpotCycleEntries.findOne({ drawId: "<T>" }, { cycleNo: 1, seq: 1, hasJp1Winner: 1, hasJp2Winner: 1 })

// Các kỳ kết sổ SAU T theo thời gian (xuyên cycle) — chính là chain B2
db.power655JackpotCycleEntries.find({ drawId: { $gt: "<T>" } }, { drawId: 1, cycleNo: 1, seq: 1 }).sort({ drawId: 1 })
```

### Xử lý

1. Resettle **tuần tự** từng kỳ trong `chainDrawIds` (`T → T+1 → … → T+n`), mỗi kỳ `{ "dbaConfirmed": true }`.
2. **Giữa mỗi bước**, DBA tái cấu trúc cycle metadata dựa trên ledger: gộp cycle kế ngược vào cycle T (đổi `cycleNo`/`seq`, reopen cycle T, cập nhật `openingJp1/2` seed nếu cần). Worker tự re-settle entries + payout (`skipCycleUpdate=true`); DBA **không** re-settle thủ công.
...
```js
db.power655TicketEntries.countDocuments({ drawId: "<DRAW_ID>", status: "scheduled" })
```
...
for (let i = 1; i < entries.length; i++) {
  ...
  if (curr.openingJp1 !== prev.closingJp1) {
    errors.push(`seq ${curr.seq}: openingJp1 mismatch`)
  }
  if (curr.openingJp2 !== prev.closingJp2) {
    errors.push(`seq ${curr.seq}: openingJp2 mismatch`)
  }
}
