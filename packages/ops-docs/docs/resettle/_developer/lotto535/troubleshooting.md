# Troubleshooting — Resettle Lotto 5/35
...
db.lotto535Draws.findOne({ drawId: "<DRAW_ID>" }, { status: 1, settledAt: 1, updatedAt: 1 })
```
...
Nếu SFN execution **đang chạy** (RUNNING): đợi nó hoàn tất (tối đa 15-20 phút với Lotto 5/35 pipeline).
...
Kỳ T đã `settled` nhưng không tìm thấy ledger entry trong `lotto535_jackpot_cycle_entries`.
...
2. Báo **đội kỹ thuật** kiểm tra:
   - `db.lotto535_jackpot_cycle_entries.findOne({ drawId: "<T>" })` → xác nhận entry thật sự mất.
   - Kiểm tra log `FinalizeSettle` của kỳ T xem `upsertEntry` có chạy không.
   - Nếu xác định được nguyên nhân và có đủ dữ liệu (opening/closing/winner flags) → khôi phục entry đúng rồi mới resettle lại.
...
## Cross-cycle (TYPE_B2) — gỡ trạng thái đóng cycle ở kỳ T

Kỳ T cũ đã **đóng cycle** (có JP winner HOẶC đã Split), nhưng kết quả mới **gỡ trạng thái đóng đó**, TRONG KHI cycle kế đã có ≥1 kỳ kết sổ.

> Trước đây tình huống này bị CHẶN (`RESETTLE_CYCLE_RESTRUCTURE`). **Nay đã bỏ chặn**: chain phát hiện theo `drawId` nên các kỳ ở cycle kế nằm trong chain B2 → resettle tuần tự, **DBA chỉ tái cấu trúc cycle metadata giữa mỗi bước**.

### Chẩn đoán

```js
// Ledger entry kỳ T — xác nhận cũ đóng cycle (hasJpWinner hoặc didSplit = true)
db.lotto535JackpotCycleEntries.findOne({ drawId: "<T>" }, { cycleNo: 1, seq: 1, hasJpWinner: 1, didSplit: 1 })

// Các kỳ kết sổ SAU T theo thời gian (xuyên cycle) — chính là chain B2
db.lotto535JackpotCycleEntries.find({ drawId: { $gt: "<T>" } }, { drawId: 1, cycleNo: 1, seq: 1 }).sort({ drawId: 1 })
```

### Xử lý

1. Resettle **tuần tự** từng kỳ trong `chainDrawIds` (`T → T+1 → … → T+n`), mỗi kỳ `{ "dbaConfirmed": true }`.
2. **Giữa mỗi bước**, DBA tái cấu trúc cycle metadata dựa trên ledger: vì kỳ T không còn đóng cycle, gộp cycle kế ngược vào cycle T (đổi `cycleNo`/`seq` của các kỳ kế, reopen cycle T). Worker tự re-settle entries + payout (`skipCycleUpdate=true`); DBA **không** re-settle thủ công.
...
```js
db.lotto535TicketEntries.countDocuments({ drawId: "<DRAW_ID>", status: "scheduled" })
```
...
```js
const entries = db.lotto535JackpotCycleEntries.find(
  { cycleNo: <CYCLE_NO> },
  { sort: { seq: 1 } }
).toArray()
...
for (let i = 1; i < entries.length; i++) {
  ...
  if (curr.opening !== prev.closing) {
    errors.push(`seq ${curr.seq}: opening mismatch (opening=${curr.opening} ≠ prev.closing=${prev.closing})`)
  }
}
...
> **Lưu ý Split**: sau cascade B2, kiểm tra thêm `didSplit` từng kỳ Evening đã đúng chưa — split có thể chuyển kỳ do opening đổi dây chuyền. Đối chiếu `entries.filter(e => e.didSplit)` với cycle đóng `closeReason: "split"`.
