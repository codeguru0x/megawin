 ...
 * Auth API Module
 *
 * Quản lý vòng đời token: tự động refresh trước khi hết hạn, logout.
 *
 * **Flow xác thực:**
 * 1. Server tenant gọi MegaWin server-to-server API lấy tokens (accessToken, refreshToken)
...
export interface AuthApi {
   ...
   * Sau khi set, mọi request sẽ tự gửi Bearer accessToken.
   ...
   *
   * @returns `true` nếu có access token chưa hết hạn
   *
   * @example
   * ```ts
  ...
  /**
   * Logout — revoke token trên server và xóa local tokens.
   *
   * Best-effort: nếu server lỗi, local tokens vẫn bị xóa.
   *
   * @throws {@link ApiClientError} nếu request lỗi (nhưng tokens vẫn bị xóa)
   *
   * @example
   * ```ts
   * await client.auth.logout();
   * // Session đã kết thúc, cần set tokens mới để tiếp tục
   * ```
   */
  logout(): Promise<void>;
}
...
export function createAuthApi(deps: AuthApiDeps): AuthApi {
  const { publicHttp, authedHttp, tokenManager, onSessionExpired } = deps;
  ...
  tokenManager.setRefreshFn(refreshAccessToken);
    ...
    async logout(): Promise<void> {
      ...
      try {
        if (tokens?.refreshToken) {
          await authedHttp.post(ENDPOINTS.auth.logout, {
            refreshToken: tokens.refreshToken,
          });
        }
      } catch {
