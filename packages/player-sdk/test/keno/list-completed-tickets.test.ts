...
describe("keno.listCompletedTickets", () => {
  ...
  it("should call GET /player/keno/tickets/completed without params", async () => {
    const responseData = { tickets: [COMPLETED_TICKET], nextCursor: null, size: 20 };
    ...
    expect(url).toBe("https://api.test.com/player/keno/tickets/completed");
    expect(init.method).toBe("GET");
  });

  it("should pass all query params including sortBy and date range", async () => {
    const fetchMock = mockFetch({ tickets: [], nextCursor: null, size: 10 });
    vi.stubGlobal("fetch", fetchMock);

    await client.keno.listCompletedTickets({
      size: 10,
      sortBy: "drawDate",
      from: "2026-02-01",
      to: "2026-02-28",
    ...
    expect(url).toContain("sortBy=drawDate");
  ...
  it("should return tickets with settlement info", async () => {
    const fetchMock = mockFetch({ tickets: [COMPLETED_TICKET], nextCursor: null, size: 20 });
    vi.stubGlobal("fetch", fetchMock);
