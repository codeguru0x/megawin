...
describe("GameConfigRepository – Global Config", () => {
  ...
  it("global config có đầy đủ jackpot settings", async () => {
    ...
    expect(config!.jackpot.seedAmount).toBe(DEFAULT_LOTTO535_CONFIG.jackpot.seedAmount);
    expect(config!.jackpot.splitThreshold).toBe(DEFAULT_LOTTO535_CONFIG.jackpot.splitThreshold);
    expect(config!.jackpot.splitRatios).toEqual(DEFAULT_LOTTO535_CONFIG.jackpot.splitRatios);
  });
  ...
  it("global config có đầy đủ default prizes", async () => {
    ...
    expect(config!.defaultPrizes.tier1).toBe(DEFAULT_LOTTO535_CONFIG.defaultPrizes.tier1);
    expect(config!.defaultPrizes.tier2).toBe(DEFAULT_LOTTO535_CONFIG.defaultPrizes.tier2);
  ...
  it("upsertGlobalConfig cập nhật partial fields", async () => {
    ...
    expect(updated!.jackpot.splitThreshold).toBe(DEFAULT_LOTTO535_CONFIG.jackpot.splitThreshold);
  });
