/**
 * Lotto 5/35 – Sync Betting Stats Worker
 *
 * Cập nhật `lotto535_draw_betting_stats` (+ number/account/combo stats) async cho mọi kỳ
 * chưa `final` — KHÔNG đụng hot path place-bet (analysis §3.1–3.3). Extends
 * {@link TickLoopWorker}: 1 worker toàn hệ (`lotto535:stats-sync`), vòng lặp/budget/cadence
 * nằm ở base class (packages/worker-core) — xem JSDoc `TickLoopWorker` cho cơ chế loop.
 *
 * Port nguyên kiến trúc từ Power 6/55 (`sync-betting-stats.ts`, xem JSDoc gốc cho lý giải
 * đầy đủ "hàng đợi việc theo trạng thái công việc, không theo status draw" + "vì sao ghi
 * theo batch, không gom cả tick vào RAM"). KHÁC Power 6/55:
 *
 * - `PrizeContext.tier1` = giải 5 chính KHÔNG ĐB (Power655 tier1 = trùng 5/6). Ngưỡng
 *   `largeBetAmount` đọc từ `config.ops.alerts` (Lotto 5/35 riêng, không dùng chung Power655).
 * - `MAX_ENTRIES_PER_DRAW_PER_TICK`/`MAX_DRAWS_PER_TICK` GIỮ NGUYÊN hằng số Power 6/55/Keno
 *   (Lotto 5/35 quay 2 kỳ/ngày — trần này không mang ý nghĩa scale riêng cho game, giữ để
 *   đồng nhất codebase, dễ đối chiếu).
 *
 * ## Mỗi invocation
 *
 ...
 * `lotto535:ops-alerts`, tách theo analysis §5.1: lỗi rule alert không làm chậm sync,
...
import { Lotto535StatsAccumulator, type PrizeContext } from "./stats-accumulator";
 ...
 *
 * GIỮ NGUYÊN giá trị Power 6/55/Keno (analysis: Lotto 5/35 quay 2 kỳ/ngày, trần này không
 * mang ý nghĩa scale riêng cho game — giữ để đồng nhất codebase, dễ đối chiếu). Không có
 * trần thì kỳ tồn đọng lớn (worker chết vài giờ rồi bật lại) sẽ hút hết budget invocation và
 * bỏ đói các kỳ khác. Vượt trần thì phần còn lại để tick sau: watermark đã tiến nên tick sau
 * tiếp tục liền mạch, không đọc lại.
 */
const MAX_ENTRIES_PER_DRAW_PER_TICK = 20_000;

/**
 * Trần số kỳ xử lý trong 1 tick — GIỮ NGUYÊN giá trị Power 6/55/Keno (xem JSDoc
...
export class SyncBettingStatsUseCase extends TickLoopWorker<void, SyncBettingStatsResult> {
  ...
  protected override readonly description = "Lotto 5/35 — đồng bộ thống kê cược theo delta (tick, mọi kỳ đang mở)";
  ...
  protected override async beforeLoop(): Promise<void> {
    ...
    // `DEFAULT_LOTTO535_CONFIG.ops` khi thiếu, tránh crash `beforeLoop` (R2 p0-01, R7 p0-02).
    this.statsConfig = config.ops?.stats ?? DEFAULT_LOTTO535_CONFIG.ops.stats;
    this.counters = { entriesApplied: 0, finalized: 0, failed: 0 }; // reset — container reuse

    // Enroll 1 lần/invocation (đồng bộ lý luận Power 6/55/Keno): draws được staff tạo batch
    // cho cả ngày; kỳ tạo GIỮA invocation chờ tối đa ~55s để vào hàng đợi — vô nghĩa so với
    // chu kỳ draw (2 kỳ/ngày). Nhờ vậy `runTick` còn đúng 1 câu chuyện: lấy hàng đợi → hút
    // delta → đóng dấu.
  ...
  private async syncDraw(
    ...
    while (true) {
      ...
      const acc = new Lotto535StatsAccumulator(drawId, prize);
      ...
      if (!ok) {
        throw new LockTakenOverError("lotto535:stats-sync");
      }
   ...
   * `lotto535_draw_combo_accounts` rồi `$set` tuyệt đối, nên tự hội tụ sau mọi crash (khác
   * `$inc` theo "account mới trong tick" — mất là mất vĩnh viễn).
   */
  private async writeBatch(
    drawId: string,
    acc: Lotto535StatsAccumulator,
    batchMaxId: string,
    stats: OpsStatsConfig,
  ): Promise<void> {
  ...
  private buildPrizeContext(config: Pick<GlobalConfigEntity, "play" | "defaultPrizes" | "ops">): PrizeContext {
    return {
      ...
      largeBetAmount: config.ops?.alerts.largeBetAmount ?? DEFAULT_LOTTO535_CONFIG.ops.alerts.largeBetAmount,
    };
  }
}
