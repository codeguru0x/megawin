/**
 * Lotto 5/35 – List Alerts Use Case
 *
 * List alert 1 kỳ cho backoffice panel (on-demand, chỉ khi mở panel — analysis §4.1
 * mirror Power 6/55). Mặc định gộp theo `type` cho gọn; `grouped=false` trả raw để
...
export class ListAlertsUseCase extends NextApiUseCase<ListAlertsInput, ListAlertsOutput> {
  ...
  private groupByType(items: Lotto535OpsAlertEntity[]): Lotto535AlertGroup[] {
    const byType = new Map<Lotto535OpsAlertType, Lotto535AlertGroup>();
