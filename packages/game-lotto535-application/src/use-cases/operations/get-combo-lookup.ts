/**
 * Lotto 5/35 – Get Combo Lookup Use Case (staff)
 *
 * Tra cứu 1 board (bộ số theo playType) trong 1 kỳ cho staff — đọc pre-aggregated
 * `lotto535_draw_combo_stats` (counter, O(1) theo unique index) + breakdown account
 * từ `lotto535_draw_combo_accounts`. PlayType TỰ SUY ở UI theo số lượng số chính +
 * đặc biệt đã chọn (analysis §3.10(7)).
 *
...
export class GetComboLookupUseCase extends NextApiUseCase<GetComboLookupInput, GetComboLookupOutput> {
  ...
  protected async execute(input: GetComboLookupInput): Promise<GetComboLookupOutput> {
    ...
    // Breakdown account nằm ở collection riêng (`lotto535_draw_combo_accounts`) — doc
