/**
 * Use Case: Reopen For Cascade (Lotto 5/35) — mở cổng resettle cho kỳ T+n trong
 ...
 * (T+1…T+n) phải re-settle vì pool jackpot tích luỹ + ranh giới split cycle đổi.
 * NHƯNG số quay (winningMain + winningSpecial) của T+1…T+n KHÔNG đổi — chỉ số
 * tiền/cycle/split tính lại.
 *
 ...
 * now` (GIỮ NGUYÊN winningMain + winningSpecial), chuyển `Settled → Published`,
 * $unset data settle cũ → mở cổng để staff bấm "Kết sổ lại" cho kỳ T+n như bình
 * thường.
 *
 * ── Guard (chặt, an toàn) ───────────────────────────────────────────────────
 ...
 *
 * Suy luận tự động từ "kỳ trước đang dở resettle" KHÔNG robust: cascade chạy tuần
 * tự nên ngay khi kỳ T+n-1 resettle xong nó về `Settled`, query trả null → reject
 * oan T+n. Cửa sổ hợp lệ chỉ vài giây lúc kỳ trước đang `Settling` → bất khả thi
 * vận hành. Do đó an toàn dựa trên 2 lớp:
 ...
 * "Kết sổ lại" (TriggerResettle) sẽ vào luồng B2 bình thường. Số quay không đổi
 * nên `detectBoundaries` re-match cho ra winner state giống cũ; số tiền tính lại
 * theo opening mới (closing kỳ trước vừa resettle).
 *
...
export class ReopenForCascadeUseCase extends UseCase<ReopenForCascadeInput, ReopenForCascadeOutput> {
  ...
  protected async execute(input: ReopenForCascadeInput): Promise<ReopenForCascadeOutput> {
    ...
    // ── Mở cổng: re-stamp publishedAt, GIỮ số quay, settled → published ───
