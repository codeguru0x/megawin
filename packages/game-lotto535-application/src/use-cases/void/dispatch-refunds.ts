 ...
 *
 * Flow mỗi lần gọi:
 *   1. Query batch entries voided chưa refund
 *   2. Group by tenantId
 *   3. Gửi batchRefund cho từng tenant (chunk ≤ 50)
 *   4. Mark dispatched/failed per entry
    ...
    try {
      ...
      for (const r of response.results) {
        if (r.status === "success" || r.status === "duplicate") {
          ...
          await this.entryRepo.markRefundFailed(
            r.entryId,
