/**
 * Cache module: Active Jackpot Cycle (Lotto 5/35) — CHỈ cho read path HIỂN THỊ.
 *
 ...
 *
 * Policy: read-through TTL 60s — jackpot chỉ đổi khi settle (Lotto 5/35 có 2 kỳ/ngày)
...
const fetcher = createCachedFetcher<void, JackpotCycleEntity | null>(
  ...
  async () => await getRepo().getActiveCycle(),
  {
    ...
    keyPrefix: LOTTO535_CACHE_KEYS.activeJackpotCycle,
    ttlSec: 60,
    negativeTtlSec: 5,
  },
);
...
/**
 * Cache read-through cho jackpot cycle đang active của Lotto 5/35.
 *
