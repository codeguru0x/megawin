/**
 * Lotto 5/35 – Entry Resettle Repository
 *
 * Collection: lotto535TicketEntries (chỉ entries trong phiên resettle)
 *
...
import { Lotto535Collections, type EntryReversal, type TicketEntryEntity } from "@megawin/game-lotto535/entities";
...
export class EntryResettleRepository extends BaseRepo<TicketEntryEntity, EntryMapper> {
   ...
   *
   * Fields bị $unset (xoá hoàn toàn data settle cũ — Lotto 5/35):
   * - `payout` — winAmount, payoutAmount, tiers, payoutTx, settledAt.
   * - `outcome` — "win" / "loss".
   * - `result` — snapshot kết quả quay (winningMain, winningSpecial, publishedAt).
   *
   * Lotto 5/35 KHÔNG có `hasCappablePrize` → chỉ unset 3 field trên.
   *
  ...
  async resetEntriesForResettle(drawId: string): Promise<number> {
    ...
    const result = await this.updateMany(
      { drawId, status: EntryStatus.Settled },
      {
        ...
        // Xoá các field được set trong bulkSettleEntries (Lotto535 không có hasCappablePrize)
        $unset: {
          payout: "",
          outcome: "",
          result: "",
