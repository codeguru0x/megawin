/**
 * Use Case: Enqueue Max 3D Reversal Orders (Step 2 của Resettle SFN).
 *
...
export class EnqueueReversalsUseCase extends InternalUseCase<EnqueueReversalsInput, EnqueueReversalsOutput> {
  ...
  protected async execute(input: EnqueueReversalsInput): Promise<EnqueueReversalsOutput> {
    ...
    const reversalBatchKey = buildResettleBatchKey(GameProduct.Max3d, drawId, resettleId, "reversal");
    ...
    while (true) {
      ...
      const orders = entries.map((e) =>
        buildReversalOrder({
          ...
          roundIds: [drawId],
          description: `Thu hồi Max 3D kỳ ${drawId} (resettle, vé ${e.ticketNo})`,
