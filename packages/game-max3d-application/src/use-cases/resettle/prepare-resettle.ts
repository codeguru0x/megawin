/**
 * Use Case: Prepare Max 3D Resettle (Step 1 của Resettle SFN).
 *
 * Trách nhiệm:
 ...
 *      Max 3D KHÔNG có `hasCappablePrize`) để Settle SFN replay với kết quả mới.
 *
...
export class PrepareResettleUseCase extends InternalUseCase<PrepareResettleInput, PrepareResettleOutput> {
  ...
  protected async execute(input: PrepareResettleInput): Promise<PrepareResettleOutput> {
    ...
    // Max 3D KHÔNG có `hasCappablePrize` → repo method tự giới hạn
