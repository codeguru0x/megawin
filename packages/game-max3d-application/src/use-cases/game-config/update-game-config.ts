...
import type { OpsConfig, PlayRules, VietlottPeriodAnchor } from "@megawin/game-max3d/entities";
...
export class UpdateGameConfigUseCase extends UseCase<UpdateGameConfigInput, UpdateGameConfigOutput> {
  ...
  protected async execute(input: UpdateGameConfigInput): Promise<UpdateGameConfigOutput> {
    ...
    if (merged.vietlott) {
      this.validateVietlottAnchor(merged.vietlott, merged.play ?? existing?.play ?? DEFAULT_MAX3D_CONFIG.play);
    }
