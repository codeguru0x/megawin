/**
 * Use Case: Get Vietlott Suggestion (Max 3D)
 *
 ...
 *
 * Max 3D dùng schedule kiểu C: `drawDaysOfWeek` + NHIỀU giờ quay/tuần
 * (`play.drawTimes: string[]` — KHÁC field scalar `drawTime` của Mega645, xem
 * `p4-slow-games.plan.md`). Max 3D có dải `drawPeriod` RIÊNG so với Max3D Pro dù
 * cùng kiểu lịch — mỗi game một neo riêng.
 *
 * Config DB thiếu `vietlott` (neo chưa cấu hình) → trả `reason: NoAnchor`, KHÔNG
 * fallback `DEFAULT_MAX3D_CONFIG` (P0.0.1).
 */
