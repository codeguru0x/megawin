/**
 * Use Case: Create Draws (Max 3D) – Batch
 *
...
import type { DrawDoc, DrawNo } from "@megawin/game-max3d/entities";
...
export class CreateDrawsUseCase extends UseCase<CreateDrawsInput, CreateDrawsOutput> {
  ...
  protected async execute(input: CreateDrawsInput): Promise<CreateDrawsOutput> {
    ...
    if (inputDraws.length < 1 || inputDraws.length > MAX3D_CREATE_DRAW_BATCH_MAX) {
      throw AppException.badRequest(`Số kỳ tạo phải từ 1 đến ${MAX3D_CREATE_DRAW_BATCH_MAX}.`);
    }
    ...
    // Guard trùng ngày NGAY TRONG LÔ — Max 3D chỉ 1 kỳ/ngày nên 2 dòng cùng drawDate là staff
