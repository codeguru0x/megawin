 ...
 *
 * RESETTLE PATH:
 * - `resettleContext` present → derive batchKey resettle (`max3d:resettle:
...
export class EnqueueDispatchPayoutsUseCase extends UseCase<EnqueueDispatchPayoutsInput, EnqueueDispatchPayoutsOutput> {
  ...
  protected async execute(input: EnqueueDispatchPayoutsInput): Promise<EnqueueDispatchPayoutsOutput> {
    ...
    const batchKey = resettleContext
      ? buildResettleBatchKey(GameProduct.Max3d, drawId, resettleContext.resettleId, "payout")
      : `${GameProduct.Max3d}:settle:${drawId}:payout`;
    ...
    while (Date.now() - startTime < MAX_EXECUTION_MS) {
      ...
      const orders = entries.map((e) =>
        buildPayoutOrder({
          ...
          roundIds: [drawId],
          description: `Trả thưởng Max 3D kỳ ${drawId}${descSuffix}`,
