/**
 * Max 3D – Entry For Stats Mapper
 *
 * Map raw doc (projection tối thiểu từ `max3d_ticket_entries`) → `EntryForStats` —
...
export function mapDocToEntryForStats(d: Document): EntryForStats {
  return {
    ...
    boards: (d.entrySummary?.boards ?? []).map((b: Document) => ({
      ...
      // Fallback 1 cho data cũ thiếu betCount (max3d-game-rules #23).
