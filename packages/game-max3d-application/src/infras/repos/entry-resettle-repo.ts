/**
 * Max 3D – Entry Resettle Repository
 *
 * Collection: max3d_ticketEntries (chỉ entries trong phiên resettle)
 *
...
import { Max3dCollections, type EntryReversal, type TicketEntryEntity } from "@megawin/game-max3d/entities";
...
export class EntryResettleRepository extends BaseRepo<TicketEntryEntity, EntryMapper> {
   ...
   *
   * Fields bị $unset (xoá hoàn toàn data settle cũ — Max 3D):
   * - `payout` — winAmount, payoutAmount, boardPayouts, payoutTx, settledAt.
   * - `outcome` — "win" / "loss".
   * - `result` — snapshot kết quả quay (specialNumber, firstPrize, ...).
   *
   * Max 3D KHÔNG có `hasCappablePrize` (không có payout cap) → chỉ unset 3 field trên.
   *
  ...
  async resetEntriesForResettle(drawId: string): Promise<number> {
    ...
    const result = await this.updateMany(
      { drawId, status: EntryStatus.Settled },
      {
        ...
        // Xoá các field được set trong bulkSettleEntries (Max3D không có hasCappablePrize)
        $unset: {
          payout: "",
          outcome: "",
          result: "",
