/**
 * Cache module: Tenant Config (Max 3D).
 *
...
import { MAX3D_CACHE_KEYS } from "./keys";
...
const fetcher = createCachedFetcher<string, TenantConfigEntity | null>(
  (tenantId) => getRepo().getTenantConfig(tenantId),
  {
    ...
    keyPrefix: MAX3D_CACHE_KEYS.tenantConfig,
    ttlSec: 60 * 10, // 10 minutes
  },
);

/**
 * Cache read-through cho tenant config Max 3D (per tenantId).
 *
