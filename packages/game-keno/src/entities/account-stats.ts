/**
 * Keno – Draw Account Stats (tích luỹ cược theo account trong 1 kỳ)
 *
 * Collection: keno_draw_account_stats — 1 document / (draw × account).
 *
 * ## Vì sao cần collection riêng thay vì mảng `topAccounts` trong stats doc?
 *
 * `topAccounts` là top-K theo metric **TÍCH LUỸ** (`amount` cộng dồn). Trước p2-01 nó được
 * nuôi bằng cách seed lại top-K từ stats doc mỗi tick → account rơi khỏi top-K **mất toàn
 * bộ lịch sử**, lần cược sau tính lại từ 0 → tổng hụt, xếp hạng sai, **sai số tỷ lệ thuận
 * số người chơi** và không tự sửa (p2-01 R5/#17).
 *
 ...
 * (`mongodb.mdc` §8, checklist §7.12 của p2-01).
 *
 ...
 * `KENO_INDEXES` — xem `mongodb.mdc` §7). KHÔNG cleanup batch trong app.
 */
 ...
 */
export interface KenoDrawAccountStatsDoc extends DeltaAccumulatedDoc {
  /** MongoDB ObjectId. */
  _id: unknown;
...
export interface KenoDrawAccountStatsEntity extends Omit<KenoDrawAccountStatsDoc, "_id"> {
