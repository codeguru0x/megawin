 ...
 * │ Trùng    │ Cách tính                       │ Xác suất ≈              │
 * ├──────────┼─────────────────────────────────┼──────────────────────────┤
 * │ 5/5      │ C(5,5)×C(75,15) / C(80,20)     │ 1/1,550.57 ≈ 0.0645%   │
 * │ 4/5      │ C(5,4)×C(75,16) / C(80,20)     │ 1/82.70 ≈ 1.209%       │
 * │ 3/5      │ C(5,3)×C(75,17) / C(80,20)     │ 1/11.91 ≈ 8.394%       │
 * │ 2/5      │ C(5,2)×C(75,18) / C(80,20)     │ 1/3.70 ≈ 27.046%       │
 * └──────────┴─────────────────────────────────┴──────────────────────────┘
 *
 * ┌────────────────────────────────────────────────────────────────────────┐
 ...
 * │ Trùng    │ Cách tính                       │ Xác suất ≈              │
 * ├──────────┼─────────────────────────────────┼──────────────────────────┤
 * │ 10/10    │ C(10,10)×C(70,10) / C(80,20)   │ 1/8,911,711             │
 ...
 * │ 5/10     │ C(10,5)×C(70,15) / C(80,20)    │ 1/19.44 ≈ 5.14%        │
 * │ 0/10     │ C(10,0)×C(70,20) / C(80,20)    │ 1/21.84 ≈ 4.58%        │
 * └──────────┴─────────────────────────────────┴──────────────────────────┘
 *
 ...
 *
 * ┌────────────┬────────────────────────────────┬─────────────┬──────────────┐
 * │ Kết quả    │ Điều kiện                      │ Xác suất ≈  │ Giải mặc định│
 * ├────────────┼────────────────────────────────┼─────────────┼──────────────┤
 * │ Lớn thắng  │ bigCount ≥ 13                  │ 9.804%      │ 26,000 VND   │
 * │ Lớn 11-12  │ bigCount = 11 hoặc 12          │ 30.034%     │ 10,000 VND   │
 * │ Hoà 10-10  │ bigCount = 10                  │ 20.324%     │ 26,000 VND   │
 * │ Nhỏ 11-12  │ smallCount = 11 hoặc 12        │ 30.034%     │ 10,000 VND   │
 * │ Nhỏ thắng  │ smallCount ≥ 13                │ 9.804%      │ 26,000 VND   │
 * └────────────┴────────────────────────────────┴─────────────┴──────────────┘
 *
 *   Kiểm tra tổng: 9.804 + 30.034 + 20.324 + 30.034 + 9.804 = 100% (5 outcome
 *   này phủ kín mọi kỳ quay, vì bigCount ∈ [0,20] luôn rơi vào đúng 1 nhóm).
 *
 *   RTP THEO CỬA CƯỢC (con số quyết định lãi/lỗ, KHÔNG phải theo từng dòng):
 *     Cửa "Lớn"  thắng cả ≥13 và 11-12 → EV = 9.804% × 26,000 + 30.034% × 10,000
 *                                          ≈ 2,549 + 3,003 = 5,552 VND → RTP 55.5%
 *     Cửa "Nhỏ"  đối xứng → RTP 55.5%
 *     Cửa "Hoà"  chỉ thắng bigCount = 10 → 20.324% × 26,000 ≈ 5,284 VND → RTP 52.8%
 *   Cược "Lớn" mà kết quả nghiêng Nhỏ (smallCount ≥ 11): thua, chiếm 39.838%.
 *
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 * │ Kết quả     │ Điều kiện                     │ Xác suất ≈  │ Giải mặc định│
 * ├─────────────┼───────────────────────────────┼─────────────┼──────────────┤
 * │ Chẵn ≥15    │ evenCount ≥ 15                │ 0.939%      │ 200,000 VND  │
 * │ Chẵn 13-14  │ evenCount = 13 hoặc 14        │ 8.865%      │ 40,000 VND   │
 * │ Chẵn 11-12  │ evenCount = 11 hoặc 12        │ 30.034%     │ 20,000 VND   │
 * │ Hoà 10-10   │ evenCount = 10                │ 20.324%     │ 20,000 VND   │
 * │ Lẻ 11-12    │ oddCount = 11 hoặc 12         │ 30.034%     │ 20,000 VND   │
 * │ Lẻ 13-14    │ oddCount = 13 hoặc 14         │ 8.865%      │ 40,000 VND   │
 * │ Lẻ ≥15      │ oddCount ≥ 15                 │ 0.939%      │ 200,000 VND  │
 * └─────────────┴───────────────────────────────┴─────────────┴──────────────┘
 *
 *   Kiểm tra tổng: 2×(0.939 + 8.865 + 30.034) + 20.324 = 100%.
 *
 *   RTP THEO CỬA CƯỢC (5 cửa độc lập, xem `matchEvenOddBet`):
 *     Cửa "Chẵn"      thắng ≥15 và 13-14 → EV ≈ 5,423 VND → RTP 54.2%
 *     Cửa "Chẵn 11-12" chỉ thắng 11-12   → EV ≈ 6,007 VND → RTP 60.1%
 *     Cửa "Hoà"        chỉ thắng 10-10   → EV ≈ 4,065 VND → RTP 40.6%
 *     Cửa "Lẻ 11-12" / "Lẻ" đối xứng với 2 cửa Chẵn tương ứng.
 *   LƯU Ý: cửa "Chẵn" KHÔNG được trả khi evenCount = 11-12 — đó là cửa riêng.
 *
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 * │ 9/10  │ 1/163,381        │ 150,000,000   │ 918.10 VND      │ 9.18%        │
 * │ 8/10  │ 1/7,384          │ 8,000,000     │ 1,083.35 VND    │ 10.83%       │
 * │ 7/10  │ 1/621            │ 710,000       │ 1,143.91 VND    │ 11.44%       │
 * │ 6/10  │ 1/87             │ 80,000        │ 918.35 VND      │ 9.18%        │
 * │ 5/10  │ 1/19.44 (5.14%)  │ 20,000        │ 1,028.55 VND    │ 10.29%       │
 * │ 0/10  │ 1/21.84 (4.58%)  │ 10,000        │ 457.91 VND      │ 4.58%        │
 * ├───────┴──────────────────┴───────────────┼─────────────────┼──────────────┤
 * │ TỔNG (RTP bậc pick10)                    │ 5,774.60 VND    │ 57.75%       │
 * └──────────────────────────────────────────┴─────────────────┴──────────────┘
 *
 *   Phân bổ doanh thu mỗi lượt chơi (giá 10,000 VND, cấu hình mặc định):
 *     100% Revenue                = 10,000 VND
 *     ├── 20% Hoa hồng đại lý     =  2,000 VND  (`rates.defaultCommissionRate`)
 *     ├── Giải thưởng thực tế     = tuỳ bậc chơi & kết quả kỳ quay
 *     │                             (pick10 mặc định: RTP 57.75% ≈ 5,775 VND)
 *     └── Phần dư còn lại         = lợi nhuận công ty
 *
 *   Keno KHÔNG có tỷ lệ "thu công ty" cấu hình riêng (không có `companyRate` như
 *   Mega/Power/Lotto) và KHÔNG có jackpot tích luỹ — mọi giải đều là giải cố định,
 *   nên lợi nhuận công ty = doanh thu − hoa hồng − giải thưởng THỰC TẾ đã trả.
 *   RTP khác nhau theo từng bậc pick1–pick10, phải xem tab Giải thưởng để biết số cụ thể.
 */
